...
static usb_dev usbfd = 0;
static OS_MUTEX msd_mutex ;
...
void usb_start()
{
    //如果是按键切入而不是DP唤醒检测切入，需要先关DP唤醒检测
    usb_online_check_without_otg_disable(get_usb_check_wkup_index());
    ...
    usb_offline_check_without_otg_enable(get_usb_check_wkup_index());
    usb_state = 1;
}
...
#else
#if TCFG_USB_CDC_BACKGROUND_RUN
            switch_app_case = usb_cdc_background_standby(usbfd);
#else
            switch_app_case = usb_standby(usbfd);
#endif
#endif
...
#else
            if (true == app_cur_task_check(APP_NAME_PC)) {
#endif
                switch_app_case = 2;
            } else {
                usb_stop();
                switch_app_case = 2;//不在PC模式也需要输出拔出事件信息
...
#if 1//TCFG_ONLY_PC_ENABLE

static u16 tid_for_usb_online_check;
static u16 tid_for_usb_offline_check;
static u32 online_cnt = 0;
static u32 offline_cnt = 0;
static u32 __wkup_index;

u32 get_usb_check_wkup_index()
{
    return __wkup_index;
}

static void usb_online_check_without_otg_timer(void *priv)
{
    u32 wkup_index = (u32)priv;
    struct sys_event event = {0};

    if (usb_read_sofpnd(0)) {
        //putchar('!');
        usb_sof_clr_pnd(0);
        event.type = SYS_DEVICE_EVENT;
        event.arg = (void *)DEVICE_EVENT_FROM_OTG;
        event.u.dev.event = DEVICE_EVENT_IN;
        event.u.dev.value = (int)"s:0";
        ...
        if (++online_cnt >= 40) {
            ...
            gpio_set_dieh(IO_PORT_DP, 1);
            gpio_set_direction(IO_PORT_DP, 1);
            //恢复IO唤醒使能
            power_wakeup_index_enable(wkup_index, 1);
            goto __finish;
        }
    }
    return;
__finish:
    online_cnt = 0;
    sys_timer_del(tid_for_usb_online_check);
    tid_for_usb_online_check = 0;
}

void usb_online_check_without_otg_enable(u32 wkup_index)
{
    if (get_usb_detect_mode() != 0) {
        return;
    }
    //挂起IO唤醒
    power_wakeup_index_enable(wkup_index, 0);
    if (!tid_for_usb_online_check) {
        //extern u32 usb_otg_sof_check_init(const usb_dev id);
        usb_otg_sof_check_init(0);
        tid_for_usb_online_check = sys_timer_add(
                                       (void *)wkup_index,
                                       usb_online_check_without_otg_timer,
                                       50
                                   );
    }
    __wkup_index = wkup_index;
}

void usb_online_check_without_otg_disable(u32 wkup_index)
{
    if (get_usb_detect_mode() != 0) {
        return;
    }
...
static void usb_offline_check_without_otg_timer(void *priv)
{
    u32 wkup_index = (u32)priv;
    struct sys_event event = {0};
    static u16 last_sof_frame = 0;
    u16 sof_frame = usb_read_sofframe(0);
    u32 dm_state = usb_read_dm_se(0);

    gpio_set_pull_up(IO_PORT_DM, 1);
    if (dm_state) {
        if (sof_frame == last_sof_frame) {
            offline_cnt ++;
        } else {
...
__finish:
    offline_cnt = 0;
    sys_timer_del(tid_for_usb_offline_check);
    tid_for_usb_offline_check = 0;
}

void usb_offline_check_without_otg_enable(u32 wkup_index)
{
    if (get_usb_detect_mode() != 0) {
        return;
    }
    if (!tid_for_usb_offline_check) {
        tid_for_usb_offline_check = sys_timer_add(
                                        (void *)wkup_index,
                                        usb_offline_check_without_otg_timer,
                                        50
                                    );
    }
}

void usb_offline_check_without_otg_disable(u32 wkup_index)
{
    if (get_usb_detect_mode() != 0) {
        return;
    }
...
static u8 usbd_without_otg_idle_query(void)
{
    if (tid_for_usb_offline_check) {
        return 0;
    } else {
...
REGISTER_LP_TARGET(pd_idle_lp_target) = {
    .name = "usbd_no_otg",
    .is_idle = usbd_without_otg_idle_query,
};
#endif


#define USB_DET_WKUP_INDEX  3  //wkup_index要根据实际配置填写

static u8 usb_detect_mode = 1;  //默认值配0是从机检测，默认值配1是otg检测
static u8 __need_do_post;

u8 get_usb_detect_mode()
{
    return usb_detect_mode;
}

static int usb_detect_mode_init()
{
    if (usb_detect_mode == 0)  { /* 默认只有从机 */
        usb_otg_suspend(0, OTG_OP_NULL);
    } else { /* 默认开otg检测 */
        power_wakeup_index_enable(USB_DET_WKUP_INDEX, 0);
    }
    __wkup_index = USB_DET_WKUP_INDEX;
    return 0;
}
late_initcall(usb_detect_mode_init);

void usb_detect_switch_to_otg()
{
    struct sys_event event = {0};
    if (usb_detect_mode == 0) {
        if (!usb_state) {
            usb_online_check_without_otg_disable(USB_DET_WKUP_INDEX);
            power_wakeup_index_enable(USB_DET_WKUP_INDEX, 0);
            usb_otg_resume(0);
            usb_detect_mode = 1;
        } else {
...
void usb_detect_switch_to_slave()
{
    struct sys_event event = {0};
    if (usb_detect_mode) {
        usb_otg_suspend(0, OTG_UNINSTALL);
        if (!usb_state) {
            //恢复IO唤醒所需的IO状态
            gpio_set_pull_up(IO_PORT_DP, 1);
            gpio_set_pull_down(IO_PORT_DP, 0);
            ...
            gpio_set_direction(IO_PORT_DP, 1);
            //恢复IO唤醒使能
            power_wakeup_index_enable(USB_DET_WKUP_INDEX, 1);
            usb_detect_mode = 0;
        } else {
            __need_do_post = 1;
        }
