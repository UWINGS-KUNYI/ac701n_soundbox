...
static const u8 uac_mic_feature_desc[] = {
    ...
    0x01,      //ControlSize:1 byte
    0x01, 0x02,  //   bmaControls[0] (Mute,Volume,Automatic)
#if MIC_CHANNEL == 2
    0x00,
#endif
#if MIC_CHANNEL == 4
...
{
    u16 sample_rate = MIC_AUDIO_RATE;
    if (uac_info) {
        sample_rate = uac_info->mic_sampling_rate;
    }
    return sample_rate;
}
...
static u32 uac_endpoint_recv(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case  MIC_ISO_EP_IN|USB_DIR_IN:
        memcpy(&uac_info->mic_sampling_rate, read_ep, 3);
#if MIC_AUDIO_RATE_NUM > 1
        open_mic(usb_device);
#endif
        break;
    case SPK_ISO_EP_OUT|USB_DIR_OUT:
        memcpy(&uac_info->spk_sampling_rate, read_ep, 3);
#if SPK_AUDIO_RATE_NUM > 1
        open_spk(usb_device);
#endif
        break;
...
static void spk_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    if (uac_info->spk_alternate_setting == 1) {
        spk_res = SPK_AUDIO_RES;
    } else if (uac_info->spk_alternate_setting == 2) {
        spk_res = SPK_AUDIO_RES_2;
    }
    spk_frame_len = uac_info->spk_sampling_rate * (spk_res / 8) * SPK_CHANNEL / 1000;
    spk_frame_len += (uac_info->spk_sampling_rate % 1000 ? (spk_res / 8) * SPK_CHANNEL : 0);
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    if (uac_info->spk_sampling_rate == 0) {
        uac_info->spk_sampling_rate = spk_get_max_sampling_rate();
    }
    if (uac_info->spk_alternate_setting == 1) {
        spk_res = SPK_AUDIO_RES;
    } else if (uac_info->spk_alternate_setting == 2) {
    ...
    spk_frame_len += (uac_info->spk_sampling_rate % 1000 ? (spk_res / 8) * SPK_CHANNEL : 0);
    ep_buffer = uac_info->spk_dma_buffer;
    uac_speaker_stream_open(uac_info->spk_sampling_rate, SPK_CHANNEL, spk_res);
...
static u32 spk_as_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case USB_REQ_SET_INTERFACE:
        if (usb_device->bDeviceStates == USB_DEFAULT) {
            ...
            uac_info->spk_alternate_setting = setup->wValue;
#if USB_ROOT2
            if (!usb_root2_testing()) {
                SetInterface_0_Lock = 1;
            }
    ...
    case USB_REQ_GET_INTERFACE:
        if (usb_root2_testing()) {
            ...
            if (setup->wValue || (setup->wLength != 1)) {
                ...
                tx_len = 1;
                tx_payload[0] = uac_info->spk_alternate_setting;
                usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
            }
        }
        break;
...
static void mic_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    if (uac_info->mic_alternate_setting == 1) {
        mic_res = MIC_AUDIO_RES;
    } else if (uac_info->mic_alternate_setting == 2) {
        mic_res = MIC_AUDIO_RES_2;
    }

    mic_frame_len = ((uac_info->mic_sampling_rate * mic_res / 8 * MIC_CHANNEL) / 1000);
    //非对齐采样率补点
    if (uac_info->mic_padding_interval) {
        if (++uac_info->mic_padding_cnt == uac_info->mic_padding_interval) {
            uac_info->mic_padding_cnt = 0;
    ...
    if (len) {
        uac_info->mic_no_data = 0;
    } else if (uac_info->mic_no_data) {
        len = mic_frame_len;
...
static void open_mic(struct usb_device_t *usb_device)
{
    ...
    if (uac_info->mic_sampling_rate == 0) {
        uac_info->mic_sampling_rate = mic_get_max_sampling_rate();
    }
    if (uac_info->mic_alternate_setting == 1) {
        mic_res = MIC_AUDIO_RES;
    } else if (uac_info->mic_alternate_setting == 2) {
    ...
    mic_frame_len = ((uac_info->mic_sampling_rate * mic_res / 8 * MIC_CHANNEL) / 1000);
    mic_frame_len += (uac_info->mic_sampling_rate % 1000 ? (mic_res / 8) * MIC_CHANNEL : 0);
    if (uac_info->mic_sampling_rate % 1000) {
        uac_info->mic_padding_interval = 1000 / (uac_info->mic_sampling_rate % 1000);
        uac_info->mic_padding_cnt = 0;
    } else {
        uac_info->mic_padding_interval = 0;
        uac_info->mic_padding_cnt = 0;
    }

    uac_mic_stream_open(uac_info->mic_sampling_rate, MIC_CHANNEL, mic_res);
...
static u32 mic_as_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case USB_REQ_SET_INTERFACE:
        if (usb_device->bDeviceStates == USB_DEFAULT) {
            ...
            uac_info->mic_alternate_setting = setup->wValue;
#if USB_ROOT2
            if (!usb_root2_testing()) {
                SetInterface_0_Lock = 1;
            }
    ...
    case USB_REQ_GET_INTERFACE:
        if (usb_root2_testing()) {
            ...
            if (setup->wValue || (setup->wLength != 1)) {
                ...
                tx_len = 1;
                tx_payload[0] = uac_info->mic_alternate_setting;
                usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
            }
        }
        break;
...
u32 uac_register(const usb_dev usb_id, const u32 class)
{
    if (uac_info == NULL) {
#if USB_MALLOC_ENABLE
        uac_info = (struct uac_info_t *)zalloc(sizeof(struct uac_info_t));
        if (uac_info == NULL) {
        ...
        if (class == AUDIO_CLASS) {
            uac_info->spk_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, spk_get_max_frame_len() + mic_get_max_frame_len());
            uac_info->mic_dma_buffer = (u8 *)(((u32)uac_info->spk_dma_buffer + spk_get_max_frame_len() + 7) / 8 * 8);  //dma地址8对齐
        } else if (class == SPEAKER_CLASS) {
            uac_info->spk_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, spk_get_max_frame_len());
        } else if (class == MIC_CLASS) {
            uac_info->mic_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, MIC_ISO_EP_IN | USB_DIR_IN, mic_get_max_frame_len());
        }
    }
