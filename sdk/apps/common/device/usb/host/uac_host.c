...
static void mic_isr_hanlder(*priv)
{
    putchar('m');
}
#endif
...
static void uac_host_start(u32 usb_id)
{
    log_debug("func:%s, line:%d\n", __func__, __LINE__);
...
static void uac_host_stop(u32 usb_id)
{
    log_debug("func:%s, line:%d\n", __func__, __LINE__);
#ifdef UAC_HOST_TEST_MODE
...
static void uac_tx_isr(struct usb_host_device *priv, u32 ep)
{
    ...
    u8 buf[spk_frame_len + fill_pkt_len];
    if (fill_pkt_len) {
        fill_pkt_cnt++;
        if (fill_pkt_cnt == 10) {
...
u32 uac_host_open_spk(usb_dev usb_id, u32 ch, u32 sample_freq, u32 bit_res)
{
    ...
    usb_h_ep_config(usb_id, host_ep, USB_ENDPOINT_XFER_ISOC, 1, as_cfg.interval, ep_out_dma_buf, sizeof(ep_out_dma_buf));
...
static void uac_rx_isr(struct usb_host_device *priv, u32 ep)
{
    ...
    u32 mic_frame_len = as_cfg.cur_sample_freq * (as_cfg.bit_res / 8) * as_cfg.ch / 1000;
    mic_frame_len += ((as_cfg.cur_sample_freq % 1000) ? (as_cfg.bit_res / 8) * as_cfg.ch : 0);
    u8 buf[mic_frame_len];
    u32 rlen = usb_h_ep_read_async(0, ep, as_cfg.target_ep, buf, mic_frame_len, USB_ENDPOINT_XFER_ISOC, 0);
    if (rlen != mic_frame_len) {
#ifdef UAC_HOST_TEST_MODE
    ...
    if (as_cfg.ch == 1) {
        ...
        /* ret = cbuf_write(&spk_cbuf, buf, rlen); */
        /* ret = cbuf_write(&spk_cbuf, buf, rlen); */
        s16 buf_2[384];
        for (u32 i = 0; i < mic_frame_len / 2; i++) {
            buf_2[i * 2] = ((u16)buf[i * 2 + 1] << 8) | (u16)buf[i * 2];
            buf_2[i * 2 + 1] = ((u16)buf[i * 2 + 1] << 8) | (u16)buf[i * 2];
        }
        mic_frame_len *= 2;
        rlen = cbuf_write(&spk_cbuf, buf_2, mic_frame_len);
    } else if (as_cfg.ch == 2) {
        ret = cbuf_write(&spk_cbuf, buf, rlen);
    }
#else
    u32 ret = cbuf_write(&mic_cbuf, buf, rlen);
#endif
    if (ret != rlen) {
        /* putchar('w'); */
    } else {
...
u32 uac_host_open_mic(usb_dev usb_id, u32 ch, u32 sample_freq, u32 bit_res)
{
    ...
    usb_h_ep_config(usb_id, host_ep | USB_DIR_IN, USB_ENDPOINT_XFER_ISOC, 1, as_cfg.interval, ep_in_dma_buf, sizeof(ep_in_dma_buf));
