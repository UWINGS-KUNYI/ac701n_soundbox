...
#define AUDIO_CAPTURE_IDLE      0x0
#define AUDIO_CAPTURE_START     0x1
#define AUDIO_CAPTURE_STOP      0x2

static u8 music_player_is_audio_capture(void)
{
    return (__this->audio_capture == AUDIO_CAPTURE_START ? 1  : 0);
}

static u8 music_player_audio_capture_close(void)
{
    return (__this->audio_capture == AUDIO_CAPTURE_STOP ? 1 : 0);
}

static struct audio_path *music_player_get_caputre_path(void)
{
    return &__this->path;
}

static int get_file_dec_status(void);
static void *wireless_file_capture_open(struct audio_path *path);
static void wireless_file_capture_close(void *capture);
static void *wireless_file_player_open(struct audio_path *path);
static void wireless_file_player_close(void *player);
static void wireless_file_capture_wakeup(void *capture);

static struct live_audio_mode_ops wireless_file_audio_ops = {
    .capture_open = wireless_file_capture_open,
    .capture_close = wireless_file_capture_close,
    .capture_wakeup = wireless_file_capture_wakeup,
    .play_open = wireless_file_player_open,
    .play_close = wireless_file_player_close,
    .play_status = get_file_dec_status,
};
...
#if (TCFG_BROADCAST_ENABLE || TCFG_CONNECTED_ENABLE)
    if (music_player_is_audio_capture()) {
        ret = file_audio_capture_open(file, dbp, music_player_get_caputre_path());
    } else
#endif
    {
...
bool music_player_get_playing_breakpoint(struct __breakpoint *bp, u8 flag)
{
    ...
    if (dev_manager_online_check(__this->dev, 1)) {
        if (file_dec_is_play() == true || file_dec_is_pause() == true || file_dec_is_fade_in_out() == true) {
            if (__this->file) {
                if (flag) {
                    ///获取断点解码信息
...
int music_player_get_play_status(void)
{
    ...
    } else if (status == FILE_DEC_STATUS_WAIT_PLAY || status == FILE_DEC_STATUS_PLAY || status == FILE_DEC_STATUS_WAIT_FADE_OUT || status == FILE_DEC_STATUS_FADE_OUT_SUCCESS) {
...
//*----------------------------------------------------------------------------*/
/**@brief    music_player切歌淡出
   @param
   @return   返回1代表淡出完成
   @note
*/
/*----------------------------------------------------------------------------*/
int music_player_fade_out(void)
{
    if (__this) {
        file_dec_fade_out();
    }
...
#endif
    ///停止解码
    if (music_player_fade_out()) {
#if (TCFG_BROADCAST_ENABLE || TCFG_CONNECTED_ENABLE)
        if (music_player_audio_capture_close()) {
            file_audio_capture_close();
        } else
#endif
        {
            file_dec_close();
...
__reset_scan_disk:
        ...
        if (reset_flag) {
            reset_flag = false;
            __this->dev = dev_manager_find_active(1);
        } else {
    ...
    if (__this->file == NULL) {
        /* return MUSIC_PLAYER_ERR_FILE_NOFOUND; */
        reset_flag = true;
        goto __reset_scan_disk;       //当播放设备第一个文件出错时重新寻找设备扫盘，防止因为返回找不到文件错误后重复循环进入播放此函数
...
#if (TCFG_BROADCAST_ENABLE || TCFG_CONNECTED_ENABLE)
void music_wireless_audio_codec_interface_register(void)
{
    live_audio_mode_setup(LIVE_FILE_CAPTURE_MODE, 0, &wireless_file_audio_ops);
}

static void *__wireless_file_player_open(struct audio_path *path)
{
    int err = MUSIC_PLAYER_ERR_NULL;
    char *logo = NULL;
...
static void *wireless_file_player_open(struct audio_path *path)
{
    return __wireless_file_player_open(path);
}
...
static void wireless_file_capture_wakeup(void *capture)
{
    file_audio_capture_wakeup(NULL);
}

static void *wireless_file_capture_open(struct audio_path *path)
{
    __this->audio_capture = AUDIO_CAPTURE_START;
    memcpy(&__this->path, path, sizeof(struct audio_path));
    return __wireless_file_player_open(path);
}

static void wireless_file_capture_close(void *capture)
{
    __this->audio_capture = AUDIO_CAPTURE_STOP;
    extern void file_audio_wait_capture_close(void);
    file_audio_wait_capture_close();//防止音乐模式广播时拔出sd卡导致无法通过关闭解码的同时关闭定时发数导致异常，在capture关闭的时候先关闭定时发数
    wireless_file_player_close(capture);
    __this->audio_capture = AUDIO_CAPTURE_IDLE;
}
