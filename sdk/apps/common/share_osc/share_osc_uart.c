...
#define	SHARE_OSC_UART_TASK_NAME		"osc_uart"
...
void share_osc_uart_get_data(void)
{
    u16 crc16;
    ...
    if (data_length >= p_newtool->length + 6) {
        ...
        share_osc_recieve_msg_from_slave();

        /* printf("uart rx-------------\n"); */
        /* put_buf(p_newtool, p_newtool->length + 6); */

        u8 *command_data = uart_rxbuf + 6;
        ...
        /* put_buf(buf_temp + 1, p_newtool->length); */

        /*数据进行分发处理*/
        const struct share_osc_interface *p;
        list_for_each_share_osc_interface(p) {
            if (p->id == p_newtool->type) {
                p->share_osc_message_deal(buf_temp + 1, p_newtool->length);
                if (buf_temp) {
                    free(buf_temp);
                    buf_temp = NULL;
                }
...
static void uart_isr_hook(void *arg, u32 status)
{
    ...
    int argv[2];
    argv[0] = (int)share_osc_uart_get_data;      // Function
    argv[1] = 0;                            // 参数个数
    int ret = os_taskq_post_type(SHARE_OSC_UART_TASK_NAME, Q_CALLBACK, sizeof(argv) / sizeof(int), argv);
    if (ret != OS_NO_ERR) {
...
void all_assemble_package_send_for_share_osc(u8 *buf, u32 len)
{
    ...
    crc16_data = CRC16(&send_buf[5], len + 1);
    send_buf[3] = crc16_data & 0xff;
    send_buf[4] = (crc16_data >> 8) & 0xff;

    share_osc_send_buf_list_add(send_buf, len + 6);
}

static void share_osc_uart_task(void *p)
{
    printf("create %s task\n", __FUNCTION__);
    share_osc_check_single_io_send_init();
    int msg[8];
    while (1) {
...
static void check_uart_io_level()
{
    ...
    /* if (!flag_uart_write_busy && gpio_read_direction(TCFG_SHARE_OSC_UART_PORT)) { */
    /*     if (gpio_read(TCFG_SHARE_OSC_UART_PORT) == 0) { */
    /*         putchar('-'); */
...
void share_osc_uart_close(void)
{
    ...
    gpio_set_die(TCFG_SHARE_OSC_UART_PORT, 1);
...
void share_osc_uart_init(void)
{
    ...
    ut.tx_pin = TCFG_SHARE_OSC_UART_PORT;
    ut.rx_pin = TCFG_SHARE_OSC_UART_PORT;
    ...
    share_osc_uart_serial_send_init(uart_bus);
    share_osc_command_init();

    task_create(share_osc_uart_task, NULL, SHARE_OSC_UART_TASK_NAME);
}
