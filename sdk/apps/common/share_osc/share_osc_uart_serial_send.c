...
/* #define SHARE_OSC_DEBUG_EN */

#ifdef SHARE_OSC_DEBUG_EN
#define share_osc_putchar(x)                putchar(x)
#define share_osc_printf                    printf
#define share_osc_put_buf(x, len)           put_buf(x, len)
#else
#define share_osc_putchar(...)
#define share_osc_printf(...)
#define share_osc_printf_buf(...)
#endif

#define SHARE_OSC_UART_SERIAL_SEND_TASK_NAME "serial_uart"
...
ShareOscSendResult share_osc_send_buf_list_add(char *buf, u16 len)
{
    if (_send_list_count >= SERIAL_SEND_LIST_MAX_COUNT) {
        /* printf("%s send buf list full! Need check slave no response's reason!\n", __FUNCTION__); */
        // 需要检查从机为什么没有回复命令
        return ShareOscSendResultFailMaxCount;
    }
    ...
    if ((!send_buf_hd) || (!send_buf)) {
        printf("%s malloc err!\n", __FUNCTION__);
        return ShareOscSendResultMallocFail;
    }
    ...
    return ShareOscSendResultSuccess;
}
...
void share_osc_uart_serial_send_task(void *p)
{
    share_osc_printf("create %s task success!\n", SHARE_OSC_UART_SERIAL_SEND_TASK_NAME);
        ...
        while (!list_empty(&_serial_send_list)) {
            ...
            while (send_buf != NULL) {
                ...
                if (OS_NO_ERR != err) {
                    ...
                    printf("[Warning]_serial_send_tx_sem wait rx timeout! Need check slave no response's reason!\n");
                }
...
{
    ASSERT(udev, "share osc uart serial send init udev err!\n");
    _udev = udev;
    task_create(share_osc_uart_serial_send_task, NULL, SHARE_OSC_UART_SERIAL_SEND_TASK_NAME);
}
