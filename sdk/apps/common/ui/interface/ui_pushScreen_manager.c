...
#define LCD_PUSHSCREEN_MODE     PUSH_BY_IMD
...
{
    gpio_direction_output((u32)lcd_dat->pin_reset, 0);
}
static void lcd_reset_h()
{
    gpio_direction_output((u32)lcd_dat->pin_reset, 1);
}
static void lcd_cs_l()
{
    gpio_direction_output((u32)lcd_dat->pin_cs, 0);
}
static void lcd_cs_h()
{
    gpio_direction_output((u32)lcd_dat->pin_cs, 1);
}
static void lcd_rs_l()
{
    gpio_direction_output((u32)lcd_dat->pin_dc, 0);
}
static void lcd_rs_h()
{
    gpio_direction_output((u32)lcd_dat->pin_dc, 1);
}

static void lcd_bl_l()
{
    gpio_direction_output((u32)lcd_dat->pin_bl, 0);
}

static void lcd_bl_h()
{
    gpio_direction_output((u32)lcd_dat->pin_bl, 1);
}
...
static void lcd_init_code(const u8 *code, u16 cnt)
{
    /* printf("init code cnt: %d\n", cnt); */
    for (int i = 0; i < cnt; i++) {
        if (i >= cnt) {
            puts("lcd_code_list read err");
            break;
        }
