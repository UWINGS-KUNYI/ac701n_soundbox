...
int wireless_dev_status_sync(const char *name, const void *const buf, size_t len, void *priv)
{
    ...
    list_for_each_wireless_dev(wireless_dev) {
        if (!strcmp(name, wireless_dev->name)) {
            if (wireless_dev->ioctrl) {
                ret = wireless_dev->ioctrl(WIRELESS_DEV_OP_STATUS_SYNC, priv, buf, len);
            }
            WIRELESS_DEV_EXIT_CRITICAL();
            return ret;
        }
...
int wireless_dev_audio_sync_enable(const char *name, void *priv, u8 enable)
{
    int ret = 0;
    const wireless_dev_ops *wireless_dev;
    WIRELESS_DEV_ENTER_CRITICAL();
    list_for_each_wireless_dev(wireless_dev) {
        if (!strcmp(name, wireless_dev->name)) {
            if (wireless_dev->ioctrl) {
                ret = wireless_dev->ioctrl(WIRELESS_DEV_OP_SET_SYNC, (u16)priv, 0);
            }
            WIRELESS_DEV_EXIT_CRITICAL();
            return ret;
        }
...
int wireless_dev_enter_pair(const char *name, u8 hdl, void *priv, u32 pair_code)
{
    int ret = -1;
    const wireless_dev_ops *wireless_dev;
    WIRELESS_DEV_ENTER_CRITICAL();
    list_for_each_wireless_dev(wireless_dev) {
        if (!strcmp(name, wireless_dev->name)) {
            if (wireless_dev->ioctrl) {
                ret = wireless_dev->ioctrl(WIRELESS_DEV_OP_ENTER_PAIR, hdl, priv, pair_code);
            }
            WIRELESS_DEV_EXIT_CRITICAL();
            return ret;
        }
