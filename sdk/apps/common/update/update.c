...
int update_result_deal()
{
    ...
    while (1) {
        ...
        if (key_voice_cnt > voice_max_cnt) {
            ...
            extern void share_osc_vm_write_reset_flag(void);
...
#ifdef TCFG_SHARE_OSC_EN
    u8 tcfg_share_osc_en = TCFG_SHARE_OSC_EN;
    update_param_ext_fill(p, EXT_SHARE_OSC_EN, &tcfg_share_osc_en, 1);
#endif
...
extern void app_broadcast_close(u8 status);
extern void app_connected_close(u8 status);
...
static void update_common_state_cbk(update_mode_info_t *info, u32 state, void *priv)
{
    ...
    case UPDATE_CH_INIT:
#if TCFG_SHARE_OSC_EN
        printf("update init,send poweroff cmd to rx");
        extern void share_osc_power_off();
        share_osc_power_off();
#endif
