...
void rcsp_get_device_feature()
{
    ...
    JL_CMD_send(JL_OPCODE_GET_TARGET_FEATURE, send_buf, sizeof(send_buf), JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e1_get_offset_and_size()
{
    ...
    rcsp_printf("%s, %d\n", __FUNCTION__, __LINE__);
    JL_CMD_send(JL_OPCODE_GET_DEVICE_UPDATE_FILE_INFO_OFFSET, NULL, 0, JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e2_ask_can_update(u8 *markData, u16 markDataSize)
{
    ...
    rcsp_printf("%s, %d\n", __FUNCTION__, __LINE__);
    JL_CMD_send(JL_OPCODE_INQUIRE_DEVICE_IF_CAN_UPDATE, markData, markDataSize, JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e3_enter_update_mode()
{
    ...
    rcsp_printf("%s, %d\n", __FUNCTION__, __LINE__);
    JL_CMD_send(JL_OPCODE_ENTER_UPDATE_MODE, NULL, 0, JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e4_exit_update_mode()
{
    rcsp_printf("%s, %d\n", __FUNCTION__, __LINE__);
    __this->canUpdate = false;
    JL_CMD_send(JL_OPCODE_EXIT_UPDATE_MODE, NULL, 0, JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e5_response_update_data(u8 opCode_SN, u8 *data, u16 len)
{
    ...
    JL_CMD_response_send(JL_OPCODE_SEND_FW_UPDATE_BLOCK, JL_PRO_STATUS_SUCCESS, opCode_SN, data, len);
}
/**
 * @brief e5发送升级数据失败
 *
...
static void _rcsp_ota_e5_response_update_data_error(u8 opCode_SN)
{
    ...
    JL_CMD_response_send(JL_OPCODE_SEND_FW_UPDATE_BLOCK, JL_PRO_STATUS_FAIL, opCode_SN, NULL, 0);
}
...
static void _rcsp_ota_e6_query_device_upadte_status()
{
    ...
    rcsp_printf("%s, %d\n", __FUNCTION__, __LINE__);
    JL_CMD_send(JL_OPCODE_GET_DEVICE_REFRESH_FW_STATUS, NULL, 0, JL_NEED_RESPOND);
}
...
static void _rcsp_ota_e7_reboot_device()
{
    ...
    JL_CMD_send(JL_OPCODE_SET_DEVICE_REBOOT, send_buf, sizeof(send_buf), JL_NEED_RESPOND);
}
...
static void _rcsp_switch_device_commucation_mode_to_ble()
{
    ...
    JL_CMD_send(JL_OPCODE_SWITCH_DEVICE, send_buf, sizeof(send_buf), JL_NEED_RESPOND);
}
...
int rcsp_m_update_cmd_handle(void *priv, u8 opCode, u8 status, u8 opCode_SN, u8 *data, u16 len)
{
    ...
    case JL_OPCODE_GET_DEVICE_UPDATE_FILE_INFO_OFFSET:
        if (status == JL_PRO_STATUS_SUCCESS) {
            if (data == NULL || len == 0) {
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusCommandFail, 0.0f);
            ...
            if (markDataSize != 0) {
                ...
                if (markData == NULL) {
                    ...
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFail, 0.0f);
                    _rcsp_release_update_var();
                    break;
                }
            }
    ...
    case JL_OPCODE_INQUIRE_DEVICE_IF_CAN_UPDATE:
        if ((status == JL_PRO_STATUS_SUCCESS) && (len > 0)) {
            ...
            if (ret == 0x00) {
                ...
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusLowPower, 0.0f);
            } else if (ret == 0x03) {
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailSameVersion, 0.0f);
            } else if (ret == 0x04) {
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailTWSDisconnect, 0.0f);
            } else if (ret == 0x05) {
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailNotInBin, 0.0f);
            } else {
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusUnknown, 0.0f);
            }
        } else {
            rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusCommandFail, 0.0f);
        }
    ...
    case JL_OPCODE_SEND_FW_UPDATE_BLOCK:
        ...
        if (seek == 0 && dataLen == 0) {
            // 查看升级状态
            JL_CMD_response_send(JL_OPCODE_SEND_FW_UPDATE_BLOCK, JL_PRO_STATUS_SUCCESS, opCode_SN, NULL, 0);
            ...
            if (!__this->needDownloadLoader) {
                ...
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusUpgrading, __this->otaProgress);
            } else {
                // 下载loader进度
            ...
            if (seek < 0) {
                ...
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusSeekFail, 0.0f);
            }
    ...
    case JL_OPCODE_GET_DEVICE_REFRESH_FW_STATUS:
        if ((status == JL_PRO_STATUS_SUCCESS) && (len > 0)) {
            ...
            if (ret == 0x00) {
                ...
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusSuccess, 1.0f);
                ...
                __this->isRelink = true;
                rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusPrepared, 1.0f);
                ...
                if (ret == 0x01) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailVerification, 0.0f);
                } else if (ret == 0x02) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailCompletely, 0.0f);
                } else if (ret == 0x03) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailKey, 0.0f);
                } else if (ret == 0x04) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailErrorFile, 0.0f);
                } else if (ret == 0x05) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailUboot, 0.0f);
                } else if (ret == 0x06) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailLenght, 0.0f);
                } else if (ret == 0x07) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailFlash, 0.0f);
                } else if (ret == 0x08) {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFailCmdTimeout, 0.0f);
                } else {
                    rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusFail, 0.0f);
                }
            }
    ...
    case JL_OPCODE_SET_DEVICE_REBOOT:
        if (status == JL_PRO_STATUS_SUCCESS) {
            rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusReboot, 1.0f);
            __this->canUpdate = false;
        }
        break;
    case JL_OPCODE_NOTIFY_UPDATE_CONENT_SIZE:
        ...
        JL_CMD_response_send(JL_OPCODE_NOTIFY_UPDATE_CONENT_SIZE, JL_PRO_STATUS_SUCCESS, opCode_SN, NULL, 0);
        break;
...
void rcsp_ota_with_file_path(char *otaFilePath, void *otaFd)
{
    ...
    if (otaFilePath == NULL || otaFd == NULL) {
        rcsp_progress_status_with_status_and_progress(RCSP_OTAProgressStatusDataIsNull, 0.0f);
        return;
    }
