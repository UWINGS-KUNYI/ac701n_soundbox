...
static void _rcsp_get_device_feature_callback(void *priv, u8 opCode, u8 *data, u16 len)
{
    ...
    if ((data != NULL) && (len > 0)) {
        ...
        for (int i = 0; i < len;) {
            ...
            u8 length = data[i];
            u8 type = data[i + 1];
            if (type == FEATURE_ATTR_TYPE_DOUBLE_PARITION) {
                isDualBankUpdate = data[i + 2];
                needDownloadLoader = data[i + 3];
            } else if (type == FEATURE_ATTR_TYPE_UPDATE_STATUS) {
...
void rcsp_c_cmd_response(void *priv, u8 opCode, u8 status, u8 *data, u16 len)
{
    rcsp_printf("%s, %x\n", __FUNCTION__, opCode);
    switch (opCode) {
