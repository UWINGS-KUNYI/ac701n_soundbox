...
#if CONFIG_CPU_BR28 && ((RCSP_MODE == RCSP_MODE_SOUNDBOX) || (RCSP == RCSP_MODE_EARPHONE))
...
#if (MUTIl_CHARGING_BOX_EN)
    buf[6] = 0x21;	//   2:TWS耳机类型   |  protocol verson
#else
...
#if RCSP_MODE == RCSP_MODE_SOUNDBOX
#if (MUTIl_CHARGING_BOX_EN)
    buf[6] = 0x01;	//   0:音箱类型   |  protocol verson
#else
#if (SOUNDCARD_ENABLE)
...
#else // RCSP_MODE != RCSP_MODE_WATCH
