...
#endif
        {
            u8 _timeout = (u8)timeout;
            JL_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_JL_FIND_DEVICE_STOP, &_timeout, sizeof(timeout));
        }
    } else {
        // 关闭铃声
...
void rcsp_cmd_recieve(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    switch (OpCode) {
