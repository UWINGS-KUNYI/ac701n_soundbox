...
int cfg_tool_effect_tool_spp_rx_data(u8 *packet, u8 size, u8 *ext_data, u16 ext_size)
{
    u8 *rx_buf = NULL;
...
void cfg_tool_event_to_user(u8 *packet, u32 type, u8 event, u8 size)
{
    ...
    app_cfg_tool_event_handler(&(e.u.cfg_tool));
    return;
#endif
    sys_event_notify(&e);
}
...
#if (TCFG_COMM_TYPE == TCFG_SPP_COMM)
static int new_cfg_tool_init(void)
{
    app_online_db_register_handle(SPP_NEW_EQ_CH, cfg_tool_effect_tool_spp_rx_data);
    app_online_db_register_handle(SPP_OLD_EQ_CH, cfg_tool_effect_tool_spp_rx_data);
    app_online_db_register_handle(SPP_CFG_CH, cfg_tool_effect_tool_spp_rx_data);
    return 0;
}
early_initcall(new_cfg_tool_init);
#endif
