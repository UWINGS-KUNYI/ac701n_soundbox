...
colorled_platform_data colorled_data = {
	.pin_red = {
		.pin = TCFG_COLORLED_RED_PIN,
		//        .timer_hdl = red_timer_hdl,
		.output_ch = -1,
	},
	.pin_gree = {
		.pin = TCFG_COLORLED_GREEN_PIN,
		//        .timer_hdl = gree_timer_hdl,
		.output_ch = -1,
	},
	.pin_blue = {
		.pin = TCFG_COLORLED_BLUE_PIN,
		//        .timer_hdl = bule_timer_hdl,
		.output_ch = -1,
	},
	.work_mode = COLOR_LED_MODE_NORNAL,
};
#endif // TCFG_COLORLED_ENABLE
...
ALINK_PARM alink0_platform_data = {
...
#endif /*TCFG_AUDIO_DAC_BIT_WIDTH*/
#if ((AUDIO_OUT_WAY_TYPE == AUDIO_WAY_TYPE_DAC_IIS) && TCFG_MIC_EFFECT_ENABLE)
	//控制延时在大概30-50ms之间，防止dac+iis+音频同时输出出现卡顿
    .dma_len = 256 * 2 * 2 * 8,
#else
#if (defined TCFG_AUDIO_DAC_BIT_WIDTH) && (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)
	.dma_len = 1024 * 8,
	...
	.dma_len = 1024 * 4,
#endif /*TCFG_AUDIO_DAC_BIT_WIDTH*/
#endif /*AUDIO_WAY_TYPE_DAC_IIS*/
    .sample_rate = TCFG_IIS_SR,
    .buf_mode = ALINK_BUF_CIRCLE,
#ifdef TCFG_IIS_CAPTURE_SAMPLE_PERIOD
    .iperiod = (TCFG_IIS_SR / TCFG_IIS_CAPTURE_SAMPLE_PERIOD + 31) / 32 * 32,
#else
...
LED7_PLATFORM_DATA_BEGIN(led7_data)
    ...
    .pin_cfg.pin7.pin[6] = IO_PORTA_05,
...
#if TCFG_COLORLED_ENABLE
	colorled_data.pin_red.timer_hdl = zalloc(sizeof(JL_TIMER_TypeDef));
	colorled_data.pin_gree.timer_hdl = zalloc(sizeof(JL_TIMER_TypeDef));
	colorled_data.pin_blue.timer_hdl = zalloc(sizeof(JL_TIMER_TypeDef));
	color_led_init(&colorled_data);
	color_led_set_api(COLOR_LED_MODE_LIGHT, TWINKLE_SLOW, COLOR_RED);
#endif
...
static u16 type_detect_timer = 0;
extern u8 get_total_connect_dev(void);
static void support_type_detect(void *priv)
{
	static u8 led_flash = 0;
	led_flash = ! led_flash;

	gpio_set_die(TCFG_12V_DET_IO, 1);
    gpio_set_pull_down(TCFG_12V_DET_IO, 0);
    gpio_set_pull_up(TCFG_12V_DET_IO, 0);
    gpio_set_direction(TCFG_12V_DET_IO, 1);
	if (gpio_read(TCFG_12V_DET_IO)) {
		/* log_info("use 12V support"); */
		gpio_direction_output(TCFG_PA_EN_IO, 0);
	} else {
		/* log_info("use vbat 4.2V support"); */
		gpio_direction_output(TCFG_PA_EN_IO, 1);
	}

#if 0
	//充电指示灯
	gpio_set_die(TCFG_CHARGE_DET_IO, 1);
    gpio_set_pull_down(TCFG_CHARGE_DET_IO, 0);
    gpio_set_pull_up(TCFG_CHARGE_DET_IO, 1);
    gpio_set_direction(TCFG_CHARGE_DET_IO, 1);
	if (gpio_read(TCFG_12V_DET_IO)) {
		if (!gpio_read(TCFG_CHARGE_DET_IO)) {
			rgb_led_status_set(0, 1, 1);
		} else  {
			rgb_led_status_set(1, 0, 1);
		}
		return;
	}
	//开启语音识别且蓝牙已连接常亮紫灯，否则紫灯闪烁
#if TCFG_UNISOUND_ENABLE
	if (get_unisound_online()) {
		if (get_total_connect_dev()) {
			rgb_led_status_set(0, 1, 0);
		} else {
			rgb_led_status_set(led_flash, 1, led_flash);
		}
		return;
	}
#endif
	//蓝牙连接状态指示灯,蓝牙连接蓝灯常亮，否则蓝灯闪烁
	if (get_total_connect_dev()) {
		rgb_led_status_set(1, 1, 0);
	} else {
		rgb_led_status_set(1, 1, led_flash);
	}
#endif
}
...
void board_init()
{
	...
	gpio_direction_output(TCFG_RF_PA_EN, 1);

	support_type_detect(NULL);
	if (!type_detect_timer) {
		type_detect_timer = sys_timer_add(NULL, support_type_detect, 500);//定时扫描
	}
...
{
	if (type_detect_timer) {
		sys_timer_del(type_detect_timer);
		type_detect_timer = 0;
	}
    ...
    soff_gpio_protect(TCFG_12V_DET_IO);
