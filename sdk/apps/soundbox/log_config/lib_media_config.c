...
const int global_bit_wide  = TCFG_AUDIO_DAC_BIT_WIDTH;//0:16bit  1:24bit

const int JL_HW_FFT_V = FFT_EXT; //fft版本

const int CONFIG_DEC_SUPPORT_BITWIDTH = (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_16BIT_MODE) ? 16 : 24;  	 //  24bit 或者 16bit
const int CONFIG_DEC_SUPPORT_CHANNELS = 2;   	 //  支持的最大声道数
const int CONFIG_DEC_SUPPORT_SAMPLERATE = 48000; //  支持的最大采样率
...
const int AAC_DEC_LIB_SUPPORT_BITWIDTH = 16;
#else
const int AAC_DEC_LIB_SUPPORT_BITWIDTH = CONFIG_DEC_SUPPORT_BITWIDTH;
#endif
const int CONFIG_DEC_SUPPORT_DAB_AAC = 0; //AAC 文件的dab+ 类 解码支持，每帧读960进行解码;

#ifdef WTS_DEC_IN_MASK
const int WTS_DEC_LIB_SUPPORT_BITWIDTH = 16;
#else
const int WTS_DEC_LIB_SUPPORT_BITWIDTH = CONFIG_DEC_SUPPORT_BITWIDTH;
#endif
...
#if (SOUNDCARD_ENABLE || RECORDER_MIX_EN)
const int config_soundcard_en = 1;
#else
const int config_soundcard_en = 0;
#endif
...
const int  WAV_DECODER_PCM_POINTS = 512;
...
const int CONFIG_OGG_OPUS_FILE_DEC_SUPPORT = TCFG_DEC_OGG_OPUS_ENABLE; //opus解码支持ogg 封装类文件解码，使用lib_ogg_opus_dec.a的库，需要45K的ram
...
const int MP3_OUTPUT_LEN = 18;
const int WMA_OUTPUT_LEN = 18;
...
const int config_sbc_sw_dec_output_hw_mode = 0; // 1 ：软件sbc解码输出与硬件模块二进制相同
...
#if (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)
const int JLA_INT24bit_INOUT = 1;
#else
...
const int config_audio_dac_vcm_chsel_delay = 700;
...
const char log_tag_const_i_AUDIO_DECODER AT(.LOG_TAG_CONST) = CONFIG_DEBUG_LIB(1);
