...
static void linein_app_init(void)
{
    ...
    if (le_broadcast_en || le_connected_en) {
        aux_wireless_audio_codec_interface_register();
        if (!bt_back_mode_en) {
...
void app_linein_task()
{
    ...
    int ret = tws_tone_play_with_callback_by_name(tone_table[IDEX_TONE_LINEIN], 1, line_tone_play_end_callback, (void *)IDEX_TONE_LINEIN, 400);
    if (ret)
#endif
    {
