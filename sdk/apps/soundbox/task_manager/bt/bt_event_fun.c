...
static u8 wireless_trans_mode = WIRELESS_TRANS_DEFAULT_MODE;

#if (TCFG_BROADCAST_ENABLE || TCFG_CONNECTED_ENABLE)

static int a2dp_drop_frame_timer;
static int get_a2dp_dec_status(void);
extern void *live_a2dp_capture_open(struct audio_path *path);
extern void live_a2dp_capture_close(void *capture);
extern void live_a2dp_capture_wakeup(void *capture);

static void *wireless_a2dp_capture_start(struct audio_path *path);
static void wireless_a2dp_capture_stop(void *capture);
static void wireless_a2dp_capture_wakeup(void *capture);
static void *wireless_a2dp_player_start(struct audio_path *path);
static void wireless_a2dp_player_stop(void *player);

static struct live_audio_mode_ops wireless_a2dp_audio_ops = {
    .capture_open = wireless_a2dp_capture_start,
    .capture_close = wireless_a2dp_capture_stop,
    .capture_wakeup = wireless_a2dp_capture_wakeup,
    .play_open = wireless_a2dp_player_start,
    .play_close = wireless_a2dp_player_stop,
    .play_status = get_a2dp_dec_status,
};
#endif
...
{
    if (user_tws_en && (wireless_trans_mode == WIRELESS_TRANS_TWS_MODE)) {
        return;
    }
...
void  bt_status_init_ok(struct bt_event *bt)
{
    ...
    bt_wait_phone_connect_control(1);
#else
    if (user_tws_en && ((wireless_trans_mode == WIRELESS_TRANS_BT_MODE) || (wireless_trans_mode == WIRELESS_TRANS_BIG_MODE))) {
        bt_tws_init();
        bt_wait_connect_and_phone_connect_switch(0);
    } else if (user_tws_en && (wireless_trans_mode == WIRELESS_TRANS_TWS_MODE)) {
        bt_tws_poweron();
    } else if (virtual_fast_connect_for_emitter) {
        bt_virtual_fast_connect_poweron(BT_RECEIVER_EN);
    } else {
        bt_wait_connect_and_phone_connect_switch(0);
    }

    if (lea_broadcast_tx_en || lea_broadcast_rx_en) {
        app_broadcast_init();
        app_broadcast_deal(BROADCAST_APP_MODE_ENTER);
    }
    if (lea_connected_central_en || lea_connected_perip_en) {
        app_connected_init();
...
#else
    if ((user_tws_en && (wireless_trans_mode == WIRELESS_TRANS_TWS_MODE)) && get_bt_tws_connect_status()) {
        ignore_tone = 1;
    }
    if (!app_var.goto_poweroff_flag) { /*关机不播断开提示音*/
        if (!__this->ignore_discon_tone && !ignore_tone && !is_wireless_trans_tone_busy()) {
...
void bt_status_phone_income(struct bt_event *bt)
{
    ...
    if ((check_esco_state_via_addr(tmp_bd_addr) != BD_ESCO_BUSY_OTHER) && (bt_user_priv_var.phone_ring_flag == 0)) {
        ...
        if (user_tws_en && (wireless_trans_mode == WIRELESS_TRANS_TWS_MODE)) {
            if (sync_play_phone_ring && !bt_tws_sync_phone_num(NULL)) {
                phone_play_en = 1;
            } else if (!sync_play_phone_ring && tws_api_get_role() == TWS_ROLE_MASTER) {
                phone_play_en = 1;
            }
        } else {
            phone_play_en = 1;
...
void bt_status_voice_recognition(struct bt_event *bt)
{
    ...
    if (app_var.siri_stu == 2) {
        unisound_set_siri_online(1);
    } else if (app_var.siri_stu == 0) {
        unisound_set_siri_online(0);
    }
#endif
...
void bt_hci_event_disconnect(struct bt_event *bt)
{
    ...
    if (le_broadcast_en && broadcast_receiver_close_edr) {
        if (wireless_trans_mode == WIRELESS_TRANS_BIG_MODE) {
    ...
    if (le_connected_en && le_connected_close_edr) {
        if (wireless_trans_mode == WIRELESS_TRANS_CIG_MODE) {
            if (app_get_connected_role() & APP_CONNECTED_ROLE_RECEIVER) {
                return;
            }
        }
    }
...
u8 get_cur_wireless_trans_mode(void)
{
    return wireless_trans_mode;
}

void set_cur_wireless_trans_mode(u8 mode)
{
    wireless_trans_mode = mode;
}

#if (TCFG_BROADCAST_ENABLE || TCFG_CONNECTED_ENABLE)
void bt_wireless_audio_codec_interface_register(void)
{
    live_audio_mode_setup(LIVE_A2DP_CAPTURE_MODE, 44100, &wireless_a2dp_audio_ops);
}
...
{
    if (bt_media_is_running()) {
        live_a2dp_capture_wakeup(capture);
    }
}

static void *wireless_a2dp_capture_start(struct audio_path *path)
{
    u8 need_capture_open = 0;
    if (le_broadcast_en || le_connected_en) {
        if (a2dp_drop_frame_timer) {
            sys_timeout_del(a2dp_drop_frame_timer);
            a2dp_drop_frame_timer = 0;
        }
        if (le_broadcast_en && (get_broadcast_role() == BROADCAST_ROLE_TRANSMITTER)) {
            need_capture_open = 1;
        }
        if (le_connected_en && ((app_get_connected_role() == APP_CONNECTED_ROLE_TRANSMITTER) ||
                                (app_get_connected_role() == APP_CONNECTED_ROLE_DUPLEX))) {
            need_capture_open = 1;
        }
        if (need_capture_open) {
            path->fmt.priv = (void *)__this->a2dp_decoder_type;
            return live_a2dp_capture_open(path);
        }
    }
    return NULL;
}
...
{
    if (bt_media_is_running()) {
        live_a2dp_capture_close(capture);
    }
}
...
static int get_a2dp_dec_status(void)
{
    if (le_broadcast_en && get_broadcast_app_mode_exit_flag()) {
        return LOCAL_AUDIO_PLAYER_STATUS_ERR;
    }
    if (le_connected_en && get_connected_app_mode_exit_flag()) {
        return LOCAL_AUDIO_PLAYER_STATUS_ERR;
    }
    if (get_call_status() != BT_CALL_HANGUP) {
        return LOCAL_AUDIO_PLAYER_STATUS_ERR;
