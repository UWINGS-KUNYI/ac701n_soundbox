...
void bt_background_init()
{
    ...
    if (state & TWS_STA_SIBLING_CONNECTED) {
        if (__this->cmd_flag != 2) {
            ...
            /* tws_api_sync_call_by_uuid('D', SYNC_CMD_BOX_ENTER_BT, 100); */
            tws_user_sync_box(SYNC_CMD_BOX_ENTER_BT, 0);
        }
    }
...
void bt_task_init()
{
    ...
    bt_wireless_audio_codec_interface_register();
#endif
}
...
void bt_task_close()
{
    ...
    if (!app_var.goto_poweroff_flag) {
        ...
        if (state & TWS_STA_SIBLING_CONNECTED) {
            ...
            tws_api_sync_call_by_uuid('D', SYNC_CMD_BOX_INIT_EXIT_BT, 400);
            /* tws_user_sync_box(SYNC_CMD_BOX_INIT_EXIT_BT, 0); */
            return;
        }
    }
#endif
