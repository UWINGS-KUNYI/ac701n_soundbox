...
void bt_function_select_init()
{
    ...
    ////SBC默认53，AAC默认320K
    ////开启广播模式后，SBC改为38，AAC改为131K
    __set_sbc_cap_bitpool(38);
    __set_aac_bitrate(131 * 1000);
#endif

#if TCFG_BT_SUPPORT_LHDC
    __set_support_lhdc_flag(1);
#else
    __set_support_lhdc_flag(0);
#endif
...
#else
    __change_hci_class_type(BD_CLASS_LOUDSPEAKER);
#endif
...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
...
#if TCFG_NORMAL_SET_DUT_MODE && !TCFG_USER_BLE_ENABLE
        log_info("edr set dut mode\n"); //BLE DUT 和 EDR DUT不能一起开
        bredr_set_dut_enble(1, 1);
#else
        log_info("edr set normal mode\n");
#endif
    ...
    case BT_STATUS_PHONE_INCOME:
        ...
        result = app_connected_deal(CONNECTED_PHONE_START);
        if (result > 0) {
            break;
        }
#endif
...
#endif
#if TCFG_CONNECTED_ENABLE
        result = app_connected_deal(CONNECTED_PHONE_STOP);
        if (result > 0) {
            break;
        }
#endif
...
#endif
#if TCFG_CONNECTED_ENABLE
        result = app_connected_deal(CONNECTED_A2DP_START);
        if (result > 0) {
            break;
        }
#endif
...
#endif
#if TCFG_CONNECTED_ENABLE
        result = app_connected_deal(CONNECTED_A2DP_STOP);
        if (result > 0) {
            break;
        }
#endif
...
static int bt_hci_event_handler(struct bt_event *bt)
{
...
#endif
#if TCFG_CONNECTED_ENABLE
        app_connected_deal(CONNECTED_EDR_DISCONN);
#endif
        bt_hci_event_disconnect(bt) ;
        clock_remove_set(BT_CONN_CLK);
        break;
    ...
    case HCI_EVENT_CONNECTION_COMPLETE:
        ...
        case CUSTOM_BB_AUTO_CANCEL_PAGE:
            g_printf("%s %d, bt->value::0x%x", __func__, __LINE__, bt->value);
            bt_hci_event_disconnect(bt) ;
            break;
...
void app_bt_task()
{
    ...
    if (!__this->cmd_flag) { //蓝牙后台拉回蓝牙模式不播放提示音
        ...
        int ret = tws_tone_play_with_callback_by_name(tone_table[IDEX_TONE_BT_MODE], 1, bt_tone_play_end_callback, (void *)IDEX_TONE_BT_MODE, 400);
        if (ret)
#endif
        {
            //协议栈初始化在提示音结束进行
