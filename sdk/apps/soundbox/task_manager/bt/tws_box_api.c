...
static void  tws_tone_play_end_callback(void *priv, int flag)
{
    u32 index = (u32)priv;

    switch (index) {
    case IDEX_TONE_TWS_DISCONN:
        app_bt_hdl.ignore_discon_tone = 0;
        break;
    default:
        break;
    }
...
static void tws_box_mode_change_deal(u8 mode, u8 value)
{
#if (TCFG_DEC2TWS_ENABLE)
    log_info("%s %d, mode:%d, value:%d", __FUNCTION__, __LINE__, mode, value);
    if (mode == TWS_BOX_EXIT_BT) {
...
static void bt_tws_box_sync(void *_data, u16 len, bool rx)
{
    ...
    if (rx) {
        ...
        if (data[0] == TWS_BOX_NOTICE_A2DP_BACK_TO_BT_MODE
            ...
            tws_box_mode_change_deal(data[0], data[1]);
            return;
        }
    }
...
void tws_user_sync_box(u8 cmd, u8 value)
{
    ...
    data[0] = cmd;
    data[1] = value;
    u32 bt_tmr = 0;//audio_bt_time_read(NULL, NULL) + 1000;
...
static void tws_sync_call_tranid_fun(int cmd, int err)
{
    struct sys_event event;
...
static void tws_event_sync_fun_tranid(struct bt_event *evt)
{
    ...
    if (reason == SYNC_CMD_BOX_EXIT_BT) {
        ...
        TWS_REMOTE_OUT_BT();
        gtws.device_role =  TWS_UNACTIVE_DEIVCE;
        tws_box_mode_change_deal(TWS_BOX_EXIT_BT, gtws.device_role);
...
#endif
        tws_box_mode_change_deal(TWS_BOX_ENTER_BT, gtws.device_role);
    }
}
...
void tws_event_connected(struct bt_event *evt)
{
    ...
    g_printf("tws common addr:\n");
    put_buf(addr[3], 6);
    g_printf("tws local addr:\n");
    put_buf(addr[3], 6);
    g_printf("tws sibling addr:\n");
    put_buf(addr[2], 6);
    ...
    if (pair_suss) {
        ...
        lmp_hci_write_local_address(addr[3]);
    }
    ...
    if (role == TWS_ROLE_MASTER) {
        ...
        if (bt_get_exit_flag() || (!get_bt_tws_discon_dly_state() && (get_call_status() == BT_CALL_HANGUP) && !(state & TWS_STA_SBC_OPEN))) {  //通话状态不播提示音
...
void tws_event_search_timeout(struct bt_event *evt)
{
    ...
    u8 temp_addr[6] = {0};
    if (!memcmp(gtws.addr, temp_addr, 6)) {
...
void tws_event_connection_detach(struct bt_event *evt)
{
    ...
    if ((gtws.state & BT_TWS_SIBLING_CONNECTED) && (!app_var.goto_poweroff_flag)) {
        ...
        if (!is_wireless_trans_tone_busy() && !phone_link_connection && (reason != TWS_DETACH_BY_REMOVE_PAIRS)) { /*关机不播断开提示音*/
            if (gtws.tws_dly_discon_time) {
                gtws.state |= BT_TWS_DISCON_DLY_TIMEOUT;
...
#else
        if (!is_wireless_trans_tone_busy() && !phone_link_connection && (reason != TWS_DETACH_BY_REMOVE_PAIRS)) {
            if (gtws.tws_dly_discon_time) {
                gtws.state |= BT_TWS_DISCON_DLY_TIMEOUT;
            } else {
                app_bt_hdl.ignore_discon_tone = 1;
    ...
    if (get_cur_wireless_trans_mode() == WIRELESS_TRANS_TWS_MODE) {
#if CONFIG_TWS_DISCONN_NO_RECONN
...
static void tws_event_role_switch_start(struct bt_event *evt)
{
#if (ESCO_AUTO_POWER_BALANCE_ROLE_SWITCH_ENABLE==1)&&(TCFG_USER_TWS_ENABLE==1)
    if (get_esco_coder_busy_flag()) {
        if (role == TWS_ROLE_SLAVE) {
...
#if CONFIG_TWS_DISCONN_NO_RECONN
        tws_api_detach(TWS_DETACH_BY_REMOVE_NO_RECONN);
#else
        tws_disconnect();
#endif
    }
