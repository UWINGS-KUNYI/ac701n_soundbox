...
int music_device_tone_play(char *logo)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(device_tone); i++) {
        if (strcmp(device_tone[i].logo, logo) == 0) {
            ...
            int ret = tws_tone_play_with_callback_by_name(device_tone[i].tone_path, 1, music_tone_play_end_callback, (void *)device_tone[i].index, 400);
            if (ret)
#endif
            {
...
#endif
    app_task_put_key_msg(KEY_MUSIC_PLAY_SUCCESS, 0);
}
//*----------------------------------------------------------------------------*/
...
static int music_player_scandisk_break(void)
{
    ...
    case APP_MSG_SYS_EVENT:
        ...
        case SYS_DEVICE_EVENT:
            ...
            break;
        case SYS_BT_EVENT:
            if ((u32)event->arg == SYS_BT_EVENT_FROM_CIG) {
...
static int music_key_event_opr(struct sys_event *event)
{
    ...
    switch (msg[0]) {
    case KEY_MUSIC_PLAY_SUCCESS:
        log_i("KEY_MUSIC_PLAY_SUCCESS !!\n");
        if (le_broadcast_en) {
    ...
    case KEY_MUSIC_PLAYER_START:
        ...
        if (!get_broadcast_role() || mode_in)
#elif TCFG_CONNECTED_ENABLE
        if (!get_connected_role() || mode_in)
#endif
#endif
        {
            //防止在上面初始化蓝牙协议栈成功后会自动开广播，从而开启广播音频，不需要此处开启音频打断广播音频.
            if (true == breakpoint_vm_read(breakpoint, logo)) {
                err = music_player_play_by_breakpoint(logo, breakpoint);
            } else {
                err = music_player_play_first_file(logo);
...
static void music_task_start()
{
    ...
    if (le_broadcast_en || le_connected_en) {
        music_wireless_audio_codec_interface_register();
        if (!bt_back_mode_en) {
...
#else
#if TCFG_TONE2TWS_ENABLE
    int ret = tws_tone_play_with_callback_by_name(tone_table[IDEX_TONE_MUSIC], 1, music_tone_play_end_callback, (void *)IDEX_TONE_MUSIC, 400);
    if (ret)
#endif
    {
