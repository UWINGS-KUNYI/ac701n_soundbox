...
static void record_mic_start(void)
{
    ...
    fmt.gain = 16;
#if RECORDER_MIX_EN
    fmt.sample_rate = RECORDER_MIX_SAMPLERATE;            //采样率：8000，16000，32000，44100
#else
...
{
    static int record_mix_flag = 0;
#if RECORDER_MIX_EN
    if (recorder_is_encoding() || recorder_mix_get_status())
#else
...
#endif
    {
...
#if (TCFG_MIC_EFFECT_ENABLE)
        if ((mic_effect_get_status()) && record_mix_flag) {
            /* g_printf("mic_effect_stop by record\n"); */
            /* mic_effect_stop(); */
        }
#endif
#if RECORDER_MIX_EN
        if (recorder_mix_get_status()) {
            g_printf("recorder_mix_encode_stop\n");
            recorder_mix_stop();
            record_mix_flag = 0;
        } else {
            record_mic_stop();
        ...
        if (mic_effect_get_status()) {
            /* g_printf("record mic effect audio\n"); */
            record_mix_flag = 1;
        }
#endif
#if RECORDER_MIX_EN
        if ((!recorder_mix_get_status()) && record_mix_flag) {
            g_printf("recorder_mix_encode_start\n");
            recorder_mix_start();
            record_mix_flag = 0;
        } else {
            record_mic_start();
...
#else
        if (record_mix_flag) {
            y_printf("recorder_mix_en is not open, cant record mic_effect audio\n");
            mic_effect_stop();
        }
        record_mic_start();
#endif
...
void app_record_task()
{
    ...
    int ret = tws_tone_play_with_callback_by_name(tone_table[IDEX_TONE_RECORD], 1, record_tone_play_end_callback, (void *)IDEX_TONE_RECORD, 400);
    if (ret)
#endif
    {
