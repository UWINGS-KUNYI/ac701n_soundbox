...
#if  (TCFG_LINEIN_ENABLE && TCFG_LINEIN_ENERGY_DETECT)
...
#if  (TCFG_LINEIN_ENABLE && TCFG_LINEIN_ENERGY_DETECT)
static void linein_energy_detect_open(void *priv)
{
...
static int power_on_init(void)
{
    ...
    app_task_switch_to(APP_LIVE_MIC_TASK);
#elif WIRELESS_1tN_EN
    if (broadcast_fixed_role == BROADCAST_ROLE_TRANSMITTER) {
        app_task_switch_to(APP_LINEIN_TASK);
    } else {
        app_task_switch_to(APP_BT_TASK);
    }
#elif WIRELESS_1t4_EN
    app_task_switch_to(APP_LINEIN_TASK);
#elif TCFG_USB_APPLE_DOCK_EN //苹果iap协议使用pc模式
...
#if  (TCFG_LINEIN_ENABLE && TCFG_LINEIN_ENERGY_DETECT)
    /* linein_dec_open(TCFG_LINEIN_LR_CH, 48000); */
    int ret = sys_timeout_add(NULL, linein_energy_detect_open, 3000);
#endif
...
static void  tone_play_end_callback(void *priv, int flag)
{
    ...
    if (app_next_task) {
        log_error("tone callback task out : %d\n", app_get_curr_task());
        return;
    }
