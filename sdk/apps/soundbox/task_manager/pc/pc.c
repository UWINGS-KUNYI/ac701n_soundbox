...
void app_pc_task()
{
    ...
    int ret = tws_tone_play_with_callback_by_name(tone_table[IDEX_TONE_PC], 1, pc_tone_play_end_callback, (void *)IDEX_TONE_PC, 400);
    if (ret)
#endif
    {
    ...
    //当其他模式正在切换到PC模式过程中usb掉线，在切换完成后加检测处理，放在usb掉线仍然处于PC模式
    if (pc_app_check() == false) {
        app_task_switch_next();
    }
