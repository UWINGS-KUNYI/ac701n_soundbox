...
/*参考数据变采样处理*/
#if (TCFG_USB_MIC_CVP_ENABLE || (TCFG_SMART_VOICE_ENABLE && TCFG_SMART_VOICE_USE_AEC))
...
#if CVP_UL_DRC_EN
wdrc_struct_TOOL_SET cvp_ul_drc_parm[1] = {0};
int cvp_ul_drc_filter(void)
{
    int tar = 0;
    struct threshold_group threshold[3] = {{0, 0}, {75.f, 75.f}, {140.f, 75.f}};    //这里减去90才是具体的db数

    cvp_ul_drc_parm[tar].is_bypass = 0;

    cvp_ul_drc_parm[tar].parm.attacktime = 10;
    cvp_ul_drc_parm[tar].parm.releasetime = 300;
    //wdrc输入数据放大或者缩小的db数（正数放大，负数缩小）
    cvp_ul_drc_parm[tar].parm.inputgain = 0.f;
    //wdrc输出数据放大或者缩小的db数（正数放大，负数缩小）
    cvp_ul_drc_parm[tar].parm.outputgain = 12.f;
    cvp_ul_drc_parm[tar].parm.threshold_num = ARRAY_SIZE(threshold);
    memcpy(cvp_ul_drc_parm[tar].parm.threshold, threshold, sizeof(threshold));
    cvp_ul_drc_parm[tar].parm.rms_time = 25;
    cvp_ul_drc_parm[tar].parm.algorithm = 0;
    cvp_ul_drc_parm[tar].parm.mode = 1;
...
int audio_aec_open(u16 sample_rate, s16 enablebit, int (*out_hdl)(s16 *data, u16 len))
{
    ...
    printf("bt_media_is_running() : %d", bt_media_is_running());
    if (bt_media_is_running()) {
        aec_param->ref_sr = TCFG_A2DP_MUSIC_SAMPLE_FRAM;
    ...
    ul_drc_param.cb = drc_get_filter_info;
    ul_drc_param.drc_name = AEID_ESCO_UL_DRC;
    cvp_ul_drc_filter();
    ul_drc_param.wdrc = &cvp_ul_drc_parm[0];
