...
#define AUDIO_FRAME_FILE_STREAM     0
#define AUDIO_FRAME_LIVE_STREAM     1
#define AUDIO_FRAME_DMA_STREAM      3
/*
 * 音频帧封装
 */
struct audio_frame {
    u8 nch;                 // 通道数
    u8 mode;                // 帧采样模式：DMA、网络流、文件流等等
    u16 delay;              // 延时信息
    u32 timestamp;          // 时间戳
    u32 coding_type;        // 格式
    int sample_rate;        // 采样率
...
#define AUDIO_STREAM_WRITING_FRAME      0x1
#define AUDIO_STREAM_READING_FRAME      0x2
#define AUDIO_STREAM_OVERRUN            0x4
#define AUDIO_STREAM_UNDERRUN           0x8
struct audio_stream_node {
    u32 coding_type;
    struct list_head entry;             // 节点链表入口
    struct list_head *head;             // 记录链表头
    u8 state;                           // 状态
    void *stream;                       // 音频流句柄
    void (*wakeup)(void *stream);       // 唤醒函数
    int (*write_frame)(void *stream, struct audio_frame *frame);    //帧写入
    int (*read_frame)(void *stream, struct audio_frame *frame, int len);    //帧读取
    void *lock;                         // 私有锁
    u8 stop;
};
