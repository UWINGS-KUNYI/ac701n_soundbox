...
enum {
    ...
    EFF_MUSIC_RL_MID_DRC     = 0x2026,
    EFF_MUSIC_RL_HIGH_DRC    = 0x2027,
    EFF_MUSIC_RL_WHOLE_DRC   = 0x2028,
    ...
    EFF_MIC_HARMONIC_EXCITER = 0x3010,
    EFF_MUSIC_HARMONIC_EXCITER = 0x3011,

    EFF_MUSIC_DYNAMIC_EQ_PRO  = 0x3012,
...
//AUX 播放音效处理
    ...
    AEID_MUSIC_1P1_GAIN, //1.1声道混合左右声道

    AEID_MIC_HARMONIC_EXCITER,
    AEID_MUSIC_HARMONIC_EXCITER,

    AEID_EQUAL_LOUDNEDD_EQ,
...
struct _AUTOTUNE_TOOL_SET {
    int is_bypass; // 1-> byass 0 -> no bypass
    struct autotune_udapte_parm parm;
};

//动态EQ DynamicEQ:
typedef struct _DynamicEQParam_TOOL_SET {
    int is_bypass;          //BIT(0): 动态eq的bypass总开关 1-> byass 0 -> no bypass,
    //BIT(1):第1段的bypass开关 1-> byass 0 -> no bypass
    //BIT(2):第2段的bypass开关 1-> byass 0 -> no bypass
    //BIT(3):第3段的bypass开关 1-> byass 0 -> no bypass
    //BIT(4):第4段的bypass开关 1-> byass 0 -> no bypass
    //BIT(5):rmstime的存储类型是int还是浮点  0：int  1:float
...
#define EXT_HP_LP_SECTIONS 2  //高阶高通、高阶低通滤波器段数
#define OTHER_SECTION_MAX (10+EXT_HP_LP_SECTIONS)
...
struct eq_tool_mic_dut {
    float global_gain;
    int seg_num;          //eq效果文件存储的段数
    int enable_section;   //
    struct eq_seg_info seg[10 + EXT_HP_LP_SECTIONS]; //eq系数存储地址
};

#if defined(CONFIG_CPU_BR25)
#define mSECTION_MAX (10 + EXT_HP_LP_SECTIONS)
#define MULTI_BAND_DRC 0
#else
#define mSECTION_MAX (32 + EXT_HP_LP_SECTIONS)
...
#define mSECTION_MAX2 (10 + EXT_HP_LP_SECTIONS)
...
#define phone_eq_nsection  (5+EXT_HP_LP_SECTIONS)
#define music_eq_nsection  (10+EXT_HP_LP_SECTIONS)
#define music_eq2_nsection  (10+EXT_HP_LP_SECTIONS)
#define mic_eq_nsection   OTHER_SECTION_MAX
#define mic_eq_5section   5
