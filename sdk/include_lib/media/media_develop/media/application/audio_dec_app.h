...
enum audio_dec_app_status {
    ...
    AUDIO_DEC_APP_STATUS_WAIT_FADE_OUT,
    AUDIO_DEC_APP_STATUS_FADE_OUT_SUCCESS,
};
...
struct audio_dec_app_hdl {
    ...
    struct audio_dec_app_src_hdl src_hdl;
    void *uac_mix_ch;
    void *rec_mix_ch;
    struct audio_stream_entry *(*up_to_uac_open_callback)(struct audio_dec_app_hdl *);
    void (*up_to_uac_close_callback)(struct audio_dec_app_hdl *);
    OS_SEM fade_out_sem;
    u8 wait_fade_out;
    s16 fade_step;
    s16 fade_value;
...
void audio_dec_app_set_up_to_uac_info(struct audio_dec_app_hdl *dec, void (*open_cb)(void *), void (*close_cb)(void *));
/*
*********************************************************************
...
/*********************************************************************
*                  Audio Decoder App Sync Probe Deal
* Description: 对解码数据做淡出
* Arguments  : *dec	解码句柄
* Return	 : 1		成功
* Note(s)    : None.
*********************************************************************
*/
int audio_dec_app_decoder_fade_out(struct audio_dec_app_hdl *dec);
