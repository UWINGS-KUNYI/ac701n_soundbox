...
typedef enum {
    ...
    EQ_IIR_TYPE_HIGH_PASS_ADVANCE,//在gain的位置存储阶数 0~6阶， 0：disbale
    EQ_IIR_TYPE_LOW_PASS_ADVANCE, //在gain的位置存储阶数 0~6阶， 0：disbale
...
struct eq_seg_info {
    ...
    int freq;             //中心截止频率
    float gain;             //增益（-12 ~12 db）,高阶低通高通滤波器的阶存储位置
    float q;                //q值（0.3~30）
};
