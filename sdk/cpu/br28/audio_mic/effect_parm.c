...
const u16 mic_drc_name[] = {AEID_MIC_DRC0, AEID_MIC_DRC1, AEID_MIC_DRC2, AEID_MIC_DRC3};
struct nband_drc mic_last_drc_parm[EFFECT_REVERB_PARM_MAX];
#else
...
void mic_eq_update_parm(u8 mode)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(mic_eq_name); i++) { //5个类型的eq
        ...
        for (int j = 0; j < seg_num; j++) { //每个类型的eq有5段系数
            ...
            cur_eq_set_update(mic_eq_name[i], tar_seg, seg_num, 0);
        }
    }
}
...
void mic_drc_update_parm(u8 mode)
{
    ...
    for (type = 0; type < 4; type++) {
        tar_wdrc = &mic_last_drc_parm[mode].wdrc_parm[type];
        cur_drc_set_update(AEID_MIC_DRC4, type, tar_wdrc);
        bypass = mic_last_drc_parm[mode].wdrc_parm[type].is_bypass;
        cur_drc_set_bypass(AEID_MIC_DRC4, type, bypass);
    }
#endif
...
void set_mic_reverb_mode_by_id(u8 mode)
{
    ...
    audio_harmonic_exciter_update_parm_api(AEID_MIC_HARMONIC_EXCITER, &mic_eff->exciter_parm.parm, mic_eff->exciter_parm.is_bypass);
#endif
