...
#if (ESCO_ADC_CH > 1)
    adc_type_t tmp_buf[ESCO_ADC_IRQ_POINTS];
#endif/*ESCO_ADC_CH*/
    u8 state;
...
void adc_mic_output_handler(void *priv, adc_type_t *data, int len)
{
    ...
    if (esco_enc) {
        ...
        adc_type_t *mic0_data = (adc_type_t *)data;
        adc_type_t *mic1_data = (adc_type_t *)esco_enc->tmp_buf;
        adc_type_t *mic1_data_pos = (adc_type_t *)data + (len / sizeof(adc_type_t));
        //printf("mic_data:%x,%x,%d\n",data,mic1_data_pos,len);
        for (u16 i = 0; i < (len / sizeof(adc_type_t)); i++) {
...
#endif/*TCFG_ESCO_IIS_MIC_ENABLE*/
...
static void adc_mic_output_handler_downsr(void *priv, s16 *data, int len)
{
    //printf("buf:%x,data:%x,len:%d",esco_enc->adc_buf,data,len);
    u16 i;
    s16 temp_buf[320];/*开混响不跑算法才可用使用局部变量*/
    if (esco_enc && esco_enc->mic_sw_src_buf) {
        /*开混响并且ADC使用24bit时，上行需要把24bit转16bit*/
#if TCFG_MIC_EFFECT_ENABLE && (defined TCFG_AUDIO_ADC_BIT_MODE) && (TCFG_AUDIO_ADC_BIT_MODE == ADC_BIT_WIDTH_24)
#if (ESCO_ADC_CH == 1)
        audio_convert_data_32bit_to_16bit_round(data, temp_buf, (len >> 2) * ESCO_ADC_CH);
        data = temp_buf;
        len = len >> 1;
#endif /*ESCO_ADC_CH*/
#endif/*TCFG_MIC_EFFECT_ENABLE*/
...
int esco_enc_open(u32 coding_type, u8 frame_len)
{
...
#else            /*master mode*/
    esco_iis_mic->in_sample_rate = (TCFG_IIS_SR / 100) * 625;
    esco_iis_mic->out_sample_rate = (fmt.sample_rate / 100) * 624;
#endif/*TCFG_IIS_MODE*/
    ...
    alink_channel_init(hw_alink, 1, ALINK_DIR_RX, NULL, audio_iis_mic_output_handler);
    ...
    audio_adc_cvp_ref_open(TCFG_CVP_REF_IN_ADC_CH, fmt.sample_rate, app_var.aec_mic_gain, audio_adc_cvp_ref_output_handler);
...
#if TCFG_USE_OTHER_MIC_BIAS
    if ((TCFG_AUDIO_MIC_PWR_CTL & MIC_PWR_FROM_MIC_BIAS)) { //同时开启mic_bias供电以及开启差分mic模式，为防止mic_bias脚与差分N端这同一引脚的功能冲突，使用其他mic的mic_bias脚进行供电
        if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_0) {
            if (TCFG_AUDIO_MIC_MODE == AUDIO_MIC_CAP_DIFF_MODE) {
                audio_adc_mic_ldo_en(MIC_LDO_BIAS1, 1, &adc_data);
            }
        }

        if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_1) {
            if (TCFG_AUDIO_MIC1_MODE == AUDIO_MIC_CAP_DIFF_MODE) {
                audio_adc_mic_ldo_en(MIC_LDO_BIAS3, 1, &adc_data);
            }
        }

        if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_2) {
            if (TCFG_AUDIO_MIC2_MODE == AUDIO_MIC_CAP_DIFF_MODE) {
                audio_adc_mic_ldo_en(MIC_LDO_BIAS0, 1, &adc_data);
            }
        }

        if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_3) {
            if (TCFG_AUDIO_MIC3_MODE == AUDIO_MIC_CAP_DIFF_MODE) {
                audio_adc_mic_ldo_en(MIC_LDO_BIAS2, 1, &adc_data);
            }
        }
...
#if TCFG_MIC_EFFECT_ENABLE
#if (RECORDER_MIX_EN)
#define LADC_IRQ_POINTS     REVERB_LADC_IRQ_POINTS//160
#else
...
#define LADC_IRQ_POINTS     ((JLA_CODING_SAMPLERATE/1000)*5)
#else
...
static void audio_adc_mode_output_handler(void *priv, s16 *data, int len)
{
    if (adc_mode_hdl != NULL) {
        u8 point_size;
        if (TCFG_AUDIO_ADC_BIT_MODE == ADC_BIT_WIDTH_24) {
            point_size = 4;
            if (TCFG_ADC_BIT_24_TO_16 == 1) {
                point_size = 2;
            }
        } else {
            point_size = 2;
