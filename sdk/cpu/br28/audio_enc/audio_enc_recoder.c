...
#define LINEIN_GAIN             4 //linein默认增益

struct ladc_mic_demo {
    struct audio_adc_output_hdl adc_output;
    struct adc_mic_ch mic_ch;
    adc_type_t adc_buf[LADC_MIC_BUFS_SIZE];    //align 4Bytes
...
#if (SOUNDCARD_ENABLE || RECORDER_MIX_EN || TCFG_MIC_EFFECT_ENABLE)
...
#define ADC_LINEIN_STORE_PCM_SIZE     (ADC_LINEIN_BUFS_SIZE * 6*2)
#else
...
struct mic_sample_hdl {
    ...
    adc_type_t adc_buf[ADC_BUFS_SIZE];
...
struct audio_adc_var {
    ...
    adc_type_t adc_buf[LADC_LINEIN_BUFS_SIZE];    //align 4Bytes
};
...
void audio_adc_linein_demo(void)
{
    ...
    if (ladc_linein) {
        ...
        audio_adc_linein_set_buffs(&ladc_linein->ch, ladc_linein->adc_buf, LADC_LINEIN_IRQ_POINTS * 2, LADC_LINEIN_BUF_NUM);
...
typedef struct {
    ...
    struct adc_mic_ch mic_ch;
    adc_type_t adc_buf[LADC_BUFS_SIZE];    //align 4Bytes
...
static void linein_sample_24bit_output_handler(void *priv, s32 *data, int len)
{
    struct linein_sample_hdl *linein = (struct linein_sample_hdl *)priv;
    ...
    spin_lock(&linein->lock);
    if (linein->audio_track) {
        audio_local_sample_track_in_period(linein->audio_track, (len >> point_offset));
    }
    spin_unlock(&linein->lock);
    ...
    if (linein->output_fade_in) {
        ...
        for (int i = 0; i < len * linein->channel_num >> point_offset ; i++) {
            tmp_data = data[i];
...
#if defined(TCFG_AUDIO_ADC_BIT_MODE)&&ADC_BIT_WIDTH_24&&(TCFG_AUDIO_ADC_BIT_MODE == ADC_BIT_WIDTH_24)
    linein_sample_24bit_output_handler(priv, (s32 *)data, len);
    return;
#endif
    spin_lock(&linein->lock);
    if (linein->audio_track) {
...
__retry:
    ...
    point_offset = 2;
#endif
    u32 tlen = (ADC_LINEIN_IRQ_POINTS << point_offset) * linein->channel_num;
    if (tlen > len) {
        tlen = len;
    }
...
void *linein_sample_open(u8 source, u16 sample_rate)
{
    ...
    if (source == TCFG_LINEIN_LR_CH) {
        if (audio_linein_open(&linein->linein_ch, sample_rate, LINEIN_GAIN) == 0) {
            //先开mic，再开linein，由于混响和linein做了特殊处理，会导致open传进的gain参数没有设置到，这里再设置一下。
            audio_linein_set_gain(LINEIN_GAIN);
        ...
        if (audio_linein_fm_open(&linein->linein_ch, sample_rate, LINEIN_GAIN) == 0) {
            linein->sample_output.handler = linein_sample_output_handler;
            linein->sample_output.priv = linein;
...
void *fm_sample_open(u8 source, u16 sample_rate)
{
    ...
    if (source & AUDIO_ADC_LINE0) {
        audio_adc_linein_open(&linein->linein_ch, AUDIO_ADC_LINE0, &adc_hdl);
        audio_adc_linein_set_gain(&linein->linein_ch, 1);
    }
    if (source  & AUDIO_ADC_LINE1) {
        audio_adc_linein1_open(&linein->linein_ch, AUDIO_ADC_LINE1, &adc_hdl);
        audio_adc_linein1_set_gain(&linein->linein_ch, 1);
        printf("\n audio_adc_linein1_open \n");
    }
    if (source  & AUDIO_ADC_LINE2) {
        audio_adc_linein2_open(&linein->linein_ch, AUDIO_ADC_LINE2, &adc_hdl);
        audio_adc_linein2_set_gain(&linein->linein_ch, 1);
        printf("\n audio_adc_linein2_open \n");
    }
    if (source  & AUDIO_ADC_LINE3) {
        audio_adc_linein3_open(&linein->linein_ch, AUDIO_ADC_LINE3, &adc_hdl);
        audio_adc_linein3_set_gain(&linein->linein_ch, 1);
    }
    audio_adc_linein_set_sample_rate(&linein->linein_ch, sample_rate);
    audio_adc_linein_set_gain(&linein->linein_ch, 1);
    audio_adc_linein_set_buffs(&linein->linein_ch, linein->adc_buf,  ADC_IRQ_POINTS * 2, ADC_BUF_NUM);
...
#if RECORDING_DAC_ENABLE
#if (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)
    s32 tmp_buf[LADC_MIC_IRQ_POINTS * 2];
#else
    s16 tmp_buf[LADC_MIC_IRQ_POINTS * 2];
#endif
#endif
...
int recorder_get_enc_tmark(u8 *buf, u16 len)
{
    int ret = 0;
    if (rec_hdl && rec_hdl->file) {
        ret = pcm2file_enc_get_tmark(rec_hdl->file, buf, len);
    }
    return ret;
}
...
void recorder_encode_stop(void)
{
    if (rec_hdl) {
        ...
        if (rec_hdl->clock_added) {
            r_printf("recorder_encode_clock_remove>>>>>>>>>");
            recorder_encode_clock_remove(rec_hdl->coding_type);
        }
        rec_hdl->file = NULL;
        if (rec_hdl->cut_head_timer) {
...
static void adc_output_to_enc(void *priv, s16 *data, int len)
{
    ...
    case ENCODE_SOURCE_LINE2_LR:
        ...
        if (rec_hdl->tmp_buf) {
            ...
            s32 *temp_data = (s32 *)data;
            for (u16 i = 0; i < len / 4; i++) {
                rec_hdl->tmp_buf[2 * i] = temp_data[i];
                rec_hdl->tmp_buf[2 * i + 1] = temp_data[i];
            }
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num * 2);
#else
            s32 *temp_data = zalloc(len * adc_hdl.channel_num * 2);
            if (!temp_data) {
                return;
            }
            audio_convert_data_16bit_to_32bit_round(data, temp_data, (len * adc_hdl.channel_num * 2) / 4);
            for (u16 i = 0; i < len / 2; i++) {
                rec_hdl->tmp_buf[2 * i] = temp_data[i];
                rec_hdl->tmp_buf[2 * i + 1] = temp_data[i];
            }
            free(temp_data);
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num * 2 * 2);
#endif
#elif (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_16BIT_MODE)
#if (TCFG_AUDIO_ADC_BIT_MODE == ADC_BIT_WIDTH_24)&& (!TCFG_ADC_BIT_24_TO_16)
            s16 *temp_data = zalloc(len * adc_hdl.channel_num / 2);
            if (!temp_data) {
                return;
            }
            audio_convert_data_32bit_to_16bit_round((s32 *)data, temp_data, (len * adc_hdl.channel_num) / 4);
            for (u16 i = 0; i < len / 2; i++) {
                rec_hdl->tmp_buf[2 * i] = temp_data[i];
                rec_hdl->tmp_buf[2 * i + 1] = temp_data[i];
            }
            free(temp_data);
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num);
#else
            for (u16 i = 0; i < len / 2; i++) {
                rec_hdl->tmp_buf[2 * i] = data[i];
                rec_hdl->tmp_buf[2 * i + 1] = data[i];
            }
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num * 2);
#endif
#endif
...
#if (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)&&(TCFG_AUDIO_ADC_BIT_MODE != ADC_BIT_WIDTH_24)

            audio_convert_data_16bit_to_32bit_round(data, rec_hdl->tmp_buf, (len * adc_hdl.channel_num * 2) / 4);
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num * 2);

#elif (TCFG_AUDIO_DAC_BIT_WIDTH != DAC_24BIT_MODE)&&(TCFG_AUDIO_ADC_BIT_MODE == ADC_BIT_WIDTH_24)

            audio_convert_data_32bit_to_16bit_round((s32 *)data, rec_hdl->tmp_buf, (len * adc_hdl.channel_num) / 4);
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num / 2);
#else
            memcpy((u8 *)rec_hdl->tmp_buf, (u8 *)data, len * adc_hdl.channel_num)
            int wlen = app_audio_output_write(rec_hdl->tmp_buf, len * adc_hdl.channel_num);
#endif
#endif
...
#if 0//(RECORDER_INPUT_BIT_WIDTH == RECORDER_INPUT_24BIT)&&(RECORDER_OUTPUT_BIT_WIDTH == RECORDER_ENC_OUTPUT_24BIT)
        u32 outlen_l = len * adc_hdl.channel_num * 3 / 4;
        s32 *temp_data_l = zalloc(REC_ALIN(outlen_l, 4));//4byte对齐
        if (!temp_data_l) {
            return;
        }
        fourbyte_24bit_to_threebyte_24bit(data, temp_data_l, len * adc_hdl.channel_num / 4);
        pcm2file_enc_write_pcm(rec_hdl->file, temp_data_l, outlen_l);
        free(temp_data_l);
#elif 0//(RECORDER_INPUT_BIT_WIDTH == RECORDER_INPUT_16BIT)&&(RECORDER_OUTPUT_BIT_WIDTH == RECORDER_ENC_OUTPUT_24BIT)
        u32 outlen_l = len * adc_hdl.channel_num * 3 / 2;
        s32 *temp_data_l = zalloc(REC_ALIN(outlen_l, 4));//4byte对齐
        if (!temp_data_l) {
            return;
        }
        _16bit_to_threebyte_24bit(data, temp_data_l, len * adc_hdl.channel_num / 2);
        pcm2file_enc_write_pcm(rec_hdl->file, temp_data_l, outlen_l);
        free(temp_data_l);
#else
        pcm2file_enc_write_pcm(rec_hdl->file, data, len * adc_hdl.channel_num);
#endif
        break;
    default:
        break;
...
int recorder_encode_start(struct record_file_fmt *f)
{
    ...
    if ((fmt.coding_type == AUDIO_CODING_WAV) || fmt.coding_type == AUDIO_CODING_G726 || fmt.coding_type == AUDIO_CODING_PCM) {
    ...
    if ((fmt.source == ENCODE_SOURCE_LINE0_LR)
        ...
        if (afmt.bit_width) {
            size += REC_ALIN((sizeof(adc_type_t) * ADC_BUFS_SIZE * 2), 4);
        } else {
            size += REC_ALIN((sizeof(adc_type_t) * ADC_BUFS_SIZE), 4);
        }
    }
    ...
    u32 curr_clock_fre = clk_get("sys");
    if (curr_clock_fre < 192 * 1000000L) {
        recorder_encode_clock_add(fmt.coding_type);
        if (clk_get("sys") > curr_clock_fre) {
            rec->clock_added = 1;
        } else {
            rec->clock_added = 0;
        }
    ...
    if (!fmt.cut_head_time) {
        rec->cut_head_flag = 0;//剪切头标志
    } else {
        rec->cut_head_flag = 1;
    ...
    if (fmt.cut_tail_time) {
        if ((fmt.coding_type == AUDIO_CODING_WAV) || (fmt.coding_type == AUDIO_CODING_G726) || fmt.coding_type == AUDIO_CODING_PCM) {
            cut_tail_size = (4 * fmt.sample_rate * fmt.channel) * fmt.cut_tail_time / 1000 / 8;
            if (afmt.bit_width) {
                cut_tail_size = ((cut_tail_size + fmt.bit_rate - 1) / fmt.bit_rate) * fmt.bit_rate * 2;
            } else {
                cut_tail_size = ((cut_tail_size + fmt.bit_rate - 1) / fmt.bit_rate) * fmt.bit_rate;
            }
        } else {
            if (0) { //(afmt.bit_width) {//MP3暂时不支持24bit编码
                cut_tail_size = fmt.bit_rate * fmt.cut_tail_time / 4;
            } else {
    ...
    switch (rec->source) {
    case ENCODE_SOURCE_MIX:
        rec->source_pcm_bitwith = RECORDER_INPUT_BIT_WIDTH;//TCFG_AUDIO_DAC_BIT_WIDTH; //0:16bit 1:24bit
        pcm2file_enc_souce_pcm_bitwith_set(rec->file, rec->source_pcm_bitwith);
        break;
    case ENCODE_SOURCE_MIC:
#if 0
        audio_adc_mic_open(&rec->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
...
#else
        rec->source_pcm_bitwith = RECORDER_INPUT_BIT_WIDTH;//TCFG_AUDIO_ADC_BIT_MODE; //0:16bit 1:24bit
        pcm2file_enc_souce_pcm_bitwith_set(rec->file, rec->source_pcm_bitwith);
        rec->adc_output.handler = adc_output_to_enc;
    ...
    case ENCODE_SOURCE_LINE1_LR:
    case ENCODE_SOURCE_LINE2_LR:
        rec->source_pcm_bitwith = RECORDER_INPUT_BIT_WIDTH;//TCFG_AUDIO_ADC_BIT_MODE;
        pcm2file_enc_souce_pcm_bitwith_set(rec->file, rec->source_pcm_bitwith);
        ...
        audio_adc_linein_set_buffs(&rec->linein_ch, rec->adc_buf, ADC_IRQ_POINTS * 2, ADC_BUF_NUM);
        rec->adc_output.handler = adc_output_to_enc;
    ...
    app_audio_output_samplerate_set(fmt.sample_rate);
    app_audio_output_start();
#endif
