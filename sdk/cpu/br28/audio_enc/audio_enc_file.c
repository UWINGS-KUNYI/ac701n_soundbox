...
#if (RECORDER_INPUT_BIT_WIDTH != RECORDER_INPUT_16BIT) || (RECORDER_OUTPUT_BIT_WIDTH != RECORDER_ENC_OUTPUT_16BIT)
#define PCM_ENC2FILE_PCM_LEN_OVERLAY		(4 * 1024 + 512) * 2
#define PCM_ENC2FILE_FILE_LEN_OVERLAY		(4 * 1024) * 2
#else
...
#else
#if (RECORDER_INPUT_BIT_WIDTH != RECORDER_INPUT_16BIT) || (RECORDER_OUTPUT_BIT_WIDTH != RECORDER_ENC_OUTPUT_16BIT)
#define PCM_ENC2FILE_PCM_LEN_OVERLAY		(1 * 1024 + 512) * 2
#define PCM_ENC2FILE_FILE_LEN_OVERLAY		(2 * 1024) * 2
#else
...
#if (RECORDER_INPUT_BIT_WIDTH != RECORDER_INPUT_16BIT) || (RECORDER_OUTPUT_BIT_WIDTH != RECORDER_ENC_OUTPUT_16BIT)
#define ENC_OUTPUT_FRAME_LEN        2048
#define ENC_READ_PCM_FRAME_LEN      128 * 2
#define ENC_OUTPUT_FILE_FRAME_LEN   512
#define FILE_HEAD_FRAME_LEN         128
#else
#define ENC_OUTPUT_FRAME_LEN        2048 / 2
#define ENC_READ_PCM_FRAME_LEN      64
#define ENC_OUTPUT_FILE_FRAME_LEN   512 / 2
#define FILE_HEAD_FRAME_LEN         128
#endif
...
struct pcm2file_enc_hdl {
    ...
    s16 output_frame[ENC_OUTPUT_FRAME_LEN];             //align 4Bytes

    int pcm_frame[ENC_READ_PCM_FRAME_LEN];                 //align 4Bytes 编码器一帧读缓存

    u8 file_head_frame[FILE_HEAD_FRAME_LEN];
    u8 file_head_len;
    /* u8 pcm_buf[PCM_ENC2FILE_PCM_LEN]; */
    u8 *pcm_buf;
    cbuffer_t pcm_cbuf;
    u8 pcm_frame_bitwith;
    int out_file_frame[ENC_OUTPUT_FILE_FRAME_LEN];// 写文件一帧buffer缓存
...
static int pcm2file_enc_write_pcm_base(void *priv, s16 *data, int len)
{
...
// 写pcm数据
int pcm2file_enc_write_pcm(void *priv, void *data, int len)
{
    struct pcm2file_enc_hdl *enc = (struct pcm2file_enc_hdl *)priv;
    if (!enc || enc->enc_err || enc->status == 0) {
        return 0;
...
#if 0
    //位宽转换
    if (enc->pcm_frame_bitwith) {
        s32 *in_buf = (s32 *)data;
        s16 *out_buf = (s16 *)data;
        audio_convert_data_32bit_to_16bit_round(in_buf, out_buf, len / 4);
        wlen = pcm2file_enc_write_pcm_base(enc, out_buf, len / 2);
        if (len != wlen * 2) {
            putchar('#');
        }
        return wlen * 2;
...
#endif
    wlen = pcm2file_enc_write_pcm_base(enc, data, len);

    return wlen;

}

int pcm2file_enc_souce_pcm_bitwith_set(void *priv, u8 bitwith)
{
    struct pcm2file_enc_hdl *enc = (struct pcm2file_enc_hdl *)priv;
    if (!enc || enc->enc_err) {
        return 0;
    }
    enc->pcm_frame_bitwith = bitwith;
    return 0;
}
...
int pcm2file_enc_get_tmark(void *hdl, u8 *buf, u16 len)
{
    ...
    int ret = audio_encoder_ioctrl(&pcm2file->encoder, 2, AUDIO_ENCODER_IOCTRL_CMD_GET_TMARK, buf);
    os_mutex_post(&pcm2file->mutex);
    return ret;
}
...
void *pcm2file_enc_open(struct audio_fmt *pfmt, char *logo, char *folder, char *filename)
{
    ...
    if (pfmt->coding_type != AUDIO_CODING_MP3 && pfmt->coding_type != AUDIO_CODING_WAV && pfmt->coding_type != AUDIO_CODING_G726 && pfmt->coding_type != AUDIO_CODING_PCM) {
        return NULL;
    }
...
#endif/*RECORDER_MIX_EN*/
    {
        ...
        if (pfmt->bit_width) {
            pcm_buf_size = PCM_ENC2FILE_PCM_LEN * 2;
            out_file_buf_size = PCM_ENC2FILE_FILE_LEN * 2;
        } else {
    ...
    } else if (pfmt->coding_type == AUDIO_CODING_WAV || pfmt->coding_type == AUDIO_CODING_G726 || pfmt->coding_type == AUDIO_CODING_PCM) {
        strcat(temp_filename, filename);
    ...
    pfmt->bit_width = RECORDER_OUTPUT_BIT_WIDTH;//这里才是设置编码输出位宽的地方
    ...
    int ret = audio_encoder_ioctrl(&pcm2file->encoder, 2, AUDIO_ENCODER_IOCTRL_CMD_SET_INPUT_BITWIDTH, RECORDER_INPUT_BIT_WIDTH);


    if (!pcm2file->encoder.enc_priv) {
