...
struct mic_enc_hdl {
    ...
    adc_type_t adc_buf[ENC_ADC_BUFS_SIZE];    //align 4Bytes
#endif
};
...
int audio_mic_enc_open(int (*mic_output)(void *priv, void *buf, int len), u32 code_type)
{
    ...
    if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_0) {
        printf("audio_mic_enc_open mic 0");
        audio_adc_mic_open(&mic_enc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    } else if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_1) {
        printf("audio_mic_enc_open mic 1");
        audio_adc_mic1_open(&mic_enc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    } else if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_2) {
        printf("audio_mic_enc_open mic 2");
        audio_adc_mic2_open(&mic_enc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    } else if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_3) {
        printf("audio_mic_enc_open mic 3");
        audio_adc_mic3_open(&mic_enc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    }
