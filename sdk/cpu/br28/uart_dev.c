...
void uart_set_dma_dir(u8 ut_num, u32 dir)
{
    if (tx_rx_pin[ut_num] != 0xff) {
        if (dir) {
            gpio_set_direction(tx_rx_pin[ut_num], 1);
            ((JL_UART_TypeDef *)uart_t[ut_num].argv)->CON1 &= ~BIT(4);
        } else {
            ((JL_UART_TypeDef *)uart_t[ut_num].argv)->CON1 |= BIT(4);
            gpio_set_direction(tx_rx_pin[ut_num], 0);
        }
    }
}
...
static u8 UT0_getbyte(u8 *buf, u32 timeout)
{
    ...
    if (JL_UART0->CON0 & BIT(6)) {
        //DMA_MODE
        if (!kfifo_length(&uart_t[0].kfifo)) {
            UT_OSSemPend(&uart_t[0].sem_rx, timeout);
        }
        UT_OSSemSet(&uart_t[0].sem_rx, 0);

        return kfifo_get(&uart_t[0].kfifo, buf, 1);
    } else {
        uart_set_dma_dir(0, 1);
...
static void UT0_isr_fun(void)
{
    ...
    if ((JL_UART0->CON0 & BIT(2)) && (JL_UART0->CON0 & BIT(15))) {
        JL_UART0->CON0 |= BIT(13);
        UT_OSSemPost(&uart_t[0].sem_tx);
        if (uart_t[0].isr_cbfun) {
            uart_t[0].isr_cbfun(&uart_t[0], UT_TX);
        }
        uart_set_dma_dir(0, 1);
    }
    if ((JL_UART0->CON0 & BIT(3)) && (JL_UART0->CON0 & BIT(14))) {
        ...
        /* uart_t[0].kfifo.buf_in += uart_t[0].frame_length; //每满frame_length字节则产生一次中断 */
        uart_t[0].kfifo.buf_in += rx_len;
        UT_OSSemPost(&uart_t[0].sem_rx);
        if (uart_t[0].isr_cbfun) {
            uart_t[0].isr_cbfun(&uart_t[0], UT_RX);
        }
    }
    if ((JL_UART0->CON0 & BIT(5)) && (JL_UART0->CON0 & BIT(11))) {
        ...
        if (rx_len) {
            ...
            UT_OSSemPost(&uart_t[0].sem_rx);
            if (uart_t[0].isr_cbfun) {
                uart_t[0].isr_cbfun(&uart_t[0], UT_RX_OT);
            }
        }
    }
}
...
static u32 UT0_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART0->CON0 & BIT(6)) {
        ...
        return kfifo_get(&uart_t[0].kfifo, buf, len);
    } else {
        uart_set_dma_dir(0, 1);
        _timeout = timeout + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
...
static void UT0_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART0_ENABLE_TX_DMA) {
        uart_set_dma_dir(0, 0);
        UT_OSSemSet(&uart_t[0].sem_tx, 0);
        JL_UART0->CON0 |= BIT(13);
        JL_UART0->CON0 |= BIT(2);
        JL_UART0->TXADR = (u32)buf;
        JL_UART0->TXCNT = len;
        UT_OSSemPend(&uart_t[0].sem_tx, 0);
        JL_UART0->CON0 &= ~BIT(2);
    } else {
        for (i = 0; i < len; i ++) {
            UT0_putbyte(*(buf + i));
...
static void UT0_set_baud(u32 baud)
{
    ...
    if (JL_UART0->CON0 & BIT(5)) {
        if (uart_t[0].rx_timeout > 10) {
            JL_UART0->OTCNT = (uart_t[0].rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            JL_UART0->OTCNT = uart_t[0].rx_timeout * UART_OT_CLK / 1000;
        }
    }
...
{
    return kfifo_length(&uart_t[1].kfifo);
}
...
{
    JL_UART0->CON0 = BIT(13) | BIT(12) | BIT(10);
    UT_OSSemCreate(&uart_t[0].sem_rx, 0);
    UT_OSSemCreate(&uart_t[0].sem_tx, 0);
    request_irq(IRQ_UART0_IDX, 3, UT0_isr_fun, 0);
    if (cbuf) {
        uart_t[0].kfifo.buffer = cbuf;
        uart_t[0].kfifo.buf_size = cbuf_size;
        uart_t[0].kfifo.buf_in = 0;
        uart_t[0].kfifo.buf_out = 0;
        uart_t[0].frame_length = rx_cnt;
        uart_t[0].rx_timeout = ot;
        JL_UART0->RXSADR = (u32)uart_t[0].kfifo.buffer;
        JL_UART0->RXEADR = (u32)(uart_t[0].kfifo.buffer + uart_t[0].kfifo.buf_size);
        JL_UART0->RXCNT = uart_t[0].frame_length;
...
{
    UT_OSSemClose(&uart_t[0].sem_rx);
    UT_OSSemClose(&uart_t[0].sem_tx);
...
static u8 UT1_getbyte(u8 *buf, u32 timeout)
{
    ...
    if (JL_UART1->CON0 & BIT(6)) {
        if (!kfifo_length(&uart_t[1].kfifo)) {
            UT_OSSemPend(&uart_t[1].sem_rx, timeout);
        }
        UT_OSSemSet(&uart_t[1].sem_rx, 0);

        return kfifo_get(&uart_t[1].kfifo, buf, 1);
    } else {
        uart_set_dma_dir(1, 1);
        _timeout = timeout + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
...
static void UT1_isr_fun(void)
{
    ...
    if ((JL_UART1->CON0 & BIT(2)) && (JL_UART1->CON0 & BIT(15))) {
        JL_UART1->CON0 |= BIT(13);
        UT_OSSemPost(&uart_t[1].sem_tx);
        if (uart_t[1].isr_cbfun) {
            uart_t[1].isr_cbfun(&uart_t[1], UT_TX);
        }
        uart_set_dma_dir(1, 1);
    }
    if ((JL_UART1->CON0 & BIT(3)) && (JL_UART1->CON0 & BIT(14))) {
        ...
        /* uart_t[1].kfifo.buf_in += uart_t[1].frame_length; //每满32字节则产生一次中断 */
        uart_t[1].kfifo.buf_in += rx_len;
        UT_OSSemPost(&uart_t[1].sem_rx);
        if (uart_t[1].isr_cbfun) {
            uart_t[1].isr_cbfun(&uart_t[1], UT_RX);
        }
    }
    ...
    if ((JL_UART1->CON0 & BIT(5)) && (JL_UART1->CON0 & BIT(11))) {
        ...
        if (rx_len) {
            ...
            UT_OSSemPost(&uart_t[1].sem_rx);
            if (uart_t[1].isr_cbfun) {
                uart_t[1].isr_cbfun(&uart_t[1], UT_RX_OT);
            }
        }
    }
}
...
static u32 UT1_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART1->CON0 & BIT(6)) {
        ...
        return kfifo_get(&uart_t[1].kfifo, buf, len);
    } else {
        uart_set_dma_dir(1, 1);
        _timeout = timeout + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
...
static void UT1_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART1_ENABLE_TX_DMA) {
        uart_set_dma_dir(1, 0);
        UT_OSSemSet(&uart_t[1].sem_tx, 0);
        JL_UART1->CON0 |= BIT(13);
        JL_UART1->CON0 |= BIT(2);
        JL_UART1->TXADR = (u32)buf;
        JL_UART1->TXCNT = len;
        UT_OSSemPend(&uart_t[1].sem_tx, 0);
        JL_UART1->CON0 &= ~BIT(2);
    } else {
        for (i = 0; i < len; i ++) {
            UT1_putbyte(*(buf + i));
...
static void UT1_set_baud(u32 baud)
{
    ...
    if (JL_UART1->CON0 & BIT(5)) {
        if (uart_t[1].rx_timeout > 10) {
            JL_UART1->OTCNT = (uart_t[1].rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            JL_UART1->OTCNT = uart_t[1].rx_timeout * UART_OT_CLK / 1000;
        }
    }
...
{
    JL_UART1->CON0 = BIT(13) | BIT(12) | BIT(10);
    UT_OSSemCreate(&uart_t[1].sem_rx, 0);
    UT_OSSemCreate(&uart_t[1].sem_tx, 0);
    request_irq(IRQ_UART1_IDX, 3, UT1_isr_fun, 0);
    if (cbuf) {
        uart_t[1].kfifo.buffer = cbuf;
        uart_t[1].kfifo.buf_size = cbuf_size;
        uart_t[1].kfifo.buf_in = 0;
        uart_t[1].kfifo.buf_out = 0;
        uart_t[1].frame_length = rx_cnt;
        uart_t[1].rx_timeout = ot;
        JL_UART1->RXSADR = (u32)uart_t[1].kfifo.buffer;
        JL_UART1->RXEADR = (u32)(uart_t[1].kfifo.buffer + uart_t[1].kfifo.buf_size);
        JL_UART1->RXCNT = uart_t[1].frame_length;
...
{
    UT_OSSemClose(&uart_t[1].sem_rx);
    UT_OSSemClose(&uart_t[1].sem_tx);
...
static u8 UT2_getbyte(u8 *buf, u32 timeout)
{
    ...
    if (JL_UART2->CON0 & BIT(6)) {
        if (!kfifo_length(&uart_t[2].kfifo)) {
            UT_OSSemPend(&uart_t[2].sem_rx, timeout);
        }
        UT_OSSemSet(&uart_t[2].sem_rx, 0);

        return kfifo_get(&uart_t[2].kfifo, buf, 1);
    } else {
        uart_set_dma_dir(2, 1);
        _timeout = timeout + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
...
static void UT2_isr_fun(void)
{
    ...
    if ((JL_UART2->CON0 & BIT(2)) && (JL_UART2->CON0 & BIT(15))) {
        JL_UART2->CON0 |= BIT(13);
        UT_OSSemPost(&uart_t[2].sem_tx);
        if (uart_t[2].isr_cbfun) {
            uart_t[2].isr_cbfun(&uart_t[2], UT_TX);
        }
        uart_set_dma_dir(2, 1);
    }
    if ((JL_UART2->CON0 & BIT(3)) && (JL_UART2->CON0 & BIT(14))) {
        ...
        /* uart_t[2].kfifo.buf_in += uart_t[2].frame_length; //每满32字节则产生一次中断 */
        uart_t[2].kfifo.buf_in += rx_len;
        UT_OSSemPost(&uart_t[2].sem_rx);
        if (uart_t[2].isr_cbfun) {
            uart_t[2].isr_cbfun(&uart_t[2], UT_RX);
        }
    }
    ...
    if ((JL_UART2->CON0 & BIT(5)) && (JL_UART2->CON0 & BIT(11))) {
        ...
        if (rx_len) {
            ...
            UT_OSSemPost(&uart_t[2].sem_rx);
            if (uart_t[2].isr_cbfun) {
                uart_t[2].isr_cbfun(&uart_t[2], UT_RX_OT);
            }
        }
    }
}
...
static u32 UT2_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART2->CON0 & BIT(6)) {
        ...
        UT_OSSemSet(&uart_t[2].sem_rx, 0);
        return kfifo_get(&uart_t[2].kfifo, buf, len);
    } else {
        uart_set_dma_dir(2, 1);
        _timeout = timeout + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
...
static void UT2_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART2_ENABLE_TX_DMA) {
        uart_set_dma_dir(2, 0);
        UT_OSSemSet(&uart_t[2].sem_tx, 0);
        JL_UART2->CON0 |= BIT(13);
        JL_UART2->CON0 |= BIT(2);
        JL_UART2->TXADR = (u32)buf;
        JL_UART2->TXCNT = len;
        UT_OSSemPend(&uart_t[2].sem_tx, 0);
        JL_UART2->CON0 &= ~BIT(2);
    } else {
        for (i = 0; i < len; i++) {
            UT2_putbyte(*(buf + i));
...
static void UT2_set_baud(u32 baud)
{
    ...
    if (JL_UART2->CON0 & BIT(5)) {
        if (uart_t[2].rx_timeout > 10) {
            JL_UART2->OTCNT = (uart_t[2].rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
        } else {
            JL_UART2->OTCNT = uart_t[2].rx_timeout * UART_OT_CLK / 1000;
        }
    }
...
{
    JL_UART2->CON0 = BIT(13) | BIT(12) | BIT(10);
    UT_OSSemCreate(&uart_t[2].sem_rx, 0);
    UT_OSSemCreate(&uart_t[2].sem_tx, 0);
    request_irq(IRQ_UART2_IDX, 3, UT2_isr_fun, 0);
    if (cbuf) {
        uart_t[2].kfifo.buffer = cbuf;
        uart_t[2].kfifo.buf_size = cbuf_size;
        uart_t[2].kfifo.buf_in = 0;
        uart_t[2].kfifo.buf_out = 0;
        uart_t[2].frame_length = rx_cnt;
        uart_t[2].rx_timeout = ot;
        JL_UART2->RXSADR = (u32)uart_t[2].kfifo.buffer;
        JL_UART2->RXEADR = (u32)(uart_t[2].kfifo.buffer + uart_t[2].kfifo.buf_size);
        JL_UART2->RXCNT = uart_t[2].frame_length;
...
{
    UT_OSSemClose(&uart_t[2].sem_rx);
    UT_OSSemClose(&uart_t[2].sem_tx);
...
const uart_bus_t *uart_dev_open(const struct uart_platform_data_t *arg)
{
    ...
    if (CONFIG_UART0_ENABLE && ut_num == 0) {
        ...
        uart_t[0].argv = JL_UART0;//arg->argv;
        uart_t[0].isr_cbfun = arg->isr_cbfun;
        uart_t[0].putbyte = UT0_putbyte;
        uart_t[0].getbyte = UT0_getbyte;
        uart_t[0].read = UT0_read_buf;
        uart_t[0].write = UT0_write_buf;
        uart_t[0].set_baud = UT0_set_baud;
        ...
        uart_t[1].argv = JL_UART1;//arg->argv;
        uart_t[1].isr_cbfun = arg->isr_cbfun;
        uart_t[1].putbyte = UT1_putbyte;
        uart_t[1].getbyte = UT1_getbyte;
        uart_t[1].read    = UT1_read_buf;
        uart_t[1].write   = UT1_write_buf;
        uart_t[1].set_baud = UT1_set_baud;
        uart_t[1].get_data_len = uart1_get_data_len;
        ...
        uart_t[2].argv = JL_UART2;//arg->argv;
        uart_t[2].isr_cbfun = arg->isr_cbfun;
        uart_t[2].putbyte = UT2_putbyte;
        uart_t[2].getbyte = UT2_getbyte;
        uart_t[2].read    = UT2_read_buf;
        uart_t[2].write   = UT2_write_buf;
        uart_t[2].set_baud = UT2_set_baud;
    ...
    if ((arg->rx_pin == arg->tx_pin)) {
        tx_rx_pin[ut_num] = arg->rx_pin;
        uart_set_dma_dir(ut_num, 1);
    } else {
        tx_rx_pin[ut_num] = 0xff;
    }
    printf("uart:%d open ok!\n", ut_num);
    return ut;
}
u32 uart_dev_close(uart_bus_t *ut)//return0:ok,1:err
{
