...
#if (SOUNDCARD_ENABLE)
//声卡音量表
u16 soundcard_vol_table[] = {
    //music, a2dp,  pc,  file, linein,  fm
    16,      16,    16,  16,   16,      16,
};
...
{
#if defined(AUDIO_VBASS_LINK_VOLUME) && AUDIO_VBASS_LINK_VOLUME
#include "audio_vbass_demo.h"
#include "media/effects_adj.h"
    u8 max_vol = app_audio_get_max_volume();
    u32 vbass_name = AEID_MUSIC_VBASS;
    if (state == APP_AUDIO_STATE_CH_LINEIN) {
        u32 vbass_name = AEID_AUX_VBASS;
    }
    audio_vbass_link_volume(vbass_name, max_vol, volume);
#endif

#if (SOUNDCARD_ENABLE)
    if (state >= APP_AUDIO_STATE_MUSIC_CH_BASE && state <= APP_AUDIO_STATE_CH_FM) {
        soundcard_vol_table[(state % APP_AUDIO_STATE_MUSIC_CH_BASE)] = (u16)volume;
    }
#endif

    if (state > APP_AUDIO_STATE_MUSIC_CH_BASE) {
        if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
            ...
            switch (state) {
            case APP_AUDIO_STATE_CH_A2DP:
                logo = "music_a2dp";
                break;
            case APP_AUDIO_STATE_CH_PC:
                logo = "music_pc";
                break;
            case APP_AUDIO_STATE_CH_FILE:
                logo = "music_file";
                break;
            case APP_AUDIO_STATE_CH_LINEIN:
                logo = "music_linein";
                break;
            case APP_AUDIO_STATE_CH_FM:
                logo = "music_fm";
                break;
            default:
                printf("app audio unknow state type %x\n", state);
                return;
            }
            audio_dig_vol_group_vol_set(sys_digvol_group, logo, AUDIO_DIG_VOL_ALL_CH, volume);
            return;
        } else {
            state = APP_AUDIO_STATE_MUSIC;
        }
    }
    ...
    case APP_AUDIO_STATE_MUSIC:
        music_state = music_player_get_play_status();
...
#if TCFG_HOST_AUDIO_ENABLE
        audio_usb_spk_set_volume(volume);
#endif
        break;
    ...
    if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
        ...
        if (state == APP_AUDIO_STATE_WTONE) {
            digvol_type = "tone";
        } else if (state == APP_AUDIO_STATE_CALL) {
            digvol_type = "call";
        } else {    // music
            digvol_type = "music";
        }
        ...
        if (music_state == FILE_DEC_STATUS_PAUSE) { //音乐暂停后，直接更新数字音量
            audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_ALL_CH, volume, 1);
        } else {
            audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_ALL_CH, volume, 0);
        }
        return;
    }
...
void app_audio_set_volume_each_channel(u8 state, s8 volume_l, s8 volume_r, u8 fade)
{
    if (state > APP_AUDIO_STATE_MUSIC_CH_BASE) {
        if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
            ...
            case APP_AUDIO_STATE_CH_FM:
                ...
                break;
            default:
                printf("app audio each channel unknow state type %x\n", state);
                return;
            }
            audio_dig_vol_group_vol_set(sys_digvol_group, logo, AUDIO_DIG_VOL_CH(0), volume_l);
            audio_dig_vol_group_vol_set(sys_digvol_group, logo, AUDIO_DIG_VOL_CH(1), volume_r);
            return;
        } else {
            state = APP_AUDIO_STATE_MUSIC;
        }
    ...
    if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
        ...
        audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(0), volume_l, 0);
        audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(1), volume_r, 0);
        return;
    }
#endif //vol_type_diggroup
...
void app_audio_state_switch(u8 state, s16 max_volume)
{
    ...
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FL, DEFAULT_DIGTAL_VOLUME);
    audio_way_set_digital_gain(AUDIO_WAY_TYPE_DAC, SOUND_CHMAP_FR, DEFAULT_DIGTAL_VOLUME);
#endif
