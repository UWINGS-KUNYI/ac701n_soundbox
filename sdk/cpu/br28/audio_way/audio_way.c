...
extern int audio_codec_irq_init(void *private_data, u8 codec_direction);
extern int audio_dac_io_irq_init(void *private_data, u8 codec_direction);
...
struct sound_pcm_substream *audio_way_get_substream(u32 way_type)
{
    struct audio_way *p;
    list_for_each_entry(p, &__this->head, entry) {
        if (p->way_type == way_type) {
...
static int audio_valid_sound_detect(void *data, int len, int *offset)
{
    s16 *pcm = (s16 *)data;
    int samples = len >> 1;
    int i;
...
static void audio_valid_sound_up_timeout(void *arg)
{
...
static void sound_pcm_valid_output_test(struct sound_pcm_stream *stream, void *data, int bytes)
{
#if (defined TCFG_SOUND_PCM_DELAY_TEST && TCFG_SOUND_PCM_DELAY_TEST)
    int offset = 0;
    if (audio_valid_sound_detect(data, bytes, &offset)) {
        struct sound_pcm_hw_params params = {0};
        sound_pcm_iotcl(stream, SOUND_PCM_GET_HW_PARAMS, &params);
        offset /= params.channels;
        int buffered_frames = sound_pcm_iotcl(stream, SOUND_PCM_GET_HW_BUFFERED_LEN, NULL);
        int timeout = (buffered_frames + offset) * 1000 / params.sample_rate;
        sys_hi_timeout_add(NULL, audio_valid_sound_up_timeout, timeout);
    }
#endif
}
        ...
        if (p->state == AUDIO_WAY_STATE_START) {
            ...
            if (p->ch_switch) { // 声道转换
...
#endif /*(AUDIO_WAY_SUPPORT & AUDIO_WAY_SUPPORT_SRC)*/
                {
                    ...
                    sound_pcm_valid_output_test(p->stream, buf, wlen);
                }
            if (olen == -1) { // 主输出方式
...
int audio_way_set_sample_rate(u32 way_type, u32 sample_rate)
{
    ...
    LIST_FOR_EACH_ENTRY_HEAD() {
        ...
        int out_sr = audio_way_get_sample_rate(BIT(i)); // 获取当前正在使用的采样;
        if (BIT(i) == AUDIO_WAY_TYPE_IIS && out_sr == 48000) { //iis 相对于DAC的48K 有偏差，实际是48.076k,故需要做变采样
            int iis_data_points = sound_pcm_device_buffered_len((void *)AUDIO_WAY_TYPE_IIS);
            out_sr = out_sr * 625 * 20 / (624 * 20) + 1 ;
            printf("iis out samplerate :%d\n", out_sr);
        }
#if (AUDIO_WAY_SUPPORT & AUDIO_WAY_SUPPORT_SRC)
        p->out_sr = out_sr;
        ...
        if (p->in_sr != p->out_sr) { // 输入输出不同，变采样
            if (p->src == NULL) {
                ...
                ASSERT(p->src);
                if (global_bit_wide) {
                    audio_hw_src_open(p->src, p->out_ch, SET_RESAMPLE_TYPE(AUDIO_SAMPLE_FMT_24BIT, SRC_TYPE_RESAMPLE));
                    /* y_printf(">>>>>> %s open src 24bit",__FUNCTION__); */
                } else {
...
/*
*********************************************************************
*                  Audio App Set Underrun Frames
* Description: 设置输出方式少于多少ms的字节数时淡出数据
* Arguments  : way_type		audio输出方式
               underrun_ms  当硬件缓存少于这个值时将缓存数据淡出
* Return	 : >0			设置成功
* 			   =0			出错
* Note(s)    : 如果有多个输出方式，设置第一个输出方式的中断点数
*********************************************************************
*/
u8 audio_way_substream_set_underrun_frames(u32 way_type, u32 underrun_ms)
{
    struct audio_way *p;
    list_for_each_entry(p, &__this->head, entry) {
        if (p->way_type == way_type) {
            struct sound_pcm_substream *substream = p->stream->substream ;
            if (substream) {
                /* struct audio_dac_context *dac = (struct audio_dac_context *)(substream->private_data); */
                int sample_rate = audio_way_get_sample_rate(AUDIO_WAY_TYPE_DAC);
                if (substream->runtime) {
                    substream->runtime->underrun_frames = sample_rate * underrun_ms / 1000;
                    /* if(!substream->runtime->fade_buf){ */
                    /* substream->runtime->fade_buf  */
                    /* } */
                }
            }
            return 1;
