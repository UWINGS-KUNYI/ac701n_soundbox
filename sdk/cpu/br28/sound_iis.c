...
#define ALINK_FIFO_BUFFERED_FRAMES(hw, ch)         ((alink_get_dma_len(hw)  - alink_get_shn(ch)) / 2 - 1)
#else
...
static void sound_iis_dma_fifo_sync(struct sound_pcm_substream *substream, u8 flag)
{
    ...
    if (flag & DMA_SYNC_R) {
        ...
        int read_frames = iis->hw_ptr - read_ptr;
    ...
    if (flag & DMA_SYNC_W) {
        ...
        if (iis->pd->bitwide == ALINK_LEN_24BIT) {
            alink_set_shn(iis->hw_ch, write_frames * 2);
        } else {
...
static void audio_iis_tx_irq_handler(void *priv, void *buf, u32 len)
{
    ...
    list_for_each_entry(substream, &iis->stream_list, entry) {
        if (substream->runtime->irq_trigger) {
            sound_pcm_substream_irq_handler(substream);
            substream->runtime->irq_trigger = 0;
        }
    }
...
static int sound_iis_new_context(void **device, struct sound_pcm_platform_data *data)
{
    ...
    *device = ctx;
    ...
    if (ctx->pd->bitwide == ALINK_LEN_24BIT) {
        ctx->hw_params.sample_bits = 24;
    } else {
    ...
    if (ctx->pd->bitwide == ALINK_LEN_24BIT) {
        ctx->fifo.frame_len = ctx->fifo.bytes / 4 / ctx->hw_params.channels;
    } else {
...
static int sound_iis_dma_prepare(struct sound_pcm_substream *substream)
{
    ...
    if (iis->hw_params.sample_bits == 24) {
        audio_cfifo_set_readlock_samples(&iis->fifo.cfifo, 1);
    }
...
static int sound_iis_dma_irq_request(struct sound_pcm_substream *substream)
{
    struct sound_iis_context *iis = substream_to_iis_context(substream);
    struct sound_pcm_runtime *runtime = substream->runtime;
    struct sound_pcm_substream *other_stream;
    u8 irq_enabled = 0;

    local_irq_disable();
    u16 pns = 0;//
    runtime->irq_trigger = 1;
    if (runtime->sw_ptr >= runtime->dma_irq_ptr) {
        pns = runtime->sw_ptr - runtime->dma_irq_ptr;
    } else {
        pns = 0xffffffff - runtime->dma_irq_ptr + runtime->sw_ptr;
    }
    ...
    list_for_each_entry(other_stream, &iis->stream_list, entry) {
        if (substream == other_stream) {
            continue;
        }

        if (other_stream->runtime->irq_trigger) {
            irq_enabled = 1;
            break;
        }
    }

    if (irq_enabled && pns < alink_get_tx_pns(iis->hw_ch)) {
        pns = alink_get_tx_pns(iis->hw_ch);
    }
...
static int sound_iis_pcm_ioctl(struct sound_pcm_substream *substream, u32 cmd, void *args)
{
    ...
    case SOUND_PCM_GET_HW_BUFFERED_LEN:
        if (substream->hw_opened) {
            return ALINK_FIFO_BUFFERED_FRAMES(iis->hw, iis->hw_ch) - (substream->hw_opened ? audio_cfifo_channel_unread_diff_samples(&substream->fifo) : 0);
        }
        return 0;
