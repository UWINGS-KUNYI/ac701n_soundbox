...
void lp_touch_key_init(const struct lp_touch_key_platform_data *config)
{
    ...
    if ((is_wakeup_source(PWR_WK_REASON_P11) == 0) || get_charge_online_flag() || (!(ch_wakeup_en)) || (is_ldo5v_wakeup())) {
    ...
    //CTMU 初始化命令
    if ((is_wakeup_source(PWR_WK_REASON_P11) == 0) || get_charge_online_flag() || (!(ch_wakeup_en)) || (is_ldo5v_wakeup())) {
        log_info("lp touch init by %d_%d_%d_%d\n", is_wakeup_source(PWR_WK_REASON_P11), get_charge_online_flag(), (!(ch_wakeup_en)), is_ldo5v_wakeup());
...
{
    log_debug("%s", __func__);
#ifdef LP_TOUCH_KEY_CHARGE_MODE_SW_DISABLE
