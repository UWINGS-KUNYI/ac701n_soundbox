...
void audio_dac_demo_open(void)
{
    ...
    dac_sound_demo = audio_sound_dac_open(&demo_dac_param);
    if (dac_sound_demo == NULL) {
        goto _error;
    }
    ret = audio_sound_dac_start(dac_sound_demo);
    if (ret) {
        goto _error;
    }
    audio_sound_dac_set_analog_gain(dac_sound_demo, 0xF, 3);
    audio_sound_dac_set_digital_gain(dac_sound_demo, 0xF, 16384);
    struct audio_way *dac_channel = dac_sound_demo->way;
    //	判断声道数，双声道需要复制多一个声道的数据
    if (dac_channel->out_ch ==  2) {
    ...
    while (1) {
        ...
        while (len) {
            // 往 dac 写数据
            wlen = audio_sound_dac_write(dac_sound_demo, ptr, len);
#if KEY_TEST
    ...
    return;
_error:
    audio_sound_dac_stop(dac_sound_demo);
    audio_sound_dac_close(dac_sound_demo);
    audio_sound_dac_init();
    return;
}
