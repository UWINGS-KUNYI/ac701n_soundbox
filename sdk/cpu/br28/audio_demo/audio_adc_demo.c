...
struct audio_sound_dac *demo_dac;
...
struct adc_demo {
    ...
    adc_type_t adc_buf[ADC_DEMO_BUFS_SIZE];
    u8 linein_ch_num;
    u8 linein_ch_sel;
};
...
static void adc_mic_demo_output(void *priv, s16 *data, int len)
{
    ...
    if (mic_demo && mic_demo->adc_2_dac) {
        if (mic_demo->mic_idx == (AUDIO_ADC_MIC_0 | AUDIO_ADC_MIC_1)) {
            ...
            /*输出两个mic的数据到DAC两个通道*/
            putchar('a');
            wlen = audio_sound_dac_write(demo_dac, data, len * 2);
#else
            ...
            wlen = audio_sound_dac_write(demo_dac, mono_data, len);
            ...
            putchar('c');
            wlen = audio_sound_dac_write(demo_dac, temp, len * 2);
#else
            /*输出一个mic的数据到DAC一个通道*/
            putchar('d');
            wlen = audio_sound_dac_write(demo_dac, data, len);
#endif
        }
    }
...
void audio_adc_mic_demo_open(u8 mic_idx, u8 gain, u16 sr, u8 mic_2_dac)
{
    ...
    if (mic_demo) {
        ...
        if (mic_2_dac) {
            ...
            struct dac_param mic_dac = {0};
            mic_dac.sample_rate = sr;
            mic_dac.delay_ms = 50;
            demo_dac = audio_sound_dac_open(&mic_dac);
            audio_sound_dac_start(demo_dac);
...
void audio_adc_linein_demo_open(u8 linein_idx, u8 gain, u16 sr, u8 linein_2_dac)
{
    ...
    if (linein_demo) {
        ...
        if (linein_2_dac) {
            audio_sound_dac_init();
            struct dac_param linein_dac = {0};
            linein_dac.sample_rate = sr;
            linein_dac.delay_ms = 50;
            demo_dac = audio_sound_dac_open(&linein_dac);
            audio_sound_dac_start(demo_dac);
            audio_sound_dac_set_analog_gain(demo_dac, 0xF, 3);
            audio_sound_dac_set_digital_gain(demo_dac, 0xF, 16384);
        }
