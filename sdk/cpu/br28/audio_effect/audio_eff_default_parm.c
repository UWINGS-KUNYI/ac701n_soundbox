...
const struct eq_seg_info mic_dut_eq_tab[] = {
...
#else
u16 music_mode_seq[] = {music_mode_seq0};//音乐模式的标号
#endif
...
void music_harmonic_exciter_parm_default_init()
{
#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
    music_exciter_parm.is_bypass = 0;
    music_exciter_parm.parm.flow_cut = 100; //0-20000,   默认值 100
    music_exciter_parm.parm.fhigh_cut = 20000;//0-20000,    默认值20000
    music_exciter_parm.parm.wetgain = 50;//0-200,       默认值 5
    music_exciter_parm.parm.drygain = 100;//0-100,       默认值 100
    music_exciter_parm.parm.excitType = EX_LEVELSOFT;// enum值（3选1）   默认值  EX_LEVELSOFT_Tool
#endif
}
void music_harmonic_exciter_file_analyze_init()
{
#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
    void *tar_buf = (void *)&music_exciter_parm;
    u16 tar_len = sizeof(music_exciter_parm);
    eff_file_analyze(music_mode_seq[0], EFF_MUSIC_HARMONIC_EXCITER, tar_buf, tar_len);
#endif
}
...
void mic_harmonic_exciter_parm_default_init()
{
#if (defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        struct eff_parm  *mic_eff = &eff_mode[index];
#if defined(TCFG_MIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MIC_HARMONIC_EXCITER_ENABLE
        mic_eff->exciter_parm.is_bypass = 0;
        mic_eff->exciter_parm.parm.flow_cut = 100; //0-20000,   默认值 100
        mic_eff->exciter_parm.parm.fhigh_cut = 20000;//0-20000,    默认值20000
        mic_eff->exciter_parm.parm.wetgain = 50;//0-200,       默认值 5
        mic_eff->exciter_parm.parm.drygain = 100;//0-100,       默认值 100
        mic_eff->exciter_parm.parm.excitType = EX_LEVELSOFT;// enum值（3选1）   默认值  EX_LEVELSOFT_Tool
#endif
    }
#endif

}
void mic_harmonic_exciter_file_analyze_init()
{
#if (defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        struct eff_parm  *mic_eff = &eff_mode[index];
#if defined(TCFG_MIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MIC_HARMONIC_EXCITER_ENABLE
        void *tar_buf = (void *)&mic_eff->exciter_parm;
        u16 tar_len = sizeof(mic_eff->exciter_parm);
        eff_file_analyze(eff_mode_seq[index], EFF_MIC_HARMONIC_EXCITER, tar_buf, tar_len);
#endif
    }
#endif
...
void mic_dut_eq_parm_default_init()
{
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE&& TCFG_AUDIO_MIC_DUT_ENABLE
    mic_dut_eq.global_gain = 0;
    mic_dut_eq.seg_num = ARRAY_SIZE(mic_dut_eq_tab);
    memcpy(mic_dut_eq.seg, mic_dut_eq_tab, sizeof(mic_dut_eq_tab));
#endif
}
void mic_dut_eq_file_analyze_init()
{
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE&& TCFG_AUDIO_MIC_DUT_ENABLE
    u16 mode_seq = mic_dut_mode_seq;
    u16 group_id = MIC_DUT_EQ;
    void *tar_buf = &mic_dut_eq;
    u16 tar_len = sizeof(mic_dut_eq);
    eff_file_analyze(mode_seq, group_id, tar_buf, tar_len);
#endif
}
...
void music_eq2_file_analyze_init()
{
    ...
    for (int tar = 0; tar < calc; tar++) {
        ...
        if (!eff_file_analyze(music_mode_seq[tar], group_id, tar_buf, tar_len)) {
            int index = get_index_by_group_id(group_id, tar);
            if (index >= 0) {
                struct music_eq2_tool *parm = (struct music_eq2_tool *)tar_buf;
                set_list_nsection(tar, index, parm->seg_num);//根据文件段数，设定在线调试段数
            }
...
void mic_wdrc4_file_analyze_init()
{
    ...
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        ...
        int i = 4;
        u16 group_id = EFF_MIC_LAST_WHOLE_DRC;
        void *tar_buf = (void *)&mic_eff->drc_parm[i];
        u16 tar_len = sizeof(mic_eff->drc_parm[i]);
        eff_file_analyze(eff_mode_seq[index], group_id, tar_buf, tar_len);
        wdrc_printf(tar_buf);
...
void dynamic_eq_parm_default_init()
{
    ...
    for (u8 tar = 0; tar < calc; tar++) {
        ...
        dynamic_eq[tar].is_bypass |= BIT(5);
        dynamic_eq[tar].nSection = 2;
...
void dynamic_eq_pro_parm_default_init()
{
#if TCFG_DYNAMIC_EQ_PRO_ENABLE
    u8 calc = mode_add;
    for (u8 tar = 0; tar < calc; tar++) {
        //dynamic eq
        dynamic_eq_pro[tar].is_bypass = 0;
        dynamic_eq_pro[tar].nSection = 1;
        for (u8 i = 0; i < dynamic_eq_pro[tar].nSection; i++) {
            dynamic_eq_pro[tar].effectParam[i].fc = 1000;
            dynamic_eq_pro[tar].effectParam[i].Q = 0.7f;
            dynamic_eq_pro[tar].effectParam[i].gain = 0.0f;
            dynamic_eq_pro[tar].effectParam[i].attackTime = 5;
            dynamic_eq_pro[tar].effectParam[i].releaseTime = 300;
            dynamic_eq_pro[tar].effectParam[i].low_thr = -10;
            dynamic_eq_pro[tar].effectParam[i].high_thr = 0;
            dynamic_eq_pro[tar].effectParam[i].type = 0x2;//peak
            dynamic_eq_pro[tar].effectParam[i].Enable = 1;
        }
    }
#endif
}
void dynamic_eq_pro_file_analyze_init()
{
#if TCFG_DYNAMIC_EQ_PRO_ENABLE
    u8 calc = mode_add;
    for (u8 tar = 0; tar < calc; tar++) {
        //dynamic eq
        u16 group_id = EFF_MUSIC_DYNAMIC_EQ_PRO;
        void *tar_buf = &dynamic_eq_pro[tar];
        u16 tar_len = sizeof(DynamicEQProParam_TOOL_SET);
        eff_file_analyze(music_mode_seq[tar], group_id, tar_buf, tar_len);
        /* dynamic_eq_printf(tar_buf); */
    }
#endif
...
{
#if ((defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)) && BASS_AND_TREBLE_ENABLE
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        struct eff_parm  *mic_eff = &eff_mode[index];
        //low
...
void adc_pga_file_analyze_init()
{
    ...
    eff_file_analyze(eff_mode_seq[0], EFF_ADC_PGA, tar_buf, tar_len);
...
void mic_low_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
    u16 i = 0;
    u16 group_id = EFF_MIC_LAST_LOW_DRC;
    u32 drc_name = get_module_name_and_index(group_id, &i, nor_list_label);

    struct threshold_group group[] = {{0, 0}, {50, 50}, {90, 90}};
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        mic_last_drc_parm[index].wdrc_parm[i].is_bypass = 0;
        mic_last_drc_parm[index].wdrc_parm[i].parm.attacktime = 10;
        mic_last_drc_parm[index].wdrc_parm[i].parm.releasetime = 300;
        mic_last_drc_parm[index].wdrc_parm[i].parm.inputgain = 0;
        mic_last_drc_parm[index].wdrc_parm[i].parm.outputgain = 0;
        mic_last_drc_parm[index].wdrc_parm[i].parm.threshold_num = ARRAY_SIZE(group);
        memcpy(mic_last_drc_parm[index].wdrc_parm[i].parm.threshold, group, sizeof(group));
        mic_last_drc_parm[index].wdrc_parm[i].parm.rms_time = 25;
        mic_last_drc_parm[index].wdrc_parm[i].parm.algorithm = 0;
        mic_last_drc_parm[index].wdrc_parm[i].parm.mode = 1;
    }
#endif
}

void mic_low_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
    u16 i = 0;
    u16 group_id = EFF_MIC_LAST_LOW_DRC;
    u32 drc_name = get_module_name_and_index(group_id, &i, nor_list_label);
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        void *tar_buf = &mic_last_drc_parm[index].wdrc_parm[i];
        u16 tar_len = sizeof(mic_last_drc_parm[index].wdrc_parm[i]);
        eff_file_analyze(eff_mode_seq[index], group_id, tar_buf, tar_len);
        wdrc_printf(tar_buf);
    }
#endif
...
void mic_mid_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
    u16 group_id = EFF_MIC_LAST_MID_DRC;
    u32 drc_name = get_module_name_and_index(group_id, &i, nor_list_label);
...
void mic_mid_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
...
void mic_high_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
    u16 group_id = EFF_MIC_LAST_HIGH_DRC;
    u32 drc_name = get_module_name_and_index(group_id, &i, nor_list_label);
...
void mic_high_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
...
void mic_whole_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
...
void mic_whole_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 i = 0;
...
void mic_crossover_wdrc_parm_default_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        mic_last_drc_parm[index].crossover.way_num = 3;
        mic_last_drc_parm[index].crossover.N = 2;
        mic_last_drc_parm[index].crossover.low_freq = 200;
        mic_last_drc_parm[index].crossover.high_freq = 2000;
    }
#endif
#endif
}

void mic_crossover_wdrc_file_analyze_init()
{
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
    u16 drc_name = AEID_MIC_DRC4;
    struct mode_list *list = get_group_list(drc_name);
    if (list) {
        for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
            u16 group_id = list->group_id[4];
            void *tar_buf = &mic_last_drc_parm[index].crossover;
            u16 tar_len = sizeof(mic_last_drc_parm[index].crossover);
            eff_file_analyze(eff_mode_seq[index], group_id, tar_buf, tar_len);
#if PARM_DEBUG
            CrossOverParam_TOOL_SET *parm = tar_buf;
            log_debug("way_num %d, N %d, low_freq %d, high_freq %d\n", parm->way_num, parm->N, parm->low_freq, parm->high_freq);
