...
#define EQ_ASYNC_EN 0  //同步
#else
#define EQ_ASYNC_EN 1  //异步
#endif
...
struct eq_tool_mic_dut mic_dut_eq; //mic频响eq
#endif
#if 0
//系数切换
void eq_sw_demo()
...
#if (defined(EQ_CORE_V1) && defined(AUDIO_VBASS_32BIT_OUT_EN) && AUDIO_VBASS_32BIT_OUT_EN)
#if (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_16BIT_MODE) && SOUND_TRACK_1_P_1_CH_CONFIG && (EFFECT_PROCESS_POITION_CONFIG == EFFECT_BEFORE_STEREO_MIX)
    //如果使用16bit位宽、1.1声道、音效在声道合并之前，那么这个eq使用16进32出
...
#if (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_16BIT_MODE) && SOUND_TRACK_1_P_1_CH_CONFIG
    //如果使用16bit位宽、1.1声道、音效在声道合并之前，那么EQ2会作为分流前preEQ，使用16进16出
    parm.out_32bit = 0;//16bit位宽输出
#else
    struct eq_parm eparm = {0};
    eparm.in_mode = DATI_INT;
    eparm.out_mode = DATO_INT;
...
struct audio_eq *music_ext_eq_open(u32 sample_rate, u8 ch_num)
{
    ...
    struct eq_parm eparm = {0};
    if (global_bit_wide || SOUND_TRACK_1_P_1_CH_CONFIG) {
        eparm.in_mode = DATI_INT;
        eparm.out_mode = DATO_INT;
        eparm.run_mode = NORMAL;
        eparm.data_in_mode = SEQUENCE_DAT_IN;
