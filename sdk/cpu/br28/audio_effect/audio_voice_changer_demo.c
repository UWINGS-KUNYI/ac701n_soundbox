...
static VOICE_CHANGER_MODE voice_changer_mode_o = 0;
...
voice_changer_hdl *audio_voice_changer_open_demo(u32 voice_changer_name, u32 sample_rate)
{
    ...
    if (voice_changer_name == AEID_MIC_VOICE_CHANGER) {
        ...
        memcpy(&parm, &eff_mode[mode].voicechanger_parm.parm, sizeof(struct voice_changer_udapte_parm));
        bypass = eff_mode[mode].voicechanger_parm.is_bypass;
#endif
#endif
    } else if (voice_changer_name == AEID_MUSIC_VOICE_CHANGER) {
        memcpy(&parm, &music_voice_changer.parm, sizeof(struct voice_changer_udapte_parm));
...
#if 0
    int ret = syscfg_read(CFG_VOICE_CHANGER_MODE, &voice_changer_mode_o, sizeof(voice_changer_mode_o));
    if (ret < 0) {
        voice_changer_mode_o  = VOICE_CHANGER_GODDESS;
    }
    audio_voice_changer_mode_switch(voice_changer_name, voice_changer_mode_o);
#endif
    return hdl;
}
