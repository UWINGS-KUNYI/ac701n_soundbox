...
const u8 eff_sdk_name[16] 		= "JL701N";
#if defined(TCFG_AUDIO_DAC_BIT_WIDTH) && (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)
const u8 eff_eq_ver[4] 			= {1, 0, 0, 3};
const u8 eff_eq_ver_old[4] 			= {1, 0, 0, 1};
#else
const u8 eff_eq_ver[4] 			= {1, 0, 0, 2};
const u8 eff_eq_ver_old[4] 			= {1, 0, 0, 0};
#endif
int eff_old_ver_ok = 0;


#define MUSIC_EQ_NSECTION_SPC  music_eq_nsection

/*drc后扩展eq支持的在线调音段数*/
#define EXT_EQ_NSECTION             (10+EXT_HP_LP_SECTIONS)
/*高低音eq支持在线调音的段数*/
#define BASS_AND_TREBLE_EQ_NSECTION (10+EXT_HP_LP_SECTIONS)
/*动态EQ模块前eq支持的在线调音段数*/
#define MUSIC_EQ2_NSECTION          (10+EXT_HP_LP_SECTIONS)
...
struct mode_list mlist[] = {
...
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE&& TCFG_AUDIO_MIC_DUT_ENABLE
    {AEID_MIC_DUT_EQ,    10,              1,    {MIC_DUT_EQ}},
#endif
    ...
    {AEID_MIC_DRC4,     0, 5,    {EFF_MIC_LAST_LOW_DRC, EFF_MIC_LAST_MID_DRC, EFF_MIC_LAST_HIGH_DRC, EFF_MIC_LAST_WHOLE_DRC, EFF_MIC_LAST_CROSSOVER }},  //最后一个group_id存方分频器
#else
    {AEID_MIC_DRC4,      0, 1,    {EFF_MIC_LAST_WHOLE_DRC}},
#endif
#endif/*TCFG_MIC_EFFECT_ENABLE*/
...
int music_harmonic_exciter_parm_ananlyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
    ...
    harmonic_exciter_param_tool_set *parm = &music_exciter_parm;
    memcpy(parm, &packet->data[1], sizeof(music_exciter_parm));
    audio_harmonic_exciter_update_parm_api(AEID_MUSIC_HARMONIC_EXCITER, &parm->parm, parm->is_bypass);
#if PARM_DEBUG

    log_debug("cfg.is_bypass %d, flow_cut %d, fhigh_cut:%d, wetgain %d drygain %d, excitType %d\n",
...
int mic_voice_changer_parm_ananlyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
    ...
    audio_voice_changer_update_demo(AEID_MIC_VOICE_CHANGER, (VOICECHANGER_PARM *)&changer_parm->parm, changer_parm->is_bypass);
#if PARM_DEBUG
...
int mic_harmonic_exciter_parm_ananlyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if (defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)
#if defined(TCFG_MIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MIC_HARMONIC_EXCITER_ENABLE
    u8 mode_id = packet->data[0];
    harmonic_exciter_param_tool_set *parm = &eff_mode[mode_id - mic_mode_seq0].exciter_parm;
    memcpy(parm, &packet->data[1], sizeof(eff_mode[mode_id - mic_mode_seq0].exciter_parm));
    audio_harmonic_exciter_update_parm_api(AEID_MIC_HARMONIC_EXCITER, &parm->parm, parm->is_bypass);
#if PARM_DEBUG
    log_debug("cfg.is_bypass %d, flow_cut %d, fhigh_cut:%d, wetgain %d drygain %d, excitType %d\n",
              parm->is_bypass, parm->parm.flow_cut, parm->parm.fhigh_cut, parm->parm.wetgain, parm->parm.drygain, parm->parm.excitType);
#endif
...
{
#if PARM_DEBUG
    log_debug("dy_parm->is_bypass %x\n", dy_parm->is_bypass);

    for (int i = 0; i < dy_parm->nSection; i++) {
        log_debug("fc %d , Q 0x%x, gain:0x%x, type %d, attacktime %d, releaseTime %d, rmsTime %d,rmsTime 0x%x, threshold 0x%x, ratio 0x%x, noisegate_threshold 0x%x, fixGain %x, algorithm %d\n",
                  ...
                  *(int *)&dy_parm->effect_param[i].gain,
                  ...
                  (int)dy_parm->effect_param[i].rmsTime,
                  *(int *)&dy_parm->effect_param[i].rmsTime,
...
#else
    audio_dynamic_eq_pro_update_parm_api(dy_eq_name, &dynamic_eq_pro[tar]);
#endif
#endif
}
...
int dynamic_eq_pro_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if TCFG_DYNAMIC_EQ_PRO_ENABLE
    u8 mode_id = packet->data[0];
    u8 tar = 0;
    u32 dy_eq_name = AEID_MUSIC_DYNAMIC_EQ;
#if defined(LINEIN_MODE_SOLE_EQ_EN) && LINEIN_MODE_SOLE_EQ_EN
    if (linein_mode_seq == mode_id) {
        tar = 1;
        dy_eq_name = AEID_AUX_DYNAMIC_EQ;
        if (!get_cur_dynamic_eq_pro_hdl_by_name(dy_eq_name)) {
            memcpy(&dynamic_eq_pro[tar], &packet->data[1], sizeof(DynamicEQProParam_TOOL_SET));
            return 0;
        }
    } else
#endif
...
int mic_dut_eq_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE&& TCFG_AUDIO_MIC_DUT_ENABLE
    u8 tar = 0;
    ...
    void *tar_seg = NULL;
    if (seg->index == (u16) - 1) {
        mic_dut_eq.global_gain = global_gain;
    } else {
        tar_seg = &mic_dut_eq.seg[seg->index];
    }
    if (seg->index != (u16) - 1) {
#if PARM_DEBUG
        log_info("idx:%d, iir:%d, frq:%d, gain:0x%x, q:0x%x \n", seg->index, seg->iir_type, seg->freq, *(int *)&seg->gain, *(int *)&seg->q);
...
#if PARM_DEBUG
        log_info("idx:%d, iir:%d, frq:%d, gain:0x%x,gain:%d q:0x%x \n", seg->index, seg->iir_type, seg->freq, *(int *)&seg->gain, (int)seg->gain, *(int *)&seg->q);
#endif
        memcpy(tar_seg, seg, sizeof(struct eq_seg_info));
        if (update) {
            if (right_ch) {
...
int mic_wdrc4_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if (defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)
    u16 type = 0; //0:low  1:mid  2:high ,3:other band, 4:crossover
    u32 drc_name = get_module_name_and_index(packet->cmd, &type, 0);
    ...
    log_debug("wdrc_parm type %d\n", type);
    u8 mode_id = packet->data[0];
    tar_wdrc = &eff_mode[mode_id - mic_mode_seq0].drc_parm[4]; //drc4
    memcpy(tar_wdrc, wdrc_parm, sizeof(wdrc_struct_TOOL_SET));
...
int mic_nband_wdrc_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if ((TCFG_MIC_EFFECT_SEL == MIC_EFFECT_REVERB_ECHO_ADVANCE) || TCFG_MIC_EFFECT_MDRC)
#if defined(TCFG_DRC_ENABLE)&&TCFG_DRC_ENABLE
    u16 type = 0; //0:low  1:mid  2:high ,3:other band, 4:crossover
    ...
    log_debug("mic wdrc_parm type %d\n", type);
    if (type == (list->group_num - 1)) { ////最后一个存分频器系数
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
        CrossOverParam_TOOL_SET *parm = (CrossOverParam_TOOL_SET *)&packet->data[1];
        tar_wdrc = &mic_last_drc_parm[mode_id - mic_mode_seq0].crossover;
        memcpy(tar_wdrc, parm, sizeof(CrossOverParam_TOOL_SET));
        if (update) {
            cur_crossover_set_update(drc_name, tar_wdrc);
        }
        ...
        tar_wdrc = &mic_last_drc_parm[mode_id - mic_mode_seq0].wdrc_parm[type];
    }
...
#if PARM_DEBUG
    struct NotchHowling_update_Param *parm = &parmt->parm;
...
#if PARM_DEBUG
    struct  plate_reverb_update_parm *parm = &parmt->parm;
...
#if PARM_DEBUG
    struct echo_update_parm *parm = &parmt->parm;
...
int eff_file_analyze(u32 mode_id, u16 group_id, void *data_buf, u32 buf_len)
{
    ...
    eff_old_ver_ok = 0;
    if (!memcmp(ver, eff_eq_ver_old, sizeof(eff_eq_ver_old))) {
        eff_old_ver_ok = 1;
    }
    if (memcmp(ver, eff_eq_ver, sizeof(eff_eq_ver))) {
        if (!eff_old_ver_ok) {
#if defined(TCFG_AUDIO_DAC_BIT_WIDTH) && (TCFG_AUDIO_DAC_BIT_WIDTH == DAC_24BIT_MODE)
            log_error("dac is 24bit, not support old version bin\n");
#endif
...
void wdrc_printf(void *_wdrc)
{
    ...
    printf("wdrc attacktime %d, releasetime %d threshold_num %d rms_time %d , rms_time %x, algorithm%d, mode%d ,0x%x,0x%x\n",
           wdrc->attacktime, wdrc->releasetime, wdrc->threshold_num, wdrc->rms_time, *(int *)&wdrc->threshold[5].in_threshold, wdrc->algorithm, wdrc->mode, *(int *)&wdrc->inputgain, *(int *)&wdrc->outputgain);
...
#if TCFG_DYNAMIC_EQ_PRO_ENABLE
REGISTER_CMD_TARGET(dyeqpro) = {//动态eq
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MUSIC_DYNAMIC_EQ_PRO,
    .cmd_deal = dynamic_eq_pro_parm_analyze,
#endif
    .eff_default_parm_init = dynamic_eq_pro_parm_default_init,
    .eff_file_analyze_init = dynamic_eq_pro_file_analyze_init,
};
#endif
...
#if ((TCFG_MIC_EFFECT_SEL == MIC_EFFECT_REVERB_ECHO_ADVANCE) || TCFG_MIC_EFFECT_MDRC)
REGISTER_CMD_TARGET(mic_ml_drc) = {//
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_LOW_DRC,
    .cmd_deal = mic_nband_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = mic_low_wdrc_parm_default_init,
    .eff_file_analyze_init = mic_low_wdrc_file_analyze_init,

};
#if defined(MULTI_BAND_DRC) && MULTI_BAND_DRC
REGISTER_CMD_TARGET(mic_mm_drc) = {
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_MID_DRC,
    .cmd_deal = mic_nband_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = mic_mid_wdrc_parm_default_init,
    .eff_file_analyze_init = mic_mid_wdrc_file_analyze_init,

};
REGISTER_CMD_TARGET(mic_mh_drc) = {//
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_HIGH_DRC,
    .cmd_deal = mic_nband_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = mic_high_wdrc_parm_default_init,
    .eff_file_analyze_init = mic_high_wdrc_file_analyze_init,

};
REGISTER_CMD_TARGET(mic_mw_drc) = {//全带drc
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_WHOLE_DRC,
    .cmd_deal = mic_nband_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = mic_whole_wdrc_parm_default_init,
    .eff_file_analyze_init = mic_whole_wdrc_file_analyze_init,
};
#endif
REGISTER_CMD_TARGET(mic_last_cross) = {//
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_CROSSOVER,
    .cmd_deal = mic_nband_wdrc_parm_analyze,
#endif
    .eff_default_parm_init = mic_crossover_wdrc_parm_default_init,
    .eff_file_analyze_init = mic_crossover_wdrc_file_analyze_init,
};
#else
//drc4与last_whole_drc共用一个组号
REGISTER_CMD_TARGET(micDrc4) = {//mic_drc4
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_LAST_WHOLE_DRC,
    .cmd_deal = mic_wdrc4_parm_analyze,
#endif
...
#if defined(TCFG_EQ_ENABLE) && TCFG_EQ_ENABLE && TCFG_AUDIO_MIC_DUT_ENABLE
REGISTER_CMD_TARGET(mic_dut_Eq) = {//mic频响测试
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = MIC_DUT_EQ,
    .cmd_deal = mic_dut_eq_parm_analyze,
#endif
    .eff_default_parm_init = mic_dut_eq_parm_default_init,
    .eff_file_analyze_init = mic_dut_eq_file_analyze_init,
...
REGISTER_CMD_TARGET(music_harmonic_exciter) = {
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MUSIC_HARMONIC_EXCITER,
    .cmd_deal = music_harmonic_exciter_parm_ananlyze,
#endif
    .eff_default_parm_init = music_harmonic_exciter_parm_default_init,
    .eff_file_analyze_init = music_harmonic_exciter_file_analyze_init,
};
...
REGISTER_CMD_TARGET(mic_harmonic_exciter) = {
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_HARMONIC_EXCITER,
    .cmd_deal = mic_harmonic_exciter_parm_ananlyze,
#endif
    .eff_default_parm_init = mic_harmonic_exciter_parm_default_init,
    .eff_file_analyze_init = mic_harmonic_exciter_file_analyze_init,
};
...
int eff_tool_sys_info(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if TCFG_EFFECT_TOOL_ENABLE
    struct sys_info {
        u32 cpu_num;		// CPU核数
        u32 use_mem;
        u32 total_mem;		// 内存相关
        u32 cpu0_use_ratio;	// CPU0占用
        u32 cpu1_use_ratio;	// CPU1占用
    };
    struct sys_info sys = {0};
    sys.cpu_num = CPU_CORE_NUM;
    sys.use_mem = memory_get_size(P_MEMORY_USED);
    sys.total_mem = memory_get_size(P_MEMORY_TOTAL);
    int cpu_usage[2] = {0};
    os_cpu_usage(NULL, cpu_usage);
    sys.cpu0_use_ratio = (cpu_usage[0] >= 100) ? 0 : cpu_usage[0];
    sys.cpu1_use_ratio = (cpu_usage[1] >= 100) ? 0 : cpu_usage[1];
    eff_send_packet(NULL, REPLY_TO_TOOL, (u8 *)&sys, sizeof(sys), sq);
    os_system_info_reset();
#endif
    return 1;
}

REGISTER_CMD_TARGET(sys_inf) = {
    .cmd = EFF_CMD_SYS_INFO,
    .cmd_deal = eff_tool_sys_info,
};
...
void eff_module_update()
{
    ...
    audio_vbass_update_demo(AEID_MUSIC_VBASS, &vbass_parm[0].parm, vbass_parm[0].is_bypass);
#endif
    ...
    audio_vbass_update_demo(AEID_AUX_VBASS, &vbass_parm[1].parm, vbass_parm[1].is_bypass);
#endif
