...
extern int eff_old_ver_ok;
#if defined(TCFG_DYNAMIC_EQ_ENABLE) && TCFG_DYNAMIC_EQ_ENABLE && defined(TCFG_DYNAMIC_EQ_PRO_ENABLE) && !TCFG_DYNAMIC_EQ_PRO_ENABLE
...
struct dynamic_eq *audio_dynamic_eq_open_demo(u32 dynamic_eq_name, u32 sample_rate, u8 channel)
{
    ...
    param.nSection = dynamic_eq[tar].nSection;
    param.SampleRate = sample_rate;
    param.DetectdataInc = (channel == 1) ? 1 : 2;
    param.DetectdataBit = DATA_INT_32BIT;//只支持32
    param.pcm_info.IndataBit =  DATA_INT_32BIT;
    param.pcm_info.OutdataBit = DATA_INT_32BIT;
    param.pcm_info.IndataInc = (channel == 1) ? 1 : 2;
    param.pcm_info.OutdataInc = (channel == 1) ? 1 : 2;
    param.pcm_info.Qval = global_bit_wide ? 23 : 15;
    for (int i = 0; i < param.nSection; i++) {
        if (!(dynamic_eq[tar].is_bypass & BIT(5))) {//旧版的效果配置需要转换
            int rms_time = 0;
            memcpy(&rms_time, &(dynamic_eq[tar].effect_param[i].rmsTime), 4);
            dynamic_eq[tar].effect_param[i].rmsTime = (float)rms_time;
            //printf("========rms_time %d\n", rms_time);
        }
    }
    ...
    if (hdl) {
#if defined(TCFG_DYNAMIC_EQ_SPILT_ENABLE)&&TCFG_DYNAMIC_EQ_SPILT_ENABLE
        struct dynamic_eq_fixparm fixparm = {0};
        fixparm.spilt = 1;
        fixparm.start_point = 1;
        dynamic_eq_set_info(hdl, &fixparm);
#endif
        dynamic_eq_bypass(hdl, dynamic_eq[tar].is_bypass);
    }
    return hdl;
}
...
void audio_dynamic_eq_update_parm(u32 dynamic_eq_name, void *parm, void *parm2, int bypass)
{
    ...
    if (hdl) {
        DynamicEQEffectParam *effect_param = (DynamicEQEffectParam *)parm;
        DynamicEQParam *param = parm2;
        for (int i = 0; i < param->nSection; i++) {//旧版的效果配置需要转换
            if (!(bypass & BIT(5))) {
                int rms_time = 0;
                memcpy(&rms_time, &(effect_param[i].rmsTime), 4);
                effect_param[i].rmsTime = (float)rms_time;
                //printf("========rms_time %d\n", rms_time);
            }
        }
        ...
        param->Enable[0] = (bypass & BIT(1)) ? 0 : 1;
        param->Enable[1] = (bypass & BIT(2)) ? 0 : 1;
        param->Enable[2] = (bypass & BIT(3)) ? 0 : 1;
        param->Enable[3] = (bypass & BIT(4)) ? 0 : 1;
        dynamic_eq_update(hdl, parm, param);
...
struct dynamic_eq_detection *audio_dynamic_eq_detection_open_demo(u32 dynamic_eq_name, u32 sample_rate, u8 channel)
{
    ...
    /* dynamic_eq[tar].nSection =  DYNAMIC_EQ_MAX_NSECTION; */
    ...
    if (hdl) {
        dynamic_eq_detection_bypass(hdl, dynamic_eq[tar].is_bypass);
    }
    return hdl;
}
