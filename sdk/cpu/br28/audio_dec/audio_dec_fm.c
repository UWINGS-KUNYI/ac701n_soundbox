...
#else
    dec->dy_eq_pro = audio_dynamic_eq_ctrl_open(AEID_MUSIC_DYNAMIC_EQ, dec->pcm_dec.sample_rate, dec->effect_ch_num);//动态eq
#endif
...
static void __fm_dec_close(void)
{
    if (fm_dec && fm_dec->start) {
...
#if AUDIO_VOCAL_REMOVE_EN
        set_vocal_remove_hdl(0);
        audio_vocal_remove_close(fm_dec->vocal_hdl);
        channel_switch_close(&fm_dec->vocal_ch_switch);
#endif
#if AUDIO_SURROUND_CONFIG
        ...
        convet_data_close(fm_dec->convert_16_to_32);
        convet_data_close(fm_dec->convert_32_to_16);
#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
        audio_harmonic_exciter_close_api(fm_dec->harmonic_exciter);
#endif
...
#else
        audio_dynamic_eq_pro_close_api(fm_dec->dy_eq_pro);
#endif
...
#endif
#if SOUND_TRACK_1_P_1_CH_CONFIG && (EFFECT_PROCESS_POITION_CONFIG == EFFECT_BEFORE_STEREO_MIX)
        channel_switch_close(&fm_dec->fl_fr_ch_2to1);
        channel_switch_close(&fm_dec->rl_rr_ch_2to1);
#endif
...
#if (RECORDER_MIX_EN)
        if (fm_dec->rec_mix_ch != NULL) {
            void *entry = rec_mix_fifo_ch_get_entry(fm_dec->rec_mix_ch);
            if (entry != NULL) {
                audio_stream_del_entry(entry);
            }
        }
        rec_mix_fifo_ch_close(fm_dec->rec_mix_ch);
        fm_dec->rec_mix_ch = NULL;
#endif
...
#if TCFG_EQ_DIVIDE_ENABLE
        if (fm_dec->vocal_tract.stream) {
            audio_stream_close(fm_dec->vocal_tract.stream);
            fm_dec->vocal_tract.stream = NULL;
        }
#endif
