...
#endif
#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
    dec->harmonic_exciter = audio_harmonic_exciter_open_api(AEID_MUSIC_HARMONIC_EXCITER, sample_rate, ch_num);
#endif
...
#else
    dec->dy_eq_pro = audio_dynamic_eq_pro_open(AEID_MUSIC_DYNAMIC_EQ, sample_rate, ch_num);//动态eq
#endif
...
int spdif_dec_stop(void)
{
    if (spdif_dec_hdl) {
        ...
        audio_dynamic_eq_ctrl_close(dec->dy_eq);
#else
        audio_dynamic_eq_pro_close(dec->dy_eq_pro);
#endif
        music_last_drc_close(dec->last_drc);
        convet_data_close(dec->convert2);
#endif
