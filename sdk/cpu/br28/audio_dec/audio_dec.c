...
#if TCFG_HOST_AUDIO_ENABLE
void *usb_master_digvol_last = NULL;
void *usb_master_digvol_last_entry = NULL;
#endif
...
#if defined(TCFG_AUDIO_DAC_BIT_WIDTH) && TCFG_AUDIO_DAC_BIT_WIDTH
#define AUDIO_MIXER_LEN				(256 * 4 * 2)
#else
        ...
        struct audio_data_frame *out)
{
    ...
    if (in->data_len - in->offset > 0) {
        if (global_bit_wide) {
            digital_phase_inverter_s24((s32 *)((int)in->data + in->offset), in->data_len - in->offset);
        } else {
            digital_phase_inverter_s16((s16 *)((int)in->data + in->offset), in->data_len - in->offset);
        }
    }
    return in->data_len;
}
        ...
        u16 output_sample_rate)
{
    ...
    if (hdl) {
        if (global_bit_wide) {
            audio_hw_src_open(hdl, channel, SET_RESAMPLE_TYPE(AUDIO_SAMPLE_FMT_24BIT, SRC_TYPE_RESAMPLE));
        } else {
...
{
    //y_printf(">>Audio_Config_Trace:");
    //audio_gain_dump();
    //audio_adda_dump();
    mem_stats();

#if 1
    int clk_freq = clk_get("sys") / 1000000;
    printf("clk_freq:%d(MHz)\n", clk_freq);
#endif
...
int audio_dec_init()
{
    ...
    audio_mixer_set_bit_wide(&mixer, global_bit_wide);
    // 使能mixer事件回调
...
#define MIXER_MIN_LEN		(128*4*2)
...
#if (RECORDER_MIX_EN)
    rec_mix_fifo_init(RECORDER_MIX_CHANNEL, RECORDER_MIX_SAMPLERATE);
#endif

#if (SOUNDCARD_ENABLE || MULTI_AUDIO_UPLOAD_TO_UAC_ENABLE)
    usb_mix_fifo = mix_fifo_init(1, 48000);
#endif
    ...
    if (ch_num != 4) {
        ...
        parm.bit_wide = global_bit_wide;
        parm.eq_name = AEID_EQUAL_LOUDNEDD_EQ;
...
#if TCFG_HOST_AUDIO_ENABLE
    //解决当输出方式不为dac时，USB host 输出异常
#if (!(AUDIO_OUT_WAY_TYPE & AUDIO_WAY_TYPE_DAC))
    entry_cnt--;
#endif
    audio_dig_vol_param usb_master_digvol_last_param = {
        .vol_start = app_var.music_volume,
        .vol_max = SYS_MAX_VOL,
        .ch_total = 2,
        .fade_en = 1,
    ...
    usb_master_digvol_last = audio_dig_vol_open(&usb_master_digvol_last_param);
    usb_master_digvol_last_entry = audio_dig_vol_entry_get(usb_master_digvol_last);
    audio_usb_host_spk_stream_init(usb_master_digvol_last);
    struct audio_stream_entry *usb_master_last_entry = NULL;
    usb_master_last_entry = get_usb_host_spk_stream_entry();

    struct audio_stream_entry *usb_master_entries_start = entries[entry_cnt - 2];
    entry_cnt = 0;
    entries[entry_cnt++] = usb_master_entries_start;
    entries[entry_cnt++] = usb_master_digvol_last_entry;
    entries[entry_cnt++] = usb_master_last_entry;
    for (int i = 0; i < entry_cnt - 1; i++) {
        audio_stream_add_entry(entries[i], entries[i + 1]);
    }
#endif
...
#if defined(DAC_AUTO_HIGH_Z_EN) && DAC_AUTO_HIGH_Z_EN
...
{
#if (SOUNDCARD_ENABLE)
    extern u16 soundcard_vol_table[6];
    u16 start_dig_vol = 0;
    if (!strcmp(logo, "music_a2dp")) {
        start_dig_vol = soundcard_vol_table[APP_AUDIO_STATE_CH_A2DP % APP_AUDIO_STATE_MUSIC_CH_BASE];
    } else if (!strcmp(logo, "music_fm")) {
        start_dig_vol = soundcard_vol_table[APP_AUDIO_STATE_CH_FM % APP_AUDIO_STATE_MUSIC_CH_BASE];
    } else if (!strcmp(logo, "music_pc")) {
        start_dig_vol = soundcard_vol_table[APP_AUDIO_STATE_CH_PC % APP_AUDIO_STATE_MUSIC_CH_BASE];
    } else if (!strcmp(logo, "music_linein")) {
        start_dig_vol = soundcard_vol_table[APP_AUDIO_STATE_CH_LINEIN % APP_AUDIO_STATE_MUSIC_CH_BASE];
    } else if (!strcmp(logo, "music_file")) {
        start_dig_vol = soundcard_vol_table[APP_AUDIO_STATE_CH_FILE % APP_AUDIO_STATE_MUSIC_CH_BASE];
    } else {
        start_dig_vol = 16;
    }
    return start_dig_vol;
#endif
...
vocal_remove_hdl *g_vocal_hdl;
struct channel_switch *g_vocal_ch_switch;
void set_vocal_remove_hdl(void *hdl)
{
    g_vocal_hdl = hdl;
    printf("set_vocal_remove_hdl 0x%x", g_vocal_hdl);
}
void set_vocal_ch_switch_hdl(void *hdl)
{
    g_vocal_ch_switch = hdl;
}
/*----------------------------------------------------------------------------*/
/**@brief    人声消除打开例子
   @param    ch_num:通道个数，samplerate:采样率，remove_ratio:人声消除比例(%)，
             low_freq:截止低频，high_freq:截止高频
   @return   句柄
   @note
*/
/*----------------------------------------------------------------------------*/
void *vocal_remove_open(u8 ch_num, u32 samplerate, u8 remove_ratio, u16 low_freq, u16 high_freq)
{
    vocal_remove_hdl *hdl = NULL;
    vocal_remove_open_parm parm = {0};
    parm.channel = ch_num;
    parm.samplerate = samplerate;
    //LOW_CUT_ENABLE:使能低切；HIGH_CUT_ENABLE:使能高切；设为0 即去除人声消除内部高低切处理
    parm.freq_cut_enable = LOW_CUT_ENABLE;
    parm.remove_ratio = remove_ratio;
    parm.low_freq = low_freq;
    parm.high_freq = high_freq;
    parm.bit_wide = global_bit_wide;
    hdl = audio_vocal_remove_open(&parm);
    u8 sw = 1;
    int ret = syscfg_read(CFG_VOCAL_REMOVE_SW, &sw, sizeof(sw));
    if (ret < 0) {
        sw = 1;
    }
    printf(">>>>>>>>>> %s   0x%x onoff %d", __FUNCTION__, sw);
    hdl->vocal_remove_dis = sw;
    return hdl;
}
...
*/
/*----------------------------------------------------------------------------*/
int vocal_remove_switch(u8 sw)
{
    if (TCFG_USER_TWS_ENABLE && (tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED)) {
        if (!sw) {
            u8 ch_type = tws_vocal_remove_ch_type_get();
            channel_switch_set_out_ch_type(g_vocal_ch_switch, ch_type);
        }
    }
    printf("vocal_remove_switch 0x%x %d", g_vocal_hdl, sw);
    return audio_vocal_remove_sw(g_vocal_hdl, sw);
}

//关闭人声消除，且tws配对成功时，需要调用此函数获取ch_type，在chanel_switch中做声道类型转换。
u8 tws_vocal_remove_ch_type_get()
{
    u8 ch_type = audio_output_channel_type();
    if (TCFG_USER_TWS_ENABLE && (tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED)) {
