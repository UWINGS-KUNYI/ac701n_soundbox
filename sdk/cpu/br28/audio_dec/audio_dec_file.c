...
void *file_mix_fifo_ch = NULL;
#endif

struct file_audio_wait_capture {
    void *buf;
    int sample_rate;
    u8 nch;
    u16 timer;
...
enum {
    FILE_DEC_FILL_DATA = 1,
};


static struct file_audio_wait_capture *file_wait_capture = NULL;

#define SLIENCE_PERIOD_TIME          10

static OS_MUTEX file_audio_wait_capture_mutex;
void file_audio_wait_capture_close(void)
{
    if (!file_wait_capture) {
        return;
    }
    printf("---%s  0x%x  %d", __FUNCTION__, file_wait_capture, file_wait_capture->timer);
    file_wait_capture->break_while = 1;
    os_mutex_pend(&file_audio_wait_capture_mutex, 0);
    if (file_wait_capture->timer) {
        sys_timer_del(file_wait_capture->timer);
        file_wait_capture->timer = 0;
        if (file_wait_capture->buf) {
            free(file_wait_capture->buf);
            file_wait_capture->buf = NULL;
        }
        printf("enter audio_dec_file.c ,sys_timer_del%d\n", __LINE__);
        free(file_wait_capture);
        file_wait_capture = NULL;
    }
    task_kill("audio_fill_data");
    os_mutex_post(&file_audio_wait_capture_mutex);
}

void file_audio_wait_capture_timer(void)
{
    os_mutex_pend(&file_audio_wait_capture_mutex, 0);
    if (!file_wait_capture) {
        os_mutex_post(&file_audio_wait_capture_mutex);
    ...
    u16 len = (SLIENCE_PERIOD_TIME * file_wait_capture->sample_rate / 1000) * file_wait_capture->nch * 2;

    if (!file_wait_capture->buf) {
        file_wait_capture->buf = zalloc(len);
        if (!file_wait_capture->buf) {
            os_mutex_post(&file_audio_wait_capture_mutex);
            return;
        }
    ...
    struct audio_frame frame = {
        .mode = AUDIO_FRAME_FILE_STREAM,
        .nch = file_wait_capture->nch,
        .sample_rate = file_wait_capture->sample_rate,
        .len = len,
        .data = file_wait_capture->buf,
        .offset = 0,
    };
    if (file_wait_capture->write_frame) {
        int wlen = 0;
        int remain_len = len;
        do {
            if (file_wait_capture->break_while) {
                os_mutex_post(&file_audio_wait_capture_mutex);
                return;
            }
            wlen = file_wait_capture->write_frame(file_wait_capture->path, &frame);
            remain_len -= wlen;
            if (remain_len) {
                frame.len = remain_len;
            }
        } while (remain_len);
    } else {
...
static int file_dec_wait_capture_handler(void *p)
{
    printf("enter audio_dec_file.c ,sys_timer_add%d\n", __LINE__);
    if (!file_wait_capture->timer && app_get_curr_task() == APP_MUSIC_TASK) {
        printf("------%s----%d", __FUNCTION__, __LINE__);
        file_wait_capture->timer = sys_timer_add(NULL, file_audio_wait_capture_timer, SLIENCE_PERIOD_TIME);
    }

    return 0;
}

void audio_fill_data_task(void *p)
{
    int msg[32];
    while (1) {
//      os_taskq_pend(NULL, msg, ARRAY_SIZE(msg));
        ...
        switch (msg[1]) {
        case FILE_DEC_FILL_DATA:
            file_dec_wait_capture_handler((struct audio_decoder *)msg[2]);
            break;
        }
    }
}

#define FILE_AUDIO_CAPTURE_NONE     0
#define FILE_AUDIO_CAPTURE_START    1
#define FILE_AUDIO_CAPTURE_STOP     2
...
const struct dec_type  dec_clk_tb[] = {
    ...
    {AUDIO_CODING_OPUS, DEC_OPUS_CLK},
    {AUDIO_CODING_OGG, DEC_OGG_CLK},
...
*/
/*----------------------------------------------------------------------------*/
static int file_dec_src_output_handler(void *priv, void *data, int len)
{
    struct file_dec_src_hdl *src_hdl = (struct file_dec_src_hdl *)priv;

    struct audio_data_frame frame = {
        .data = data,
        .data_len = len,
        .channel = src_hdl->ch_num,
        .sample_rate = src_hdl->sample_rate,
    };

    audio_stream_run(&src_hdl->entry, &frame);

    return src_hdl->used_len;
}


static void file_dec_src_data_process_len(struct audio_stream_entry *entry, int len)
{
    struct file_dec_src_hdl *src_hdl = container_of(entry, struct file_dec_src_hdl, entry);

    src_hdl->used_len = len;
}


static int file_dec_src_data_handler(struct audio_stream_entry *entry,
                                     ...
                                     struct audio_data_frame *out)
{
    ...
    if (src_hdl->hw_src) {
        int wlen = audio_src_resample_write(src_hdl->hw_src, in->data, in->data_len);
        if (wlen < in->data_len) {
            audio_resample_hw_trigger_interrupt(src_hdl->hw_src, (void *)entry, (void (*)(void *))audio_stream_resume);
        }
        return wlen;
    }
...
static void file_dec_src_open(struct file_dec_hdl *dec, u16 in_sample_rate, u16 out_sample_rate)
{

    dec->src_hdl.ch_num = dec->file_dec.output_ch_num;
    dec->src_hdl.sample_rate = out_sample_rate;
    printf("file dec sr:%d, or:%d ", in_sample_rate, out_sample_rate);
    dec->src_hdl.hw_src = zalloc(sizeof(struct audio_src_handle));
    if (dec->src_hdl.hw_src) {
        if (global_bit_wide) {
            audio_hw_src_open(dec->src_hdl.hw_src, dec->file_dec.output_ch_num, SET_RESAMPLE_TYPE(AUDIO_SAMPLE_FMT_24BIT, SRC_TYPE_RESAMPLE));
        } else {
            audio_hw_src_open(dec->src_hdl.hw_src, dec->file_dec.output_ch_num, SRC_TYPE_RESAMPLE);
        }
        audio_hw_src_set_rate(dec->src_hdl.hw_src, in_sample_rate, out_sample_rate);
        audio_src_set_output_handler(dec->src_hdl.hw_src, &dec->src_hdl, file_dec_src_output_handler);
    }

    dec->src_hdl.entry.data_handler = file_dec_src_data_handler;
    dec->src_hdl.entry.data_process_len = file_dec_src_data_process_len;
}
/*----------------------------------------------------------------------------*/
/**@brief    改变解码后播放状态
   @param
...
int file_decoder_stop_handler_app(struct audio_decoder *decoder)	// 解码结束
{
    ...
    struct file_decoder *file_dec = container_of(decoder, struct file_decoder, decoder);
    struct file_dec_hdl *dec = container_of(file_dec, struct file_dec_hdl, file_dec);
    if (dec->pcm_capture == FILE_AUDIO_CAPTURE_START) {
        static u8 capture_mutex_init_flag = 0;
        int ret = 0;
        if (capture_mutex_init_flag == 0) {
            capture_mutex_init_flag = 1;
            os_mutex_create(&file_audio_wait_capture_mutex);
        }
        if (!file_wait_capture) {
            file_wait_capture = (struct file_audio_wait_capture *)zalloc(sizeof(struct file_audio_wait_capture));
            if (!file_wait_capture) {
                return 0;
            }
        }

        file_wait_capture->sample_rate = dec->capture_sample_rate;
        file_wait_capture->nch = dec->pcm_ch_num;
        file_wait_capture->path = dec->path;
        file_wait_capture->write_frame = dec->write_frame;
        ret = task_create(audio_fill_data_task, NULL, "audio_fill_data");
        if (ret) {
            printf("audio_fill task create failed ");
        }
        ret = os_taskq_post("audio_fill_data", 2, FILE_DEC_FILL_DATA, (void *)file_wait_capture);
        if (ret > 0) {
            r_printf("post file dec fade out message failed %d", ret);
        }
    }
#endif
    return 0;
...
*/
/*----------------------------------------------------------------------------*/
void file_audio_capture_wakeup(void *priv)
{
    if (!file_dec) {
        return ;
    }
...
*/
/*----------------------------------------------------------------------------*/
struct file_pcm_capture_hdl {
    struct audio_stream_entry entry;
} pcm_capture_node;

static void file_pcm_capture_process_len(struct audio_stream_entry *entry,  int len)
{
}
// 解码输出数据流节点数据处理回调
static int file_pcm_capture_data_handler(struct audio_stream_entry *entry,
        struct audio_data_frame *in,
        struct audio_data_frame *out)
{
    struct file_pcm_capture_hdl *hdl = container_of(entry, struct file_pcm_capture_hdl, entry);
    int wlen = 0;
    /* putchar('D'); */
    if (file_dec->write_frame) {
        struct audio_frame frame = {
            .mode = AUDIO_FRAME_FILE_STREAM,
            .nch = file_dec->pcm_ch_num,
            .sample_rate = file_dec->file_dec.decoder.fmt.sample_rate,
            .len = in->data_len,
            .data = in->data,
            .offset = in->offset,
        };
        wlen = file_dec->write_frame(file_dec->path, &frame);
    }
...
static int file_dec_start(void)
{
    ...
    //更新参数到同步
    if (dec->pcm_capture == FILE_AUDIO_CAPTURE_START) {
        dec->capture_sample_rate = dec->file_dec.sample_rate;
        audio_decoder_set_handler(&dec->file_dec.decoder, &file_decoder_handler_app);
    }

#if (SOUNDCARD_ENABLE)
    file_mix_fifo_ch = mix_fifo_ch_open(usb_mix_fifo, audio_output_channel_num());
#endif

#if (RECORDER_MIX_EN)
    dec->rec_mix_ch = rec_mix_fifo_ch_open(audio_output_channel_num());
#endif

__open_ok:
    if (dec->pcm_capture == FILE_AUDIO_CAPTURE_NONE) {
#if CHECK_SR_WHEN_DECODING
    ...
    if (dec->file_dec.dec_no_out_sound == 0) {
        ...
        int ret = 0;//localtws_enc_api_open(&enc_f, 0);
        if (ret == true) {
            dec->file_dec.dec_no_out_sound = 1;
            // 重定向mixer
            p_mixer = NULL;//&g_localtws.mixer;
            // 关闭资源等待。最终会在localtws解码处等待
            audio_decoder_task_del_wait(&decode_task, &dec->wait);
...
#if (SOUNDCARD_ENABLE || RECORDER_MIX_EN)
    if (dec->file_dec.sample_rate != audio_mixer_get_sample_rate(p_mixer)) {
        file_dec_src_open(dec, dec->file_dec.sample_rate, audio_mixer_get_sample_rate(p_mixer));
    }
#endif
    ...
    if (global_bit_wide) {
        dec->file_dec.decoder.fmt.bit_width = BIT_WIDE_24BIT;
        /* dec->convert_16_to_32 = convet_data_open(1 << 4 | 1, 2048); */
    } else {
#if defined(AUDIO_VBASS_32BIT_OUT_EN)&&AUDIO_VBASS_32BIT_OUT_EN
        dec->convert_32_to_16 = convet_data_open(0, 2048);
#endif

    }

#if AUDIO_VOCAL_REMOVE_EN
    if (dec->pcm_capture == FILE_AUDIO_CAPTURE_NONE) {//广播时不走这里的人声消除
        dec->vocal_hdl = vocal_remove_open(dec->effect_ch_num, dec->file_dec.sample_rate, 100, 200, 13000);
        if (dec->file_dec.output_ch_num != audio_output_channel_num()) {
            //解码输出通道数与dac输出通道数不一致，需要开启声道转换
            dec->vocal_ch_switch = channel_switch_open(dec->file_dec.output_ch_type, 0);
            channel_switch_set_bit_wide(dec->vocal_ch_switch, global_bit_wide);
            dec->effect_ch_num = audio_output_channel_num();
        }
    ...
    dec->vbass_prev_gain = audio_gain_open_demo(AEID_MUSIC_VBASS_PREV_GAIN, dec->effect_ch_num);
    dec->ns_gate = audio_noisegate_open_demo(AEID_MUSIC_NS_GATE, dec->file_dec.sample_rate, dec->effect_ch_num);
    //虚拟低音
    dec->vbass = audio_vbass_open_demo(AEID_MUSIC_VBASS, dec->file_dec.sample_rate, dec->effect_ch_num);

#endif

#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
    dec->harmonic_exciter = audio_harmonic_exciter_open_api(AEID_MUSIC_HARMONIC_EXCITER, dec->file_dec.sample_rate, dec->effect_ch_num);
#endif

#if TCFG_EQ_ENABLE && TCFG_AUDIO_OUT_EQ_ENABLE
    dec->high_bass = high_bass_eq_open(dec->file_dec.sample_rate, dec->effect_ch_num);
    dec->hb_drc = high_bass_drc_open(dec->file_dec.sample_rate, dec->effect_ch_num);
    ...
    dec->eq = music_eq_open(dec->file_dec.sample_rate, dec->effect_ch_num);// eq
#if TCFG_DRC_ENABLE && TCFG_MUSIC_MODE_DRC_ENABLE
    dec->drc = music_drc_open(dec->file_dec.sample_rate, dec->effect_ch_num);//drc
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_fr = music_drc_fr_open(dec->file_dec.sample_rate, dec->effect_ch_num);//drc
#endif
    ...
    dec->ext_eq = music_ext_eq_open(dec->file_dec.sample_rate, dec->effect_ch_num);
#endif

#if defined(TCFG_DYNAMIC_EQ_ENABLE) && TCFG_DYNAMIC_EQ_ENABLE
    dec->eq2 = music_eq2_open(dec->file_dec.sample_rate, dec->effect_ch_num);// eq
#if defined(TCFG_DYNAMIC_EQ_PRO_ENABLE) && !TCFG_DYNAMIC_EQ_PRO_ENABLE
    dec->dy_eq = audio_dynamic_eq_ctrl_open(AEID_MUSIC_DYNAMIC_EQ, dec->file_dec.sample_rate, dec->effect_ch_num);//动态eq
#else
    dec->dy_eq_pro = audio_dynamic_eq_pro_open_api(AEID_MUSIC_DYNAMIC_EQ, dec->file_dec.sample_rate, dec->effect_ch_num);//动态eq
#endif
    /* dec->last_drc = music_last_drc_open(dec->file_dec.sample_rate, dec->effect_ch_num); */
    ...
    dec->gain = audio_gain_open_demo(AEID_MUSIC_GAIN, dec->effect_ch_num);
#endif
...
#if TCFG_EQ_ENABLE && TCFG_MUSIC_MODE_EQ_ENABLE
    dec->eq_rl_rr = music_eq_rl_rr_open(dec->file_dec.sample_rate, dec->effect_ch_num);// eq
#if TCFG_DRC_ENABLE &&TCFG_MUSIC_MODE_DRC_ENABLE
    dec->drc_rl_rr = music_drc_rl_rr_open(dec->file_dec.sample_rate, dec->effect_ch_num);//drc
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    dec->drc_rr = music_drc_rr_open(dec->file_dec.sample_rate, dec->effect_ch_num);//drc
#endif
#endif
    if (!global_bit_wide) {
...
#if defined(TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE) && TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE
    dec->gain_rl_rr = audio_gain_open_demo(AEID_MUSIC_RL_GAIN, dec->effect_ch_num);
#endif
    ...
    dec->ext_eq2 = music_ext_eq2_open(dec->file_dec.sample_rate, dec->effect_ch_num);
#endif
    ...
    if (dec->pcm_capture == FILE_AUDIO_CAPTURE_START) {
        pcm_capture_node.entry.data_process_len = file_pcm_capture_process_len;
        pcm_capture_node.entry.data_handler = file_pcm_capture_data_handler;
        entries[entry_cnt++] = &(pcm_capture_node.entry);
    ...
    if (dec->pick_flag) {
        ...
        entries[entry_cnt++] = 0;//&g_localtws.push.entry;
    } else
#endif
    {
#if TCFG_SPEED_PITCH_ENABLE
...
#if (SOUNDCARD_ENABLE)
        if (dec->src_hdl.hw_src) {
            entries[entry_cnt++] = &dec->src_hdl.entry;

        }
        entries[entry_cnt++] = mix_fifo_ch_get_entry(file_mix_fifo_ch);
#elif (RECORDER_MIX_EN)
        if (dec->src_hdl.hw_src) {
            entries[entry_cnt++] = &dec->src_hdl.entry;
...
__err3:
    ...
    if (dec->src_hdl.hw_src) {
        audio_hw_src_stop(dec->src_hdl.hw_src);
        audio_hw_src_close(dec->src_hdl.hw_src);
        free(dec->src_hdl.hw_src);
        dec->src_hdl.hw_src = NULL;

    }

    if (file_mix_fifo_ch != NULL) {
        void *entry = mix_fifo_ch_get_entry(file_mix_fifo_ch);
        if (entry != NULL) {
            audio_stream_del_entry(entry);
        }
    }
    mix_fifo_ch_close(file_mix_fifo_ch);
    file_mix_fifo_ch = NULL;
#elif (RECORDER_MIX_EN)
...
int file_dec_open(void *file, struct audio_dec_breakpoint *bp)
{
...
#if AUDIO_VOCAL_REMOVE_EN
    dec->file_dec.output_ch_num = 2;
#else
    dec->file_dec.output_ch_num = audio_output_channel_num();
#endif
    dec->effect_ch_num = dec->file_dec.output_ch_num;
    ...
    if (file_dec->pcm_capture == FILE_AUDIO_CAPTURE_START) {
...
*/
/*----------------------------------------------------------------------------*/
int file_audio_capture_open(void *file, struct audio_dec_breakpoint *bp, struct audio_path *opath)
{
    if ((!file_dec) || (!file) || !opath) {
        printf("file audio capture arguments error\n");
        return -EPERM;
    }
    file_dec->path = opath->output.path;
    file_dec->write_frame = opath->output.write_frame;
    file_dec->pcm_ch_num = opath->fmt.channel;
    file_dec->capture_sample_rate = opath->fmt.sample_rate;
    file_dec->pcm_capture = FILE_AUDIO_CAPTURE_START;
    return file_dec_open(file, bp);
}

void file_audio_capture_close(void)
{
    if (!file_dec) {
        return;
    }
    printf(">>>>>>>>>%s", __FUNCTION__);
    file_dec->pcm_capture = FILE_AUDIO_CAPTURE_STOP;
    file_dec_close();
}
/*----------------------------------------------------------------------------*/
/**@brief    关闭文件解码
...
void file_dec_close(void)
{
    ...
    printf(">>>>>>>>>%s   %d", __FUNCTION__, file_dec->pcm_capture);
    if (file_dec->pcm_capture == FILE_AUDIO_CAPTURE_STOP) {
        file_audio_wait_capture_close();
    }
    if (file_dec->file_dec.status) {
...
#endif
#if TCFG_AUDIO_EFFECT_TASK_ENABLE
        if (file_dec->effect_task_entry) {
            audio_effect_task_close(file_dec->effect_task_entry);
        }
#endif
#if AUDIO_VOCAL_REMOVE_EN
        if (file_dec->vocal_hdl) {
            set_vocal_remove_hdl(0);
            audio_vocal_remove_close(file_dec->vocal_hdl);
            channel_switch_close(&file_dec->vocal_ch_switch);
        }
#endif
#if AUDIO_SURROUND_CONFIG
        if (file_dec->surround) {
...
#endif
        if (file_dec->convert_16_to_32) {
            convet_data_close(file_dec->convert_16_to_32);
        }
        if (file_dec->convert_32_to_16) {
            convet_data_close(file_dec->convert_32_to_16);
        }

#if defined(TCFG_MUSIC_HARMONIC_EXCITER_ENABLE) &&TCFG_MUSIC_HARMONIC_EXCITER_ENABLE
        if (file_dec->harmonic_exciter) {
            audio_harmonic_exciter_close_api(file_dec->harmonic_exciter);
        }
#endif

#if TCFG_EQ_ENABLE && TCFG_AUDIO_OUT_EQ_ENABLE
        if (file_dec->high_bass) {
...
#else
        if (file_dec->dy_eq_pro) {
            audio_dynamic_eq_pro_close_api(file_dec->dy_eq_pro);
        }
#endif
        if (file_dec->last_drc) {
            music_last_drc_close(file_dec->last_drc);
        }
        if (file_dec->convert2) {
...
#if TCFG_EQ_DIVIDE_ENABLE&& !WIRELESS_SOUND_TRACK_2_P_X_ENABLE
        if (file_dec->eq_rl_rr) {
            music_eq_rl_rr_close(file_dec->eq_rl_rr);
        }
        if (file_dec->drc_rl_rr) {
...
#endif
        if (file_dec->convert_rl_rr) {
            convet_data_close(file_dec->convert_rl_rr);
        }
#if defined(TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE) && TCFG_PHASER_GAIN_AND_CH_SWAP_ENABLE
        if (file_dec->gain_rl_rr) {
...
#endif
#if SOUND_TRACK_1_P_1_CH_CONFIG && (EFFECT_PROCESS_POITION_CONFIG == EFFECT_BEFORE_STEREO_MIX)
        if (file_dec->fl_fr_ch_2to1) {
            channel_switch_close(&file_dec->fl_fr_ch_2to1);
        }
        if (file_dec->rl_rr_ch_2to1) {
            channel_switch_close(&file_dec->rl_rr_ch_2to1);
        }
#endif
        ...
        if (file_dec->src_hdl.hw_src) {
            audio_hw_src_stop(file_dec->src_hdl.hw_src);
            audio_hw_src_close(file_dec->src_hdl.hw_src);
            free(file_dec->src_hdl.hw_src);
            file_dec->src_hdl.hw_src = NULL;
        ...
        if (file_dec->rec_mix_ch != NULL) {
            void *entry = rec_mix_fifo_ch_get_entry(file_dec->rec_mix_ch);
            if (entry != NULL) {
                audio_stream_del_entry(entry);
            }
        }
        rec_mix_fifo_ch_close(file_dec->rec_mix_ch);
        file_dec->rec_mix_ch = NULL;
#endif
...
#if TCFG_EQ_DIVIDE_ENABLE
        if (file_dec->vocal_tract.stream) {
            audio_stream_close(file_dec->vocal_tract.stream);
            file_dec->vocal_tract.stream = NULL;
        }
#endif

    }
    file_dec->pcm_capture = FILE_AUDIO_CAPTURE_NONE;
...
#if TCFG_AUDIO_DAC_BIT_WIDTH
        rep.repair_buf = &dec->repair_buf_24bit;
#else
...
/*----------------------------------------------------------------------------*/
#if AUDIO_VOCAL_REMOVE_EN
void *file_vocal_remove_hdl_get()
{
#if TCFG_APP_MUSIC_EN
    if (file_dec) {
        return file_dec->vocal_hdl;
    } else {
        return NULL;
    }
#else
