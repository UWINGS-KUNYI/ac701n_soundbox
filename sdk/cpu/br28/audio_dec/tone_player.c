...
*/
/*----------------------------------------------------------------------------*/
static int tone_play_open_with_callback_deal(const char **list, u8 follow, u8 preemption, void (*evt_handler)(void *priv, int flag), void *evt_priv, int sync_confirm_time)
{
    if (follow && tone_dec) {
    ...
    if (tone_dec && tone_dec->dec_file && tone_dec->dec_file->dec) {
        if (audio_dec_app_decoder_fade_out(tone_dec->dec_file->dec)) { //淡出结束后再结束解码
            printf("tone dec fade out end\n");
        }
    }
...
static int tone_play_open_with_task_callback(int param)
{
    struct tone_play_task_cb_info *info = (struct tone_play_task_cb_info *)param;
    //printf("tone_play_open_with_task_callback \n");
    return tone_play_open_with_callback_deal((const char **)info->list, info->follow, info->preemption, info->evt_handler, info->evt_priv, info->sync_confirm_time);
}
...
int tone_play_open_with_callback_base(const char **list, u8 follow, u8 preemption, void (*evt_handler)(void *priv, int flag), void *evt_priv, int sync_confirm_time)
{
    if (strcmp(os_current_task(), "app_core")) {
        //printf("toneplay task:%s \n", os_current_task());
        OS_SEM *sem = malloc(sizeof(OS_SEM) + 4 + sizeof(struct tone_play_task_cb_info));
        ASSERT(sem);
        int *err = (int *)(sem + 1);
        struct tone_play_task_cb_info *info = (struct tone_play_task_cb_info *)(err + 1);
        info->list = (int)list;
        info->follow = follow;
        info->preemption = preemption;
        info->evt_handler = evt_handler;
        info->evt_priv = evt_priv;
        info->sync_confirm_time = sync_confirm_time;

        int msg[5] = {0};
        msg[0] = (int)tone_play_open_with_task_callback;
        msg[1] = 1 | BIT(8) | BIT(9);
        msg[2] = (int)info;
        msg[3] = (int)err;
        msg[4] = (int)sem;
        ...
        do {
            int os_err = os_taskq_post_type("app_core", Q_CALLBACK, 5, msg);
    ...
    return tone_play_open_with_callback_deal(list, follow, preemption, evt_handler, evt_priv, sync_confirm_time);
}
...
{
    return tone_play_open_with_callback_base(list, follow, preemption, evt_handler, evt_priv, 0);
}
