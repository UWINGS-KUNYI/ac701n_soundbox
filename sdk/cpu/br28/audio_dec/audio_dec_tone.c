...
const int TONE_DEC_PP_FADE_MS = 50;	// 提示音被打断时的淡出时长。0-不使用淡入淡出
//////////////////////////////////////////////////////////////////////////////
...
static int tone_dec_file_app_evt_cb(void *priv, enum audio_dec_app_event event, int *param)
{
    ...
    case AUDIO_DEC_APP_EVENT_START_INIT_OK:
        ...
        file_dec->flag = AUDIO_DEC_FILE_FLAG_AUDIO_STATE_TONE; // 使用提示音音量
    ...
    case AUDIO_DEC_APP_EVENT_STREAM_OPEN:
        ...
        if (global_bit_wide) {
            /* dec->convert = convet_data_open((1 << 4 | 1), 2048); */
            /* if (dec->convert) { */
            /* (entries_hdl->entries_addr)[entries_hdl->entries_cnt++] = &dec->convert->entry; */
            /* } */
        }
...
static int tone_dec_sine_app_evt_cb(void *priv, enum audio_dec_app_event event, int *param)
{
    ...
    case AUDIO_DEC_APP_EVENT_START_INIT_OK:
        ...
        clock_add_set(DEC_TONE_CLK);
        sine_dec->flag = AUDIO_DEC_FILE_FLAG_AUDIO_STATE_TONE; // 使用提示音音量
    ...
    case AUDIO_DEC_APP_EVENT_STREAM_OPEN:
        ...
        if (global_bit_wide) {
            dec->convert = convet_data_open((1 << 4 | 1), 512);
            if (dec->convert) {
                (entries_hdl->entries_addr)[entries_hdl->entries_cnt++] = &dec->convert->entry;
            }
        }
...
#if (RECORDER_MIX_EN)
static struct audio_stream_entry *tone_dec_to_rec_mix_open(struct audio_dec_app_hdl *dec)
{
    dec->rec_mix_ch = rec_mix_fifo_ch_open(dec->ch_num);
    if (dec->rec_mix_ch) {
        return rec_mix_fifo_ch_get_entry(dec->rec_mix_ch);
    }
    return NULL;
}

static void tone_dec_to_rec_mix_close(struct audio_dec_app_hdl *dec)
{
    if (!dec->rec_mix_ch) {
        return;
    }

    struct audio_stream_entry *entry = rec_mix_fifo_ch_get_entry(dec->rec_mix_ch);
    if (entry) {
...
#if (SOUNDCARD_ENABLE)
static struct audio_stream_entry *tone_dec_to_uac_entry_open(struct audio_dec_app_hdl *dec)
{
    dec->uac_mix_ch = mix_fifo_ch_open(usb_mix_fifo, dec->ch_num);
    if (dec->uac_mix_ch) {
        return mix_fifo_ch_get_entry(dec->uac_mix_ch);
    }
    return NULL;
}

static void tone_dec_to_uac_entry_close(struct audio_dec_app_hdl *dec)
{
    if (!dec->uac_mix_ch) {
        return;
    }

    struct audio_stream_entry *entry = mix_fifo_ch_get_entry(dec->uac_mix_ch);
    if (entry) {
        audio_stream_del_entry(entry);
    }
    mix_fifo_ch_close(dec->uac_mix_ch);
    dec->uac_mix_ch = NULL;
}
#endif
...
#if (SOUNDCARD_ENABLE)
    audio_dec_app_set_up_to_uac_info(dec, tone_dec_to_uac_entry_open, tone_dec_to_uac_entry_close);
#endif

#if (RECORDER_MIX_EN && TONE_RECORDER_MIX_EN)
    audio_dec_app_set_up_to_uac_info(dec, tone_dec_to_rec_mix_open, tone_dec_to_rec_mix_close);
#endif
}
