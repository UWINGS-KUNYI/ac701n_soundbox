...
struct audio_mic_hdl {
    ...
    adc_type_t adc_buf[AUDIO_ADC_BUFS_SIZE];    //align 2Bytes
    s16 output_buf[AUDIO_ADC_BUFS_SIZE];	//数据输出缓存
    cbuffer_t output_cbuf;
    u16 dump_packet;
    struct audio_eq *mic_dut_eq_hdl;
};
...
void *mic_dut_eq_open(u32 sample_rate, u8 ch_num)
{
    ...
    eq_param.max_nsection = mic_dut_eq.seg_num;
    eq_param.nsection = mic_dut_eq.seg_num;
    eq_param.seg = mic_dut_eq.seg;
    eq_param.global_gain = mic_dut_eq.global_gain;
    eq_param.cb = eq_get_filter_info;
    eq_param.eq_name = AEID_MIC_DUT_EQ;

    struct audio_eq *eff = audio_dec_eq_open(&eq_param);
    return eff;
#else
    return NULL;
...
void mic_dut_eq_run(struct audio_eq *eq_drc, void *data, u32 len)
{
    struct audio_eq *eff_hdl = (struct audio_eq *)eq_drc;
#if TCFG_EQ_ENABLE
    if (eff_hdl) {
        audio_eq_run(eff_hdl, data, len);
    }
#endif/*TCFG_EQ_ENABLE*/
}

void mic_dut_eq_close(struct audio_eq *eq_drc)
{
#if TCFG_EQ_ENABLE
    struct audio_eq *eff_hdl = (struct audio_eq *)eq_drc;
    if (eff_hdl) {
        audio_dec_eq_close(eff_hdl);
        eff_hdl = NULL;
    }
#endif /*TCFG_EQ_ENABLE*/
}
...
int audio_mic_dut_info_get(mic_dut_info_t *info)
{
    mic_dut_info_t mic_info = {
        ...
        .channel = 1,
        .bit_wide = 16,
        .sr_enable_bit = MIC_ADC_SR_8000 | MIC_ADC_SR_16000 | MIC_ADC_SR_32000 | MIC_ADC_SR_44100,
        .sr_default = 32000,
        .mic_gain = 19,
        .mic_gain_default = 10,
        .dac_gain = 15,
...
static struct audio_sound_dac *demo_dac = NULL;
...
int audio_mic_dut_dac_start(u32 sr, u16 vol)
{
    ...
    demo_dac_param.sample_rate = sr;
    demo_dac_param.delay_ms = 50;
    demo_dac = audio_sound_dac_open(&demo_dac_param);
    if (demo_dac == NULL) {
        goto _error;
    }
    int ret = audio_sound_dac_start(demo_dac);
    if (ret) {
        goto _error;
    }
    return 0;
...
int audio_mic_dut_dac_write(s16 *data, int len)
{
    ...
    int wlen = audio_sound_dac_write(demo_dac, dac_data, data_len);
    if (wlen != data_len) {
        printf("dut scan dac write fail %d, %d!!! \n", wlen, data_len);
    }
    return wlen;
}
...
#if MIC_DUT_EQ_ENABLE
        audio_mic->mic_dut_eq_hdl = mic_dut_eq_open(sr, 1);
#endif /*MIC_DUT_EQ_ENABLE*/
...
#if MIC_DUT_EQ_ENABLE
            mic_dut_eq_close(audio_mic->mic_dut_eq_hdl);
#endif /*MIC_DUT_EQ_ENABLE*/
...
void audio_mic_dut_eq_run(s16 *data, int len)
{
    ...
    if (scan_start && audio_mic) {
        mic_dut_eq_run(audio_mic->mic_dut_eq_hdl, data, len);
    }
#endif /*MIC_DUT_EQ_ENABLE*/
}
