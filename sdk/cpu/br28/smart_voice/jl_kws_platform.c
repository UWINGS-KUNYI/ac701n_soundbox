...
#if defined(CONFIG_WATCH_CASE_ENABLE) || defined(CONFIG_SOUNDBOX_CASE_ENABLE)
#include "clock_cfg.h"
#endif /*defined(CONFIG_WATCH_CASE_ENABLE) || defined(CONFIG_SOUNDBOX_CASE_ENABLE)*/
...
#define KWS_MULTI_USE_VAD_MIC_EN    0
...
{
#if KWS_MULTI_USE_VAD_MIC_EN
    return __audio_smart_voice_detect_open(model == JL_KWS_COMMAND_KEYWORD ? VOICE_VAD_MIC : VOICE_MCU_MIC, model);
#else
    return __audio_smart_voice_detect_open(model == JL_KWS_COMMAND_KEYWORD ? VOICE_MCU_MIC : VOICE_MCU_MIC, model);
#endif
}
...
#else
    smart_voice_core_post_msg(5, SMART_VOICE_MSG_SWITCH_SOURCE, VOICE_MCU_MIC, VOICE_DATA_BUFFER_SIZE, SMART_VOICE_SAMPLE_RATE, JL_KWS_COMMAND_KEYWORD);
#endif
