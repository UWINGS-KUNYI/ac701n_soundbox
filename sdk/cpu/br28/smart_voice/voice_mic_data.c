...
struct main_adc_context {
    ...
    adc_type_t dma_buf[VOICE_MIC_DATA_PERIOD_FRAMES * 2 * MAIN_ADC_CH_NUM];
    /* s16 *dma_buf; */
...
{
    struct voice_mic_data *voice = (struct voice_mic_data *)priv;
    if (bt_media_is_running() && voice_aec_hdl) {
        int wlen = 0;
        ...
        extern void audio_aec_refbuf(s16 * buf, u16 len);
        wlen = audio_dac_read(0, voice_aec_hdl->ref_tmpbuf, len * 3);
        audio_aec_refbuf(voice_aec_hdl->ref_tmpbuf, len * 3);

        static u8 burk = 0;
        if (cbuf_get_data_len(&voice_aec_hdl->aec_cbuf) >= 512) {
            wlen = cbuf_read(&voice_aec_hdl->aec_cbuf, &(voice_aec_hdl->int_tmpbuf[burk][0]), 512);
            audio_aec_inbuf(&(voice_aec_hdl->int_tmpbuf[burk][0]), wlen);
            burk++;
            if (burk > 2) {
                burk = 0;
            }
...
int audio_smart_voice_aec_open(void)
{
    ...
    voice_aec_hdl = zalloc(sizeof(struct voice_aec_hdl_t));
    if (voice_aec_hdl == NULL) {
        printf("voice_aec_hdl malloc fail !!!");
        return -1;
    }

    cbuf_init(&voice_aec_hdl->aec_cbuf, voice_aec_hdl->aec_data_buf, sizeof(voice_aec_hdl->aec_data_buf));
    audio_aec_open(16000, AEC_EN, cvp_output_hdl);
    audio_dac_read_reset();
    return 0;
}
...
static void audio_main_adc_dma_data_handler(void *priv, s16 *data, int len)
{
    ...
    audio_convert_data_32bit_to_16bit_round(data, data, (len >> 2) * MAIN_ADC_CH_NUM);
    len >>= 1;
#endif
    s16 *pcm_data = data;
#if (MAIN_ADC_CH_NUM > 1)
...
static int audio_main_adc_mic_open(struct voice_mic_data *voice)
{
    ...
    u16 mic0_gain = MAIN_ADC_GAIN;
    audio_adc_mic_open(&voice->main_adc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    audio_adc_mic_set_gain(&voice->main_adc->mic_ch, mic0_gain);
#endif
#if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_1)
    u16 mic1_gain = MAIN_ADC_GAIN;
    audio_adc_mic1_open(&voice->main_adc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    audio_adc_mic1_set_gain(&voice->main_adc->mic_ch, mic1_gain);
#endif
#if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_2)
    u16 mic2_gain = MAIN_ADC_GAIN;
    audio_adc_mic2_open(&voice->main_adc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    audio_adc_mic2_set_gain(&voice->main_adc->mic_ch, mic2_gain);
#endif
#if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_3)
    u16 mic3_gain = MAIN_ADC_GAIN;
    audio_adc_mic3_open(&voice->main_adc->mic_ch, TCFG_AUDIO_ADC_MIC_CHA, &adc_hdl);
    audio_adc_mic3_set_gain(&voice->main_adc->mic_ch, mic3_gain);
#endif
